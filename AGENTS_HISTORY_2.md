...
12. **NEVER DROP THE STASH.** `git stash drop` destroys the only reference to uncommitted work. A dropped stash is a dangling commit that can be recovered from `git fsck --lost-found`, but it's easy to miss and hard to identify. If a stash has conflicts, resolve them — never drop and recreate. The stash is a living artifact of a prior session's context; losing it loses history, intent, and the boundary between what was committed and what was not.
