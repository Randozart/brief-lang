# Briev Compiler Refactor: Pattern B Architecture
...
Refactor the ~78K-line Briev compiler from monolithic match-arm dispatching to a **Pattern B (Struct-Variant Delegation)** architecture, where each AST construct lives in its own feature file with co-located parsing, typechecking, evaluation, and codegen logic.
...
CLI: `briev run --test --group config` runs only tests tagged with `"config"`.
...
| **Benchmark as guard** | `build_and_bench.sh` after every phase — if Briev beats C by an implausible margin, something is wrong. |
