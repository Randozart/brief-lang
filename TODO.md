...
The harness at line 163 compiles `brief_rt.c` to `/tmp/brief_rt.o` with `-flto`, then tries to link it with the benchmark's `.ll` file. But the compiler's internal linking has already created FFI cached `.o` files that conflict.

**Fix (verified working):** Replace lines 160-170 with one-step clang:

```bash
if [ ! -f "$bin" ]; then
    clang -O3 "benchmarks/${name}.ll" "lib/runtime/brief_rt.c" \
        -lm -o "$bin" 2>&1
fi
```
