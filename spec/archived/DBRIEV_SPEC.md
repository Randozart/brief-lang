...
DBriev (Data Briev) is a declarative, contract-enforced data language that combines the best of TOML, Prolog/Datalog, and SQL into a unified verifiable data layer. It treats data as logical propositions that can be queried, validated, and synchronized across systems.
...
5. ** Briev-Native**: Queries feel like Briev state machine syntax, not foreign SQL.

### 1.2 Relationship to Briev

| Briev Variant | Purpose | DBriev Role |
|-------------|--------|-------------|
| `.bv` | Core state machines | Can query DBriev data via FFI |
| `.rbv` | Web views | Binds to DBriev for reactive data |
| `.ebv` | Embedded hardware | Reads DBriev config for registers |
...
The ALIAS keyword provides **logical to physical address separation**, enabling the same Briev code to target different hardware by swapping the `.dbv` config file.
...
DBriev supports three complementary query styles that don't conflict with Briev keywords:
...
```briev
// Auto-generate REST from DBriev schema
REST @users {
    GET /users           // -> @users->ALL
