# Rendered Briev (.rbv) — Specification v1.0
...
Rendered Briev (`.rbv`) is a single-file reactive UI format where Briev logic and HTML/CSS views coexist. Briev is the reactive state engine; HTML and CSS are declarative projections of that state.

### Design Philosophy

- **Briev owns all state.** The view is a passive mirror.
...
- **Compile-time proofs extend to the view.** Directive references are validated against Briev state at compile time.
...
```html
<script type="briev">
  // Briev code: imports, state, transactions, definitions
</script>

<view>
  <!-- HTML with Briev directives -->
</view>
...
| Rule | Enforcement |
|------|-------------|
| Exactly one `<script type="briev">` block | Compile error if missing or duplicate |
...
The `<script type="briev">` block accepts standard Briev syntax:
...
Briev supports the `List<T>` generic type for homogeneous collections:
...
Directives are HTML attributes prefixed with `b-`. They bind DOM elements to Briev state.
...
Expressions in directive values are a subset of Briev expressions:
...
```rust
fn extract_blocks(source: &str) -> Result<(BrievSource, ViewSource, Option<CssSource>), Error> {
    // Find <script type="briev">...</script>
    // Find <view>...</view>
...
### 6.3 Briev Compilation

The Briev section is compiled using the existing Briev compiler infrastructure:
- Lexer
- Parser
- Type checker
...
| Code | Message |
|------|---------|
| RBV001 | Missing `<script type="briev">` block |
| RBV002 | Missing `<view>` block |
| RBV003 | Duplicate `<script type="briev">` block |
...
All Briev compilation errors pass through with RBV prefix.
...
```
dist/
├── component.wasm    # Compiled Briev → WASM
...
```bash
# Compile single file
brievc compile component.rbv --out dist/

# Watch mode
brievc watch src/ --out dist/
```
