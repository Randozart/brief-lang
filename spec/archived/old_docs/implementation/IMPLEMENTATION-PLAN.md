...
This document outlines the implementation roadmap for the Briev compiler, focusing on completing the contract verification system and fixing known issues in the standard library.
...
### Goal
Enhance error messages to teach Briev.
...
**6.3 Fix Suggestions**
- Suggest specific changes in Briev syntax
...
3. **Design Rationale**: Briev's goal is "if it runs, it won't have bugs." A conservative verifier that accepts valid programs is preferable to one that rejects valid programs. As the verifier matures, we can strengthen these checks.
...
- `spec/v4-briev-lang-spec.md` - Previous version for reference
