# Getting Started with Briev

**Version:** 1.0  
**Purpose:** Your first steps learning Briev programming  

---

## Table of Contents

1. [What is Briev?](#what-is-briev)
2. [Prerequisites](#prerequisites)
3. [Installing Briev](#installing-briev)
...
7. [Key Concepts in Briev](#key-concepts-in-briev)
...
Briev is a programming language designed to help you write **bug-free code** from the start.

Most programming languages let you write anything, even things that don't make sense, and then you spend hours debugging later. Briev takes a different approach: **you write a contract first, then write the code**.
...
A **contract** in Briev has two parts:

1. **Precondition** - What must be true before your code runs
2. **Postcondition** - What must be true after your code runs

If the preconditions aren't met, Briev won't let the code run. If the postconditions aren't met, Briev tells you something went wrong. This catches bugs at the moment they happen, not hours later when you're trying to figure out why your program crashed.

### What Can You Build with Briev?

Briev is designed to work in multiple contexts:

- **Web applications** - Briev compiles to WebAssembly, so it runs in browsers
- **Backend services** - Can handle business logic with contracts ensuring correctness
- **FFI libraries** - Briev can call functions from other languages (Rust, C, Python, JavaScript, Go, and more)
...
- **Symbolic verification** - Briev can mathematically prove your contracts are achievable
...
Before learning Briev, you should have:
...
If you have experience in any programming language—JavaScript, Python, C, Rust, or even BASIC—you have enough background for Briev.

### Software You Need

1. **A text editor** - We recommend [Visual Studio Code](https://code.visualstudio.com/) because it's free and works well with Briev
2. **A terminal/command line** - Briev runs from the command line. On Windows, this is PowerShell or Command Prompt. On Mac/Linux, it's called Terminal.
3. **Rust** (for building Briev from source) - Installation instructions below
...
Briev is currently distributed as source code that you compile yourself. This sounds intimidating but is actually quite simple.

### Step 1: Install Rust

Briev is written in Rust, so you need Rust installed first.
...
```bash
# Clone the Briev repository (or navigate to where you downloaded it)
git clone https://github.com/your-username/briev-compiler.git
cd briev-compiler

# Build Briev in release mode (this takes a few minutes the first time)
cargo build --release
```
...
```bash
export PATH="$PATH:/path/to/briev-compiler/target/release"
```
...
This installs Briev to `~/.local/bin/briev`.
...
```
Briev Compiler v0.1.0

Usage: briev <command> [options] [file]
...
**"briev: command not found"**

- Make sure the PATH includes where Briev is installed (`~/.local/bin` or the target directory)
...
Let's create your first Briev program. We'll make a simple program that says "Hello, World!"
...
hello-briev/
├── src/
│   └── main.bv      # Your Briev code lives here
├── Cargo.toml        # Rust project settings (for compiling to WebAssembly)
└── briev.toml       # Briev project settings
```
...
Back in your terminal, from the `hello-briev` folder:
...
You've just written and run your first Briev program. Let's break down exactly what you wrote.
...
The word `txn` is short for "transaction." A transaction is a unit of work in Briev—it's how you define something your program can *do*.
...
The contract is where Briev's magic happens. It has two parts:
...
After the transaction runs, the result should equal `"Hello, World!"`. The variable `result` is special in Briev—it holds the value that the transaction produces.
...
When Briev verifies your program, it checks that `result` will indeed equal `"Hello, World!"` when the transaction completes. If it can't prove this mathematically, it will give you an error.
...
Here's what makes Briev special: try changing `[count < 100]` to `[true]` and running:
...
Briev noticed that `[true]` accepts any state, and that's suspicious. If increment can run *always*, what's stopping it from running when count is already 100?

This is Briev catching a potential bug before you even run the code.

---

## Key Concepts in Briev
...
Transactions are the main building blocks of Briev programs. When something needs to happen, you write a transaction for it.
...
Briev uses these boolean values:
...
You've learned the basics of Briev! Here's where to go from here.
...
- **[hello-world](../examples/hello-world/)** - The simplest possible Briev project
...
Briev can call functions from other languages:
...
- **[CLI-GUIDE.md](../spec/CLI-GUIDE.md)** - Complete command reference for the Briev compiler
...
**FFI (Foreign Function Interface)** - How Briev calls functions written in other languages.
...
**Mapper** - A component that translates between Briev types and foreign language types.
...
**Symbolic verification** - Briev's ability to mathematically prove that contracts can be satisfied.

**Transaction (`txn`)** - A named unit of work with a contract and a body.

---

*Welcome to Briev! We're glad you're here.*
