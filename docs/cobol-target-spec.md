...
```cobol
* BRIEF PRE-CONDITION GUARDS
IF LS-AMOUNT <= 0
    DISPLAY "BRIEF CONTRACT FAILED: PRECONDITION_01: amount > 0"
    MOVE 4000 TO RETURN-CODE
    GOBACK
END-IF.
IF LS-BALANCE < LS-AMOUNT
    DISPLAY "BRIEF CONTRACT FAILED: PRECONDITION_02: balance >= amount"
    MOVE 4000 TO RETURN-CODE
    GOBACK
END-IF.
```
...
IF LS-BALANCE NOT = WS-EXPECTED
    DISPLAY "BRIEF CONTRACT FAILED: POSTCONDITION_01: balance == old(balance) - amount"
    MOVE 4000 TO RETURN-CODE
    GOBACK
END-IF.
```
...
IF NOT (WS-SYSTEM-READY = 'Y')
    DISPLAY "BRIEF CONTRACT FAILED: PRECONDITION_01"
    MOVE 4000 TO RETURN-CODE
