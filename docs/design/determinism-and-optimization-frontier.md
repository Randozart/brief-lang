...
Because Brief programs form an assignment dependency graph (a value implies other values through transactions), boundedness is *also* graph-structured. The bounds on a `trg` or FFI value are not isolated — they propagate through every expression and transaction that reads them.
...
- **Overflow**: `a + 1` where `a = MAX_INT` — the arithmetic is well-defined by Brief's semantics (saturating or wrapping, per contract), but bound propagation must account for it.
...
A `trg` declaration says "this value may change at any tick." But "any tick" is not always an unknowable interval. In embedded Brief, `trg` variables are wired to direct MMIO addresses:
...
This is a fundamentally different model from a global reactive framework (like FRP with a global push/pull phase). Brief's `trg` is a **local equilibrium break** — the rest of the system doesn't even notice.
