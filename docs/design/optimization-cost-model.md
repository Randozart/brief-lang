...
- The `rbv` (Rendered Brief) path already targets browser rendering — a WebGPU shader backend is the natural extension
...
Emission uses the existing `emit_statement` infrastructure — the fused body is valid Brief statements that the LLVM backend already handles. No new IR emitters needed.
...
Invoked via `brief-compiler llvm file.bv --optimize-report [--optimize-budget 256] [--optimize-size 10240]`:
