...
This document synthesizes the Brief compiler's optimization design — the decision tree the backend traverses and the rationale behind each path. See `llvm-backend-optimization-catalog.md` for the earlier 5-path formulation; this document reflects the full evolved pipeline.

---

## Core Principle: Contracts Enable Optimizations

Every optimization is sound **because** the programmer declares contracts. Without preconditions (guards), postconditions, and state declarations, the compiler would have to guess — Brief never guesses, it proves.
...
**Verified**: Kalman n=12, C=72, T=12, K=18, R=16, W=4 → peak=28 ≥ 16 → SLP disabled. Brief 0.71s vs C 0.75s.
...
### LTO Pipeline
**What**: `compile_to_bitcode()` compiles C sources to bitcode via clang. `link_and_optimize()` merges program bitcode with `brief_rt.c` and any `import "link/..."` dependencies via `llvm-link`, then runs `opt -O2` for cross-module optimization.
