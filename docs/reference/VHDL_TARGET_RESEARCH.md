# Research: VHDL Target for Embedded Briev

## Concept: SystemVerilog + VHDL Dual Output from .ebv

This document explores adding VHDL as a second transpile target for Embedded Briev, alongside SystemVerilog. VHDL offers advantages in formal verification and certain hardware design workflows.
...
```briev
// config.ebv - Briev embedded code
led_on: Bool = false
...
*Both targets share the same Briev contract verification, ensuring correctness before transpilation.*