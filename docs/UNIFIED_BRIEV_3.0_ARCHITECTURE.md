# Unified Briev 3.0 Architecture: Silicon-Software Convergence

## 1. Vision
Briev 3.0 unifies the **Logic Engine** and **Grammar** across Core Briev (.bv), Rendered Briev (.rbv), and Embedded Briev (.ebv). This allows for **Silicon-Software Co-Design**, where the same logic can be validated in a browser (WASM) and synthesized into physical hardware (SystemVerilog/MCU).
...
- **Rendered Briev (.rbv):** The exclusive container for UI, HTML (`<view>`), and CSS (`<style>`).
- **Embedded Briev (.ebv):** The exclusive container for Silicon Pin Mapping and Memory-Mapped I/O (MMIO) address definitions.
- **Core Briev (.bv):** The shared substrate for platform-agnostic math, state machines, and business logic.

The "Unification" refers to the fact that every Briev script, whether inside an `.rbv` UI or an `.ebv` hardware file, now understands Hardware Geometry and Vector Lifting.

## 3. Core Concepts

### 3.1. Hardware Geometry (Vectors)
The `[]` syntax in Briev 3.0 represents **Physical Geometry**, not just data structures.
