# Meta-Circular Tamer: Brief Compiling Through Itself to Native
...
and the tamer pipeline proves Brief's general-purpose fitness.

## Overview

The meta-circular tamer is a stress test: a Brief program (the tamer VM interpreter)
compiled to `.lair` bytecode by the Rust `briefc`, loaded into the C tamer VM, that
then interprets a user's Brief program and emits LLVM IR to produce a native binary.

It answers three questions:

1. **Can Brief function as a systems language?** The tamer does pointer arithmetic,
   struct field access, bytecode parsing, bitwise operations, and FFI calls — all in
   Brief, running inside a VM, inside a C host.

2. **Can the compilation pipeline survive a round-trip through itself?** The user's
   program goes typed AST → beastpack → Brief deserializer → LLVM IR emission → clang.
   Every step but clang is in Brief. If this works, the path to full self-hosting is
   clear.

3. **Are our abstractions general enough?** If the tamer works, then any program that
   can be expressed in Brief can be compiled to a native binary through the same
...
│       │                                                         │
│       ▼ Brief tamer (running inside C VM)                       │
│  ┌─────────────────────────────────────────────────────────┐    │
...
The VM backend (`src/backend/vm/emit_expr.rs`) compiles Brief source to `.lair`
bytecode. The C interpreter (`tamer/interp.c`) executes `.lair` bytecode. The
Brief tamer (`lib/tamer/vm.bv`) interprets `.lair` bytecode **inside** the C
interpreter — a VM inside a VM.

The user's program, when executed inside the Brief tamer's inner VM, must emit
LLVM IR text via HCALL to the C host. This means the user's program's bytecode
contains instructions that the Brief tamer understands, and when it encounters
...
Brief tamer's codegen pass reads the beastpack (serialized AST) and walks the
...
But for an MVP, we don't need the full beastpack → LLVM IR pipeline in Brief.
We can take a simpler approach: **the user's `.lair` bytecode, when executed by
the Brief tamer's inner VM, performs the codegen via HCALL**. Each opcode in
...
## Phase 1: Brief-Side Tamer Completeness (7 days)

The Brief tamer (`lib/tamer/`) must handle all opcodes that the VM backend
actually emits. Current gaps:

### 1a. Add missing opcodes to vm.bv (2 days)

The VM backend emits opcodes that the Brief tamer's `vm.bv` doesn't handle:
...
in the Brief VM interpreter's `exec_op` match.

### 1b. Wire HCALL dispatch through to C host functions (2 days)

The Brief tamer's `exec_op` handles `OP_HCALL`:
...
This is a no-op — it skips the HCALL without calling the host. The Brief tamer
executes inside the C VM, so an HCALL from the Brief tamer must trigger an HCALL
...
**Solution**: When the Brief tamer's inner VM encounters an HCALL, it must:
...
The Brief tamer's `exec_op` for HCALL:
```brief
0x71 => { // hcall
    let host_id = read_u32(bc, pc + 1);
...
and sizing struct arrays at the Brief level. (Brief doesn't support runtime-sized
...
## Phase 2: Beastpack Deserialization in Brief (10 days)
...
The beastpack data is gzip-compressed. The Brief tamer needs to decompress it
...
**Option B**: Implement gzip decompression in Brief. This is ~200 lines of Brief
...
### 2c. BEAST text parser in Brief (5 days)
...
nothing about Brief's AST shape. It consumes text and produces `BeastExpr` trees.
This goes in `lib/std/sexpr.bv`. Only the `BeastExpr`-to-Brief-AST mapping is
...
## Phase 3: LLVM IR Emission in Brief (14 days)
...
### 3b. LLVM IR string builder in Brief (3 days)
            ...
            ChainSegment::Ref(name) => {
                ...
                match resolved {
                    ...
                    ChainSegment::Definition(d) => {
                        term CodegenPath::Brief(d);
                    },
                }
            },
        };
    };
    // No match — error (should not happen with a Brief fallback)
                ...
                when tag == Keyword("definition") {
                    ...
                    if len(chain) > 0 {
                        ...
                        match variant {
                            ...
                            CodegenPath::Brief(d) => emit_brief_fn(d),
                        };
                    } else {
                        // Single definition: emit directly
                        emit_brief_definition(item);
                    };
                };
            },
        };
...
Currently these are either FFI-only or missing. Port to native Brief or add
as `#` intrinsics.
...
### 5a. Unit tests for the Brief tamer (2 days)
...
The existing `test_brief_tamer.c` loads `combined.lair` and calls `tame()` at
...
|------|-----------|--------|------------|
| Brief string operations too slow for BEAST parsing | Medium | High | Optimize hot paths; pre-allocate string builders; batch HCALLs |
...
| Gzip decompression in Brief too complex | Medium | Medium | Skip compression for MVP; use SysCall# for v1 |
...
| Byte order or alignment mismatch between Rust VM backend and Brief tamer | Medium | High | Auto-generate opcode constants from a shared source; add assertion tests |
| The Brief language itself has a bug that blocks compilation | Medium | Critical | Test tamer compiles with current `briefc` first; fix bugs as found |
| String building in Brief causes memory explosion | Medium | Medium | Use streaming emission via HCALL rather than building giant strings |
...
|-------|--------|-----------------|-------------|
| Phase 0: Plumbing | 5 days | None | `briefc bounty` produces correct `.bounty`; C tamer calls `tame()` by name |
...
| Phase 2: Beastpack in Brief | 13 days | `binary.bv` (extend), `sexpr.bv`, `hash/blake3.bv` | Beastpack header parsed; BEAST text parsed; AsmFn+chain round-trip in BEAST |
...
**~25 days** to get `briefc bounty add.bv && tamer add.bounty` producing a
...
- `popcount_chain.bv` compiles to native code with `popcnt` on x86, `cnt` on ARM, Brief fallback elsewhere
...
- ✓ **No type name matching** — all Brief type queries go through protocol membership, not name strings
