# GLUE in Briev — Fully Native Bridge Generation
...
Rewrite the entire GLUE bridge generator pipeline in Briev — as `$defn`
compile-time functions and a `$(Normalized)` stage block — so the bridge
is generated, compiled, and optimized entirely by Briev's compiler.

The hypothesis: **a Briev-native bridge can beat C's linker** for
...
Briev .bv plugin (current state):
  .bv → Tag$ exports → StrReplace$ templates → FileWrite$ → Rust .rs files
         ↑                                    ↑
    $defn render_fn                    FileWrite$ with persist
```

The `.bv` plugin already does template rendering and file writing in Briev.
The missing pieces: generating Briev bridge code instead of Rust wrappers,
and orchestrating the full compile-link pipeline from within macros.

## Proposed Architecture

```
Briev-native pipeline (briev build with inline stage block):
  .bv → [Normalized stage block runs:]
         1. Tag$ exports → extract bridge info (already done in $defn)
         2. $defn generate_bridge_bv(): emit Briev bridge code
...
Just Briev source files that the compiler optimizes as one unit.

## Key Insight: Emit Briev, Not Rust

**Current approach** — emit Rust wrapper source that calls into Briev `.so` via FFI:
```
Briev .so (compiled) ← FFI → Rust wrapper ← FFI → Python/Caller
```
Two FFI boundaries. Caller → ctypes → Rust → ctypes → Briev. Each boundary
has overhead: argument marshaling, ABI conversion, stack frame setup.

**Proposed approach** — emit Briev bridge source, compile as one unit:
```
Caller ← FFI → Briev binary (bridge code inlined by LTO)
```
One FFI boundary. The bridge is just a Briev module that the compiler
...
produces a Python-callable C extension module written in Briev syntax:
...
This generates Briev source files that:
- Declare Python FFI functions as `frgn`
- Define `bridge_<fn>` wrappers that marshal between Python ABI and Briev
...
```briev
$(Normalized @ highest) {
    // 1. Generate bridge source code in Briev
...
compiled together with the rest of the project next time `briev build` runs.
...
| Metric | Current (Rust wrappers) | Proposed (Briev bridge) | Target |
|--------|------------------------|------------------------|--------|
| Bridge call overhead (Python → fn) | ~1.2µs (ctypes → .so) | ~0.8µs (LTO inlined) | Match C's ~0.8µs |
| Compile time | ~2s (llc + rustc) | ~0.5s (single brievc pass) | Beat C |
| Lines of Rust | ~400 (export.rs, config.rs) | 0 | Zero |
| Lines of Briev | ~60 (generator.bv) | ~120 (all helpers) | Manageable |

## Risk: What if LTO Can't Inline?

Even without cross-boundary inlining, the call overhead for a Briev-to-Briev
...
the remaining overhead is just ctypes. Moving to a direct Briev binary
eliminates ctypes entirely — the FFI boundary is just the Python C API,
which Briev already handles via `frgn`.
