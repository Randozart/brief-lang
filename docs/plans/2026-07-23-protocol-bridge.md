# Protocol Bridge + Briev-Native Wrappers
...
1. Move Python C extension wrappers from C to Briev (`export defn` + `frgn`)
2. Add protocol bridge `$defn` for languages without C FFI (WASM, sandbox)
3. Config-driven dispatch on `bridge_kind`

## Step 1: Wrappers in Briev, C only for struct literals
...
`PyInit_bridge` — ~10 lines. The wrappers are native Briev functions.
...
| Transport | Briev side | Target side |
|-----------|-----------|-------------|
...
- `c_wrapper` → emit Briev wrappers + minimal C struct file
