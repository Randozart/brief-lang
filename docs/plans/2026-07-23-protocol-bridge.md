# Protocol Bridge + Briv-Native Wrappers
...
1. Move Python C extension wrappers from C to Briv (`export defn` + `frgn`)
2. Add protocol bridge `$defn` for languages without C FFI (WASM, sandbox)
3. Config-driven dispatch on `bridge_kind`

## Step 1: Wrappers in Briv, C only for struct literals
...
`PyInit_bridge` — ~10 lines. The wrappers are native Briv functions.
...
| Transport | Briv side | Target side |
|-----------|-----------|-------------|
...
- `c_wrapper` → emit Briv wrappers + minimal C struct file
