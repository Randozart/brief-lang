# Plan: Data Briev as Universal Config + Board-Owned Hardware Map
...
**Branch:** `feat/data-briev-config` (new)

**This is the AUTHORITATIVE record** for two coupled streams that share one
mechanism: (1) making **Data Briev** (`.dbv`/`.dbvl`, via `src/dbriev/v2.rs`)
...
| Plan | Role | Status |
|---|---|---|
| **[This plan](2026-08-03-data-briev-config-and-board-hardware-map.md)** | The authoritative record — the DB-as-config migration, the board-owned hardware map, the shared address contract, and the `.ebv` follow-up handoff. All design research lives here. | Active |
| [2026-07-30-meta-circular-tamer.md](2026-07-30-meta-circular-tamer.md) | The freestanding `.ebv` stress test (VM, pointers, bytecode in Briev). Depends on `.ebv` having a real no-OS runtime; this plan provides the board/address foundation it builds on. | Design → foundation |
...
### 2. Why Data Briev, now

`config/*.toml` is the *last* configuration in the system not expressed in a
Briev-native form. Data Briev (`.dbv`/`.dbvl`) is already shipped — `src/dbriev/
v2.rs` parses it (`;` separator, `>` directives, bare tokens default), and
`src/dbriev/bridge.rs::document_to_program` converts it to the Briev AST. It is
...
Notably, quotes (`"..."`) **are** legal Data Briev — they are opt-in via the
...
Data Briev family is two formats. They are complementary, not interchangeable:
...
Briev formatting + `write`, `Now#`→ freestanding clock) is **out of scope for
...
> **obsolete pre-v2 syntax** and does not parse under `src/dbriev/v2.rs`. Three
...
> **2026-08-03 (parser verification, empirical):** probing `src/dbriev/v2.rs`
...
- Add a small Rust loader (in `src/dbriev/` or a new `src/config_db.rs`) that
...
in `src/dbriev/v2.rs` (tests module) that feeds the *intended* v2 flat
`map.dbv` (schemas only) + `addresses.dbvl` + `registers.dbvl` content through
`parse_document` and asserts a well-formed `DbrievDocument`: the `Device`/
...
>    the NAME. Matches `docs/architecture/data-briev.md` semantics.
...
- ✅ Update `docs/architecture/data-briev.md`: note it is now the universal config
