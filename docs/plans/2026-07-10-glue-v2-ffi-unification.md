...
**Driver:** End-to-end bidirectional FFI (Brief → foreign, foreign → Brief) with zero-copy data, no C compiler dependency, and cross-language LLVM LTO for LLVM-based targets.

---

## 1. Philosophy

The GLUE protocol (General Language Unification Engine) is Brief's strategy for
piecemeal language adoption ("strangler fig" pattern). A developer replaces one
function at a time with Brief, and the boundary is invisible to the host language.

Two integration paths exist, selected automatically by the compiler based on
target capability:

**Path A — LLVM Inlining (for Rust, C, C++, Swift, Zig):**
Brief emits LLVM IR with proper function definitions. GLUE merges Brief's IR
...
native symbols. Brief's `#export` functions ARE native functions.

**Path B — Meld Projection (for Python, Node, JVM):**
Brief compiles to a shared library with C ABI exports. Data does NOT cross a
marshaling layer — meld declarations project Brief types onto the host
runtime's heap memory (PyObject*, v8::Value, jobject). Brief reads/writes
...
| DBVL parser | `src/glue/dbvl_reader.rs` | Brief's native data-interchange format |
...
| `brief link` subcommand | `src/main.rs`, `src/glue/link.rs` | nm-based symbol analysis was a convenience, not core to the vision |
...
entry GlueTarget {
    ...
    types_module: String;   // Path to .bv declaring foreign type names for Brief's universe
...
- **language**: Lookup key for `brief export bridge.bv <language>`
- **types_module**: Brief source file declaring foreign type names. A bridge
  file imports this module and uses `meld` to connect Brief types to foreign types.
...
- **c_type_map**: Maps each Brief type name to its C ABI representation. This is
...
Meld proves layout compatibility between two types in Brief's universe. The C ABI
...
Brief's type universe. These are minimal — just enough for `meld` to reference
...
`brief export bridge.bv rust --out ./out` produces:
...
```
// bridge-exports.dbvl — auto-generated by brief export
...
// Melds: meld, brief_type, foreign_type, routes_semicolon_separated
meld, Int, i64, Ptr->Ptr;Size->8;Bytes->8
meld, Float, f64, Ptr->Ptr;Size->8;Bytes->8

// C type mappings: ctype, brief_type, c_type
...
**Content:** Minimal type declarations (`type i64; type f64; type cstring; type bool; type char;`) for each target. These register foreign type names in Brief's type universe so `meld` can reference them.
...
Lines written:
  ...
  ctype, <brief_type>, <c_type>                             // for each entry in c_type_map
```
...
// approach generated Cargo.toml/build.rs/src/ that wrapped Brief
...
// c_type_map now maps Brief types to C ABI types, not adapter-
...
### New tests
- `test_serialize_ctype_dbvl` — ctype lines emitted as `ctype,<brief>,<c>`
