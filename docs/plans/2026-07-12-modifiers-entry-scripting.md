...
| **Stdlib `cli.c.bv`** | Extensible CLI framework in pure Briev | 16F |
...
fn analyze_file_pipeline(path: &Path) -> Result<CompilationJob, CompilerError> {
    ...
    match parts.len() {
        2 => {
            let variant = BrievVariant::from_str(parts[1])?;
            Ok(CompilationJob {
        ...
        3 => {
            let flags = parts[1];
            let variant = BrievVariant::from_str(parts[2])?;
...
2. **Call graph isolation:** No internal Briev code can call `build()`.
...
clear error: `error: mixed indentation (tabs and spaces) in formatted Briev
...
Allow writing linear Briev code at the top level without explicitly wrapping
