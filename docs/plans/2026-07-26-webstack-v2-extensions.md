...
**Depends on:** `docs/plans/2026-07-26-rendered-briev-webstack-v2.md` (Phases 1-7 complete)
...
These are boilerplate. The GLUE `[web]` target generates type-correct stubs, but the Briev-side declarations should live in stdlib so users can `import "web/console.bv"` and get them.
...
Each file is 3-5 lines of Briev:
...
This is architecturally clean but adds a runtime dependency (`wasmtime` or the Briev interpreter). The compile-time approach is simpler and zero-cost — if the initial state is fully determined at compile time (no `GetEnvInt#` or `frgn from #Web` in initializers), the SSR is a pure function of the source.
...
```bash
brievc build app.rbv --backend webstack --ssr
```
...
| `docs/architecture/features/rendered-briev-wasm.md` | Update spec with SSR section |
...
- A dev server — a small file watcher + HTTP server (can be a Briev tool or a thin shell)
...
| `tools/brievc-dev/` | New dev server (file watcher + HTTP + WebSocket) |
...
| Document | Update |
|----------|--------|
| `docs/architecture/features/rendered-briev-wasm.md` | Add SSR section under "Outputs", add HMR section under "Future" |
