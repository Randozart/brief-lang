...
Briev: 8× v2f32 vector phis, 133 shuffle/insert/extract ops
...
- `field_briev_types` Vec (reorder by new index)
...
| Path | Why Rejected |
|------|-------------|
| FMA codegen | Both Briev and C produce zero FMA on Ivybridge (no -fma flag). Not the gap. |
...
| Sqrt vectorization | Both cover all 10 gravity pairs. Briev uses 2 scalar vs C's fully packed, but these add <2% to runtime. |
