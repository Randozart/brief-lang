...
This plan eliminates all C dependencies from the Briev compiler pipeline and
replaces `Alloc#` with `struct`-embedded static arrays proven by contract. The
tamer (install-time compiler) becomes a pure Briev program compiled by Briev's
own LLVM backend, using `SysCall#` with inline assembly for OS interaction and
`struct` with `Int[N]` fields for all buffers. No `malloc`, no `briev_rt.c`,
no `tamer/*.c` — zero C.
...
| 2 | `SysCall#` Inline Asm + Runtime Port | Phase 1.6 | No `briev_rt.c` — all runtime in Briev using syscall asm |
...
| 3 | Tamer in Briev | Phase 2.5 | No `tamer/*.c` — pure Briev tamer with struct arrays, no `Alloc#` |
...
**Also fix the Briev VM** (`lib/tamer/vm.bv:257-287` and `lib/tamer/combined.bv`):
...
Briev VM's `OP_CALL` must store `pc + 3` as the return PC in the frame record,
...
Update `tamer/interp.h` and the Briev VM opcode constants.
...
```bash
brievc build --backend llvm test.bv -o test.ll
...
if let FromSpec::Protocol(proto) = &fb.from {
    ...
    let lib = protocol_config.resolve(default_triple, proto).map_err(|e| {
        format!("frgn '{}': {}", fb.effective_briev_name(), e)
    })?;
...
### 1f. Compiler Registry — `brievc registry`
...
**Solution:** A per-user registry directory at `~/.briev/registry/` populated
by `brievc registry add`.
...
Project-local `.briev/registry/` overrides the user-wide one (same merge
semantics as project-local config).

#### CLI commands

```
brievc registry add <path> [--name <name>]
  # Copy file/dir to ~/.briev/registry/<name>, version-locked (no symlink).
  # --name defaults to the file stem: ./my-queue.bv → "my-queue".

brievc registry list
  # Print every entry in ~/.briev/registry/ with type (file/dir) and size.

brievc registry remove <name>
  # rm -rf ~/.briev/registry/<name>* matching.
```
...
by project-local `.briev/registry/` and `--registry-dir` CLI flag.
...
| Platform path | `dirs::data_dir()` | Cross-platform: Linux `~/.local/share/briev/`, macOS `~/Library/Application Support/briev/`, Windows `%APPDATA%/briev/` |
| Project override | `.briev/registry/` | Same pattern as `.briev/config/` for target config |
...
1. `brievc registry add ./test.bv` → file appears in `~/.briev/registry/test.bv`
2. `brievc registry list` → output contains `test.bv`
3. `brievc registry remove test` → file removed
4. `import <test>` resolves to `~/.briev/registry/test.bv` when it exists
5. `from <test.c>` resolves to `~/.briev/registry/test.c` when it exists
...
- Update `learn-briev/` — update tutorial files
...
| `learn-briev/05-data-types.md` | `:>` → `.#` in examples |
| `learn-briev/13-projections.md` | Rewrite for `.#` |
...
| `learn-briev/05-data-types.md` | 1.6 | `:>` → `.#` in examples |
| `learn-briev/13-projections.md` | 1.6 | Rewrite projections section for `.#` syntax |
...
Replace the current `call @briev_syscall(i64 %num, ...)` with target-conditional
inline assembly:

**x86_64 (Linux):**
```rust
// 2026-07-26: Emit inline syscall asm instead of calling briev_syscall().
...
Before this change, every `SysCall#` called `briev_syscall()` in `briev_rt.c`,
...
### 2b. Port `briev_rt.c` Functions to Briev

Each C function becomes a Briev `defn` using `SysCall#`. Organized into protocol
modules under `lib/std/posix/`. Each function is ported one at a time, verified,
then the C original is removed from `briev_rt.c`.

#### Migration Order

| Step | C Function | Briev Equivalent | Depends On |
|------|-----------|------------------|------------|
...
| 4 | `briev_str_to_c` | **Delete** — no C strings needed without C runtime | — |
| 5 | `briev_cstr_to_briev` | **Delete** — same reasoning | — |
| 6 | `briev_free_briev_str` | **Delete** — same reasoning | — |
...
| 10 | `briev_syscall` | **Delete** — replaced by inline asm | Phase 2a |
| 11 | `briev_sysconf` | `posix/syscall.bv`: `syscall(SYS_sysconf, name)` | Phase 2a |
| 12 | `ShellCmd` | `posix/process.bv`: `clone` + `execve` + `pipe` | Steps 1, 7 |
| 13 | `__getenv_briev` | `posix/env.bv`: read `/proc/self/environ` | Steps 7-8 |
...
// ── Print ─────────────────────────────────────────────────────────────────
// These replace __print_char, __print_int, __print from briev_rt.c.
...
export defn print_int(n: Int) -> Int {
    ...
    // This is a simplified version — full formatting is handled by Briev code.
...
// ── File I/O ──────────────────────────────────────────────────────────────
// These replace __read_file__ and __write_file__ from briev_rt.c.
...
// Replaces ShellCmd from briev_rt.c (which used popen).
...
`briev_rt.c` were from the pre-intrinsic era. They would cause linker errors
...
1. `lib/runtime/briev_rt.c` — does the C function exist?
...
had no corresponding C implementation in `briev_rt.c`. They were a historical
...
Phase 2b ports them to Briev.

### After Each Migration Step

For each C function ported to Briev in Step 2b:

1. Write the Briev implementation in `lib/std/posix/*.bv`
2. Compile a minimal test program that uses the new Briev function
3. Verify the output matches the old C function's behavior
4. Remove the C implementation from `lib/runtime/briev_rt.c`
...
## Phase 3 — Tamer in Pure Briev (No `Alloc#`, No C)
...
### 3b. Write Tamer VM in Briev with Struct Arrays

Replace `lib/tamer/main.bv`, `lib/tamer/vm.bv`, `lib/tamer/loader.bv` with
struct-based versions. The existing Briev VM (`vm.bv:314` lines) already has a
...
### 3c. LLVM IR Emission from Briev
...
**Future (pure Briev):** Walk the `.beastpack` binary format in Briev and emit
...
The Briev code would iterate items, match on type, and produce LLVM IR text.

**For this plan:** Implement MVP via HCALL in Phase 3, defer pure-Briev
beastpack walking to a follow-up.

### 3d. Clang Invocation from Briev
...
### 4a. Bytecode Static Analysis in Briev
...
// minimum sufficient buffer sizes. Implemented in Briev, runs
                               ...
                               bc_data: Ptr<Int>, bc_len: Int) -> Int
{
    ...
    // This is a dataflow analysis in Briev itself.
...
Future Briev feature: dimension from expression:
...
`alloca` with a dynamic operand. The Briev type system would need to allow
...
the Briev AST at compile time. Extending it to also analyze `.lair` bytecode
at runtime (in the Briev tamer) is a different analysis — it would be written
in Briev and operate on the VM bytecode format, not the Rust AST. The two
...
- **Runtime (Briev):** Analyzes the `.lair` bytecode being interpreted to
...
| 3d | clang invocation via Briev clone+execve | Integration | `.ll` → binary |
...
be updated to use the new Briev equivalents. Run `--correctness` only for
...
| `lib/tamer/vm.bv` | 0a, 3b | Fix CALL/RET in Briev VM, rewrite with struct arrays |
...
| `lib/runtime/briev_rt.c` | 2b | Gradually delete functions as ported |
...
| `lib/tamer/vm.bv` (comment) | 0a | Add rationale comment at OP_CALL/OP_RET for the Briev VM fix. |
...
| `learn-briev/` | 1.5 | Update tutorial files to use `when` instead of `[cond]{body}`. |
...
| `docs/architecture/bounty-architecture.md` | 3b | Document the pure-Briev tamer's architecture. |
...
| Porting a C function to Briev introduces a subtle bug | 2b | Medium | Medium | Compare output of C and Briev versions side-by-side |
...
| Briev VM struct-array version is slower than pointer version | 3b | Low | Medium | Contracts prove bounds; LLVM optimizes proven-safe GEP chains |
