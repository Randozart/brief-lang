...
fn coerce_to_param_type(
    ...
    // Determine Brief types for semantic conversion.
...
1. **No allocation benchmark**: Create a minimal `.bv` file with a `node` that has no `Alloc#` calls. Compile with `briefc`. Verify the generated `.ll` has no `call ptr @malloc(i64` in the main loop or txn functions.
