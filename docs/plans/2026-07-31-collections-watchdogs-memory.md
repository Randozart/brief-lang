...
Briev countdown loop = **40 instructions** vs C = **34**; the 6 extra are
...
| `?[N ms]` | **time** — a wall-clock deadline | `Now#` (new `clock_gettime(CLOCK_MONOTONIC)` intrinsic in `briev_rt.c`); elapsed-vs-deadline compare |
...
- **A/B experiment (interleaved x3, BOUND=50M, countdown-dispatched,
  non-folded)**: sweep_dense (16 scalars) Briev 0.41s vs C 0.26s = 1.57x;
  sweep_arr (Float[16]) Briev 0.40s vs C 0.35s = 1.17x. **Conclusion (rule
  #19): the hypothesis that the array form makes Briev faster is REFUTED** —
  Briev's throughput is unchanged (0.40s both); the gap closure is C's array
  reference being slower. No cyclic-shift pass is shipped (no Briev-side win).
...
- **C5** — `series_converge`: `?[(x-last)^2 > eps] -> print_best(x)` fires on
  convergence. Briev and C both print 0.500050008 (MATCH, 1.00x). Registered
  in the harness.
...
- **Garbage scheduler** (the global-lifetime inference, `analysis/global_lifetime.rs`):
  ...
  `Free#` (via `__briev_free`) exactly after that txn's body (countdown exit
  ...
  free in the file) — matches C. `__briev_free_count` calibrates frees ==
...
grammar, `learn-briev` collections + watchdog chapters, `docs/architecture`).
...
| `Now#` time source (C) | `clock_gettime(CLOCK_MONOTONIC)` in `briev_rt.c`; deterministic ticks remain the default |
