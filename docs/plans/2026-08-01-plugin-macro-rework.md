# Briev: Plugin / Macro Syntax Rework + The Bits Model (String as `#String` Protocol)
...
**Worktree:** `../briev-compiler-plugin-rework` (new, from `main` `d6c6c818`)
**Baseline worktree:** `../briev-compiler-baseline` — synced to `d6c6c818` on 2026-08-01
...
Rework the plugin / macro surface of Briev to match the language's disclosure
...
| `defn main` | Emitted as `briev_main` (`emit_toplevel.rs:1133`) but **never called** by the runtime — reactor only fires reactive `txn`/`node` | `src/backend/llvm/emit_toplevel.rs:1133` |
...
| Env vars | briev_rt.c provides `__getenv_briev` / `__getenv_int`; `frgn` wrappers in `lib/std/ffi/env.bv` | `lib/runtime/briev_rt.c:127-157`, `lib/std/ffi/env.bv` |
...
| Print codegen | `PrintInt#` → `call i64 @__print_int` (briev_rt.c, `always_inline` + LTO) | `src/backend/llvm/intrinsics.rs:65-90`, `lib/runtime/briev_rt.c:178` |
...
```briev
frgn __argv_count() -> Int        from "lib/runtime/briev_rt.c" fallback 0;
frgn __argv_get(i: Int) -> String from "lib/runtime/briev_rt.c" fallback "";
frgn __argv_has(flag: String) -> Bool   from "lib/runtime/briev_rt.c" fallback false;
frgn __argv_value(flag: String) -> String from "lib/runtime/briev_rt.c" fallback "";
...
**Env-var fallback:** `entry_cmd()` also honors `$BRIEV_ENTRY_CMD` if set (test /
...
- At the top of `main`, store into module globals:
  `@__briev_argc = internal global i32 0`, `@__briev_argv = internal global ptr null`,
  via `store i32 %argc, ptr @__briev_argc` / `store ptr %argv, ptr @__briev_argv`.
- briev_rt.c gains: `__argv_count`, `__argv_get`, `__argv_has`, `__argv_value`,
  `__argv_command` (reading the globals; `extern int64_t __briev_argc; extern void* __briev_argv;`).
...
- `defn main` wiring: if a `defn main()` exists (no explicit `entry!`), the
  plugin synthesizes the same one-shot node calling `briev_main()` once, fixing
...
2. **A String value is a pointer** to `[len: i64][bytes]`. The value register
   ...
   `Ptr<T>` pattern). Matches the runtime contract: `briev_str_to_c(int64_t)`
   (`lib/runtime/briev_rt.c:48-102`), `__print(int64_t)`, `__print_str(int64_t)`
...
4. **Casts change interpretation, never memory.** **`#String → #Bit`** = the
   ...
   **The header is never part of the bits.** In both directions the cast moves
   ...
   as `briev_cstr_to_briev` does). If the source already was a `#String`, the
...
- **Format-demo clang error** (the Part A Phase 1 blocker): `brievc build`
...
- **B-D1 (representation):** A String value is a `ptr` to `[len][bytes]` in
  ...
  a *runtime* encoding detail inside `briev_str_to_c` — never a compiler type.
...
2. A/B against `../briev-compiler-baseline` (now at `d6c6c818` = current state,
...
| `learn-briev/*`, `spec/SPEC.md` | Tutorial + spec sweep |
...
| `src/backend/llvm/emit_expr.rs:2437-2454` | `Eq`/`Ne` on `#String` operands → content compare (len + bytes; runtime `briev_str_eq` or inline) |
...
the backend (`emit_expr.rs` Eq/Ne arms → `briev_str_eq` runtime call, declared
in `mod.rs`). Bitwise `&`/`|`/`^`/`~` on `#String` operate on content bytes via
`briev_str_band/bor/bxor/bnot` (same-length result), gated by the central
...
corrupting pointers for `briev_str_eq` — now gated on `feature_sso_strings`;
...
| `lib/runtime/briev_rt.c` | `__argv_count/__argv_get/__argv_has/__argv_value/__argv_command` |
...
`@__briev_argc`/`@__briev_argv` (external globals the runtime links against).
`lib/runtime/briev_rt.c` gains `__argv_count/__argv_get/__argv_has/__argv_value/
__argv_command` (command = first non-flag argv[1..], honoring `$BRIEV_ENTRY_CMD`).
...
| File | Change |
|------|--------|
| `src/plugin/script_plugin.rs` (new) | §4.5 synthesis; `defn main` wiring to `briev_main` |
...
`main()` once (fixes the dead-code gap — `briev_main` was defined but never
...
now maps a call to `main` → `briev_main` (the defn rename), and the
...
`briev_bits_to_str`, which re-materializes the `[len][bytes]` header by
...
| runtime | `briev_len` / `briev_char_len` helpers for the `#String` defaults (via `briev_str_to_c`) |
...
default = UTF8 char count via `briev_char_len` (runtime counts codepoints);
...
| `learn-briev/` | Tutorials: scripting, CLI, printing |
...
`learn-briev/12-pragmas.md` + `15-custom-types.md` (the `<~` → `!>` syntax).
...
11. Phase 5 + **Phase B4**: docs/highlighter/learn-briev, legacy retirement,
    final benchmark A/B.
