...
`gen_rust.bv` generates `extern "C"` declarations for Briv exports. The Tier 1
...
**Worker — Pure Briv with `frgn` for POSIX calls.**

The shim is a Briv program using `frgn` for `shm_open`/`mmap`/`ftruncate`
and a `node` for the event loop. No C compiler needed — just `briv build --shared`.
...
const shm_fd = frgn_shm_open("/briv_bridge", O_RDWR | O_CREAT, 0o666);
...
2. Spawns the worker process (the compiled Briv binary)
...
**Files changed:**
- `lib/ffi/gen_shm.bv` — new generator (produces Briv shim + client wrappers)
