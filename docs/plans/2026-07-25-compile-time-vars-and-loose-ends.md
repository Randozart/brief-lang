...
`gen_rust.bv` generates `extern "C"` declarations for Brief exports. The Tier 1
...
**Worker — Pure Brief with `frgn` for POSIX calls.**

The shim is a Brief program using `frgn` for `shm_open`/`mmap`/`ftruncate`
and a `node` for the event loop. No C compiler needed — just `brief build --shared`.
...
const shm_fd = frgn_shm_open("/brief_bridge", O_RDWR | O_CREAT, 0o666);
...
2. Spawns the worker process (the compiled Brief binary)
...
**Files changed:**
- `lib/ffi/gen_shm.bv` — new generator (produces Brief shim + client wrappers)
