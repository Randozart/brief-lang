...
`type` is Brief's metaprogramming keyword. It defines how bits are
...
completely that `let fourteen: RomanNumeral = XIV` is a valid Brief
...
The codec file is itself Brief:
...
codec files — no compiler magic. Brief becomes the universal bridge not
