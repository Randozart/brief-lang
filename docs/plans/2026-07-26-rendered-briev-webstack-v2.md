# Plan: Rendered Briev Webstack v2 — WASM-First Architecture

**Date:** 2026-07-26
**Status:** Implementation plan
**Spec:** `docs/architecture/features/rendered-briev-wasm.md`
...
All work happens in a **dedicated git worktree** at `../briev-compiler-wasm-webstack/` to avoid interfering with other agents working on the core compiler.

### Worktree Setup

```bash
cd /home/randozart/Desktop/Projects/briev-compiler
git worktree add -b wasm-webstack-v2 ../briev-compiler-wasm-webstack main
cd ../briev-compiler-wasm-webstack
...
3. **Add `glue/web/types.bv`:**
   ...
   // Maps Briev types to web runtime representations
...
2. **`generate()` emits `dom-shim.mjs` with:**
   ...
   ```javascript
   // Auto-generated by Briev/GLUE Web Generator
...
| Briev type | wasm_abi | marshal in | marshal out |
|------------|----------|------------|-------------|
...
1. For each `ForeignBinding` with `FromSpec::Protocol("#Web")`:
   a. Briev name → JS function name (the `as` clause or foreign symbol)
...
| Document | Update |
|----------|--------|
| `docs/architecture/features/rendered-briev-wasm.md` | (this spec) — definitive reference |
