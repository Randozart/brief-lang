...
| `lib/runtime/briev_rt.c` | C | `briev_str_to_c` — handle SSO + new heap layout |
...
`push_field_type` reads `fields.len()` — for String it's 2, so pushes 2 × `i64`. The existing `field_briev_types` records `Type::Custom("String")` for both slots. State access automatically widens to 2 slots.
...
| Test | What it asserts |
|------|----------------|
| `test_c_runtime_sso` | `briev_str_to_c` handles SSO handle |
| `test_c_runtime_heap` | `briev_str_to_c` handles heap handle (new no-header layout) |
