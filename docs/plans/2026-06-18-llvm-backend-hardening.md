...
**Governing Principle:** Briev shall not break under valid syntax. If the compiler accepts a program, the binary must be correct or the compiler must emit a compile-time error. A silently-crashing binary is a compiler bug — always.
...
### 3a. Convert `briev_rt.c` to Bitcode Pipeline

**Files:** `src/backend/llvm/mod.rs`, `lib/runtime/briev_rt.c`

**Problem:** Current pipeline compiles Briev → LLVM IR (`program.ll`), then assembles to `.s` with `llc`, then links `briev_rt.o` as a native `.o` file. This prevents cross-module inlining and constant propagation through the C shim boundary.

**Fix:** Add an `llvm-link` step in the backend pipeline:

1. Pre-compile `briev_rt.c` to bitcode once (build.rs or manual):
   ```bash
   clang -O3 -emit-llvm -c lib/runtime/briev_rt.c -o lib/runtime/briev_rt.bc
   ```

2. After generating `program.ll`, merge with runtime bitcode:
   ```bash
   llvm-link program.ll lib/runtime/briev_rt.bc -S -o linked.ll
   ```
...
**Implementation:** Add `link_bitcode` method to the LLVM backend that runs `llvm-link` and `opt` via `std::process::Command`, gated on existence of `briev_rt.bc` (fall back to current .o linking if not found).
...
3. Compare with emitted Briev IR
4. Adjust Briev IR to match if Clang's version is more correct/optimizable
...
3. If officina is fixed by other agent: `./target/release/briev-compiler llvm officina.bv -o officina` + run
...
clang -O3 -emit-llvm -c briev_rt.c -o briev_rt.bc
# Merge with generated Briev IR:
llvm-link program.ll briev_rt.bc -S -o final.ll
# The compiler now auto-detects briev_rt.c and runs this pipeline
