# GLUE in Briv — Fully Native Bridge Generation
...
Rewrite the entire GLUE bridge generator pipeline in Briv — as `$defn`
compile-time functions and a `$(Normalized)` stage block — so the bridge
is generated, compiled, and optimized entirely by Briv's compiler.

The hypothesis: **a Briv-native bridge can beat C's linker** for
...
Briv .bv plugin (current state):
  .bv → Tag$ exports → StrReplace$ templates → FileWrite$ → Rust .rs files
         ↑                                    ↑
    $defn render_fn                    FileWrite$ with persist
```

The `.bv` plugin already does template rendering and file writing in Briv.
The missing pieces: generating Briv bridge code instead of Rust wrappers,
and orchestrating the full compile-link pipeline from within macros.

## Proposed Architecture

```
Briv-native pipeline (briv build with inline stage block):
  .bv → [Normalized stage block runs:]
         1. Tag$ exports → extract bridge info (already done in $defn)
         2. $defn generate_bridge_bv(): emit Briv bridge code
...
Just Briv source files that the compiler optimizes as one unit.

## Key Insight: Emit Briv, Not Rust

**Current approach** — emit Rust wrapper source that calls into Briv `.so` via FFI:
```
Briv .so (compiled) ← FFI → Rust wrapper ← FFI → Python/Caller
```
Two FFI boundaries. Caller → ctypes → Rust → ctypes → Briv. Each boundary
has overhead: argument marshaling, ABI conversion, stack frame setup.

**Proposed approach** — emit Briv bridge source, compile as one unit:
```
Caller ← FFI → Briv binary (bridge code inlined by LTO)
```
One FFI boundary. The bridge is just a Briv module that the compiler
...
produces a Python-callable C extension module written in Briv syntax:
...
This generates Briv source files that:
- Declare Python FFI functions as `frgn`
- Define `bridge_<fn>` wrappers that marshal between Python ABI and Briv
...
```briv
$(Normalized @ highest) {
    // 1. Generate bridge source code in Briv
...
compiled together with the rest of the project next time `briv build` runs.
...
| Metric | Current (Rust wrappers) | Proposed (Briv bridge) | Target |
|--------|------------------------|------------------------|--------|
| Bridge call overhead (Python → fn) | ~1.2µs (ctypes → .so) | ~0.8µs (LTO inlined) | Match C's ~0.8µs |
| Compile time | ~2s (llc + rustc) | ~0.5s (single brivc pass) | Beat C |
| Lines of Rust | ~400 (export.rs, config.rs) | 0 | Zero |
| Lines of Briv | ~60 (generator.bv) | ~120 (all helpers) | Manageable |

## Risk: What if LTO Can't Inline?

Even without cross-boundary inlining, the call overhead for a Briv-to-Briv
...
the remaining overhead is just ctypes. Moving to a direct Briv binary
eliminates ctypes entirely — the FFI boundary is just the Python C API,
which Briv already handles via `frgn`.
