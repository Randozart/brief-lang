...
pub enum Fallback {
    ...
    Static(Expr),
    /// Call a Briev function with the frgn's parameters
...
#   bridge_kind     — how to call into this language from compiled Briev
#   calling_convention — ABI at the boundary
#   c_type_map      — Briev type → C ABI type for C ABI boundary
...
// Zero-copy melds: if Briev CBuffer has same layout as PyBytes
...
```briev
// 2026-07-22: Node.js type declarations for Briev's type universe.
...
```rust
// ── Frgn dispatch resolution ───────────────────────────────────────────
let glue_config = briev_compiler::glue::config::load_glue_config(
...
**Goal:** Add `briev export` and `briev link` subcommands, unify the two
export paths.
...
### 5.2 — Wire `briev export` CLI subcommand
...
fn run_export(args: &[String]) -> Result<(), String> {
    let file_path = args.first().ok_or("usage: briev export <bridge.bv> <language> --out <dir>")?;
    let language = args.get(1).ok_or("usage: briev export <bridge.bv> <language> --out <dir>")?;
...
fn run_link(args: &[String]) -> Result<(), String> {
    ...
    let result = briev_compiler::glue::link::analyze_library(Path::new(lib_path))?;
...
pub fn generate_python_wrapper(exports: &[ExportDecl], bridge_name: &str) -> String {
    ...
    out.push_str(&format!(
        ...
        def _init_state():
            ...
            return _lib.__briev_init_state()
...
| File | Tests |
|------|-------|
| `tests/frgn_export_e2e_tests.rs` | End-to-end: write a bridge .bv, run `briev export`, verify outputs |
...
- Update `build.rs` to work with `briev export` output
- Add a `glue.toml` override (optional)

**`examples/glue-python-bridge/`:**
- Update `gluerun.py` to work with `briev export` output
...
1. Use `briev-compiler export` instead of nonexistent `$BRIEV export`
...
**Goal:** Port the AST pretty-printer (`src/ast/display.rs`) from Rust to Briev,
then call it through the GLUE bridge. This exercises the full FFI pipeline in
reverse direction (Briev → Host), validates protocol transforms on recursive
...
no mutable references. Perfect Briev candidate.
...
### 8.2 — Briev implementation

**New file:** `lib/pp/types.bv` — pretty-print functions for types:

```briev
// lib/pp/types.bv
// 2026-07-22: AST pretty-printer ported to Briev (Phase 8).
...
```briev
// bridge/pp-bridge.bv
// 2026-07-22: Bridge for calling Briev pretty-printer from Rust compiler.
// Import the pretty-printer implementation
import "lib/pp/types.bv";
import "lib/pp/exprs.bv";

export defn briev_pp_type(type_tag: String, payload: String) -> String {
    // Dispatch on type_tag, call the appropriate pp function
    // ...
};
```

The Rust compiler calls `briev_pp_type` via the GLUE bridge:
```rust
// In src/ast/display.rs (modified):
fn display_type(ty: &Type) -> String {
    if let Ok(result) = glue::call("briev_pp_type", &[serialize_type(ty)]) {
        return result;
    }
...
2. Generate it using the Briev implementation via GLUE bridge
...
fn test_pp_ast_roundtrip() {
    ...
    let briev_pp = call_briev_pp(&ast);    // Briev via GLUE
    assert_eq!(rust_pp, briev_pp);
}
```
...
|-------------|-------------|------|
| 8a — `Type` | 4-5 Briev functions, simple strings | Low |
| 8b — `ForeignBinding` | Briev calls sub-printers | Low |
...
| 8f — Full cutover | Rust Display impl delegates entirely to Briev | High |

### 8.6 — What this validates about the GLUE pipeline

| Requirement | How Phase 8 exercises it |
|------------|-------------------------|
| Protocol path resolution | Briev `String` ↔ Rust `String` via `#String<UTF8>` |
...
| Export wrapper generation | `briev export pp-bridge.bv rust --out ...` |
| `calling_convention = "lto"` | Rust LTO path — no C ABI boundary overhead |
| Performance | Pretty-printing is not hot code; tail-call optimization under Briev contracts |

### 8.7 — When to consider Phase 8 done

- `Type::Display` delegates to Briev (verified by round-trip test)
- `ForeignBinding::Display` delegates to Briev
- `Statement::Display` delegates to Briev
- `Expr::Display` delegates to Briev (the big one — all 30+ variants)
- `Definition::Display` and `TopLevel::Display` delegate to Briev
...
### E2 — `as` with the same name as the Briev name
...
### E5 — `briev export` with no exports

Graceful: "No exports found in bridge file. Nothing to generate." Exit 0.

### E6 — `briev link` on a library with no T (text) symbols
