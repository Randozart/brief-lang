...
### 3.4 Hardware / Briev Variants

| # | File | Type | Features |
|---|------|------|----------|
| 15 | `examples/inop-side-effect.bv` | `.bv` | `inop!` (side-effectful intrinsic), BILD body with side-effects |
| 16 | `examples/data-briev` | `.dbv/.dbvs/.dbvl` | Data Briev schema, data files, validation |
...
- **New files (19):** `macro-demo.bv`, `proof-oracle.bv`, `foreach.bv`, `sync-domain.bv`, `arrow-mutation.bv`, `projections.bv`, `map-set.bv`, `swan-song.bv`, `error-handling.bv`, `stdlib-demo.bv`, `gpu-compute.abv`, `networking.bv`, `process-spawn.bv`, `mmap-demo.bv`, `inop-side-effect.bv`, `data-briev/*`, `glue-macro.bv`, `assume-pragma.bv`, `view-directives.rbv`
