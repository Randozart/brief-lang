...
| UTF8View, StaticString, SmallString64 decls + pure-Briev ops | (committed) | ✅ Implemented |
...
```bash
cargo build && \
  BOUND=5 ./target/release/briev-compiler build benchmarks/precompute_sum.bv --out benchmarks && \
...
- Runtime benchmarks: Briev is within 2× of C (acceptable for initial SSO/SVO rollout)
...
| How does Briev compare to C? | Ratio column |
...
- **Briev vs C ratios** should be similar to July 18 baseline for non-string benchmarks
