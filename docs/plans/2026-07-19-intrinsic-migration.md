...
Migrate terminal I/O and environment variable access from compiler intrinsics (`Print#`, `PutChar#`, `GetEnv#`, `GetEnvInt#`) to the standard library. Operations that can be expressed in pure Brief using lower-level intrinsics (`SysCall#`, `Load#`) should not be compiler-known.
...
- Pure-Brief stdlib implementations using `SysCall#(Write, ...)` and `Load#`-based environ scan
...
| `Sqrt#`/`Sin#`/`Cos#`/`Fabs#`/`Ceil#`/`Floor#`/`Pow#` | LLVM `@llvm.*` intrinsics — no portable pure-Brief impl |
...
| `GetEnv#(name)` | `lib/std/env.bv`: `getenv(key) -> String` | pure-Brief `Load#`-based environ scan |
| `GetEnvInt#(name)` | `lib/std/env.bv`: `get_env_int(key) -> Int` | `getenv` + `parse_int` in Brief |
...
## Phase 1: GetEnv#/GetEnvInt# → Pure-Brief Environ Scan
...
**Mechanism:** A one-line C helper returns the `environ` pointer as an integer. Pure-Brief code then:
...
// Pure-Brief getenv scans this pointer array to find key=value pairs.
frgn __get_environ() -> Int;
```

### Pure-Brief getenv implementation
...
defn getenv(key: String) -> String {
    ...
    txn scan [true][true] -> String {
        ...
        // Compare key prefix — pure-Brief string ops
...
> Note: `starts_with`, `slice`, `parse_int` are pure-Brief string utilities available in stdlib. `Load#` is the retained memory-read intrinsic. `ptr_to_string` converts a null-terminated C string pointer to a Brief String — uses `Load#` + byte iteration.
...
| File | Change |
|------|--------|
| `lib/runtime/brief_rt.c` | Add `__get_environ()` |
...
**Helper — `int_to_str` in pure Brief:**
```brief
...
```markdown
| ~~I/O~~ Terminal & Env | — | Moved to stdlib. `!Print`/`!PrintLn` dispatched via Front plugin; `!GetEnv`/`!GetEnvInt` resolved to pure-Brief environ scan. All use `SysCall#(Write, ...)` or `Load#` underneath. |
```
...
- Test: `!GetEnvInt("BOUND")` → `get_env_int("BOUND")` → pure-Brief scan
...
- Compare output of Brief and C programs for `print_loop`, `mandelbrot`, `fannkuch_redux`
...
| `lib/std/io.bv` | Pure-Brief I/O: `print_int`, `print_str`, `print_float`, `print_char`, `int_to_str` |
| `lib/std/env.bv` | Pure-Brief env: `getenv`, `get_env_int` |
...
| `lib/runtime/brief_rt.c` | 1 — add `__get_environ()` |
...
- **C runtime**: `// 2026-07-19: Helper for pure-Brief environ scan. Returns char **environ as Int.`
