# Compiler-in-Briv — dogfooding the GLUE FFI

**Date:** 2026-08-04
**Status:** Active plan
**Branch:** `compiler-in-briv` (worktree `../briv-compiler-dogfood`)
...
The compiler uses its own FFI: **one Rust compiler pass written in Briv,
compiled by `brivc` to a native `.a`, linked into `brivc` at build time, and
...
incremental compiler-in-Briv path.

## The PoC: `compute_export_needs_state` in Briv
...
A serializer emits a **tagged Data Briv projection** per export: name, params,
return type, the declared **state-field names**, and the **body as a tagged
tree** (statement kinds, identifier references, call names) — enough for Briv
...
A Briv program that reads the projection (`CStr`), walks each export body for
...
### P3 — native linkage
A root **`build.rs`** runs `brivc build lib/compiler/needs_state.bv --library`
→ `libneeds_state.a` and links it into `brivc` (the rust-host pattern). No
circularity — the Rust `brivc` compiles the pass. `extern "C"` binding:
...
bitmask. **Behavioral test**: `brivc export/bindings` on `boundary.bv` and
`node_bridge.bv` produce **byte-identical** shims to today; a transition test
asserts the Briv result equals the (now reference) Rust result on a corpus of
bridges. `cargo test --lib` green.

### P5 — generalize
Extract the reusable AST-projection serializer + Briv reader into
...
`docs/architecture/compiler-in-briv.md`.

## Decisions

1. Handoff: **tagged Data Briv projection** (readable, established via
...
- `brivc`'s build gains a brivc invocation in `build.rs` (the rust-host crate
  already proves this).
...
(Briv result == Rust reference) on a corpus; Praetor on changed dirs.
...
2. **Real compiler bugs fixed along the way** (each committed with the
 ...
 4. **`lib/compiler/needs_state.bv`**: rewritten as PURE string scanning (no
    ...
    for iteration, `char_at`/`briv_str_substr` frgns for scanning (the LLVM
    backend's String slice returns the whole array — see BUGS.md). The dynamic
    String slice was implemented as runtime `briv_str_substr` + a `char_at`
 ...
 6. **THE PASS WORKS**: `needs_state_compute` matches the Rust reference on all
    ...
    first (`__briv_init_state()`), which the first C driver got wrong — the
...
`briv_pass::compute_export_needs_state` (Briv pass when loaded, reference
otherwise). `tests/c_driver_needs_state.rs` and the `briv_pass` unit test
assert the Briv result equals the Rust reference on the five-bridge corpus;
...
`briv_pass.rs` is now a generic `LoadedPass` dlopen loader; build.rs compiles
both passes. The Briv `soa_reorder_compute` produces the EXACT permutation of
the Rust `reorder_fields` on the real `nbody_newton.bv` benchmark (asserted by
`soa_reorder.rs::briv_reorder_matches_reference_on_nbody` +
`briv_pass.rs::soa_pass_matches_reorder_fields`), and the backend wires it at
`mod.rs:1808`. Recipe: `docs/architecture/compiler-in-briv.md`. Also fixed
