...
**Baseline worktree:** `../briev-compiler-baseline` (detached HEAD at `666fb502`)
...
| Briev-level LICM | `analysis/licm.rs` | hoists loop-invariant let-bindings before codegen |
...
### 5.4 Rule 18 violations — hardcoded Briev type-name matching (Tier D)

AGENTS.md Rule 18: never match Briev type names in Rust. Verified sites:
...
4. For dispatch-changing phases: benchmark A/B vs `../briev-compiler-baseline`
...
**Zero MISMATCH.** (Ratio < 1 = Briev faster.)
...
and the type-driven range. §8.4 replaces every hardcoded Briev type-name match in
...
- The baseline worktree `../briev-compiler-baseline` is never updated during
