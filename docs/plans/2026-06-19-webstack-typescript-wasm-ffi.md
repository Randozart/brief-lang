...
>
> See:
> - `docs/architecture/features/rendered-briev-wasm.md` — current spec
> - `docs/plans/2026-07-26-rendered-briev-webstack-v2.md` — current plan
>
...
Briev's web target has two compilation paths that coexist in the same build:
...
|---|---|
| `"c"` (known intrinsic: `__print_int`, etc.) | Compile `briev_rt.c` to wasm32 bitcode, link |
...
## Phase 0 — `.rbv` Format: Briev as Default

### Goal
Flip the `.rbv` file format so Briev code is the default content, not wrapped in `<script>` tags. CSS imports via `import "file.css"` already works — document and keep.
...
Everything outside `<view>...</view>` and `<style>...</style>` is Briev code.

### 0.3 — CSS Imports (already works)

Within the Briev section:
...
| File | Change | Lines |
|---|---|---|
| `src/rbv.rs` | `RbvFile::parse()` — remove `<script>` extraction, treat everything outside `<view>`/`<style>` as Briev | ~20 |
...
- Otherwise, use new Briev-as-default logic
...
|---|---|
| `test_parse_rbv_new_format` | Briev as default, `<view>` extracted, `<style>` optional |
...
### Goal
`briev build ui.rbv` emits valid `.ts` + `.html` — no wasm-bindgen dependency.

### A.1 — Signal Storage (webstack.rs)

Replace `Vec<JsValue>` with typed vectors:

| Briev type | TS storage | JS boundary |
|---|---|---|
...
- Emit a TS function with the Briev name as a JS builtin call
...
if target == "wasm32" {
    ...
    // briev_rt.c compiled to wasm32 bitcode, linked in
}
```

Also compile `briev_rt.c` to wasm32 bitcode to resolve standard intrinsics.
...
| Declaration | Action |
|---|---|
| `frgn name from "c"` where name is known intrinsic | Resolved via `briev_rt.wasm` |
...
```
Phase 0:
Step 0a: Rewrite RbvFile::parse() — Briev as default, backward compat
