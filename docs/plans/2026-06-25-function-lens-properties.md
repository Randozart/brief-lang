...
**Date:** 2026-06-25
**Status:** Planned
**Replaces:** `docs/plans/2026-06-25-native-briev-io-followup.md` Item 2 (Extension B)

---

## Motivation

Functions are first-class citizens in Briev's contract system, but there is no way to
...
**Indirect calls from within Briev are intentionally not supported.** Calling
...
Every use case for indirect calls has a better Briev-native mechanism:

| C pattern | Briev equivalent | Guarantees |
|---|---|---|
...
For Embedded Briev targets, `fn_name :> Address` is critical: it allows storing
...
declarations with `volatile_store#`. The address is just an integer; Briev's
...
`docs/plans/2026-06-25-native-briev-io-followup.md`:
...
- Consistent with Briev's existing lens philosophy
