...
**Purpose:** Tokenize Brief source text into a token stream.
...
**Purpose:** Type definitions for the Brief type system.
...
- `OpBinding` enum:
  ...
  - `Function(String)` — user-defined Brief function
...
**What it must contain:**
- `Display` for `Expr` — produces valid Brief source text
- `Display` for `Type` — produces valid Brief type syntax
- `Display` for `Statement` — produces valid Brief statement syntax
- `Display` for `TopLevel` — produces valid Brief top-level syntax
...
**Purpose:** Lower Brief types to LLVM types.
...
**Purpose:** ABI marshaling between C calling convention and Brief internal types.

**Functions:**
- `fn marshal_param_to_brief(param_ty: &Type, param_reg: &str) -> String`
  - Bool: `trunc i8 %param to i1`
  - Others: identity (pass through)
- `fn marshal_brief_to_return(ret_ty: &Type, ret_reg: &str) -> String`
...
- `bindgen.rs`: `__brief_init_state`/`__glue_release` in C/Rust/Python headers
