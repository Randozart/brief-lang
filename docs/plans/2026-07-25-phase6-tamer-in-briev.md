# Phase 6: Write the Tamer in Briev — Self-Hosted Install-Time Compiler
## 2026-07-25

## Overview

The tamer system tool is currently implemented in C (`tamer/main.c`, `tamer/interp.c`).
Phase 6 rewrites the entire tamer in **Briev itself**, then compiles it to a native binary
via `brievc build --backend llvm`. This is the ultimate validation of the Bounty
architecture — Briev compiling a compiler tool, written in Briev, that compiles user
programs at install time.

### Architecture

```
lib/tamer/*.bv  (Briev source — the tamer)
    │
    ▼ brievc build --backend llvm tamer_rt.c
    │
...
| Layer | Language | Role |
|-------|----------|------|
| `lib/tamer/*.bv` | Briev | VM interpreter, .bounty parser, orchestration |
...
written in Briev using pattern matching. The **host FFI** (LLVM, linker, filesystem)
...
changes is WHO implements the interpreter — it moves from C to Briev.

### Why This Matters

1. **Self-hosting milestone**: Briev compiles a real-world systems tool written in
...
2. **No more C in the critical path**: Eventually the `tamer_rt.c` can also be
   replaced with Briev + direct LLVM IR emission, but for Phase 6 it remains
   as a thin (~100 line) FFI layer.

3. **Demonstrates the VM backend's purpose**: The `brievc bounty` command produces
...
| File | Change |
|------|--------|
| `tamer/Makefile` | Add rule to compile tamer from Briev: `brievc build --backend llvm lib/tamer/main.bv tamer_rt.c -o tamer` |
...
The VM implementation is a Briev module that implements the fetch-decode-execute
...
**Stack operations are `frgn` calls**, not inline Briev operations. This is
...
`load_i64`) which are inline in Briev and use Ptr arithmetic:
...
Or they could be direct `frgn` calls if Briev can't do pointer dereferencing
...
interpreter loop moves to Briev.

```c
// tamer_rt.c — Low-level runtime for the Briev-compiled tamer.
...
// ── VM state (kept in C, accessed by Briev via Ptr<Int>) ────────────────
typedef struct {
    uint64_t* stack;
...
| 6 | Compile: `brievc build --backend llvm lib/tamer/main.bv tamer_rt.c -o tamer` | Makefile | Steps 1-5 |
...
```bash
# 1. Build the native tamer from Briev source
cd lib/tamer
brievc build --backend llvm main.bv ../../tamer/tamer_rt.c -o ../../tamer/briev_tamer

# 2. Create a test bounty
brievc bounty /tmp/test.bv -o /tmp/test.bounty

# 3. Process with the Briev-compiled tamer
./tamer/briev_tamer /tmp/test.bounty
...
Run each phase's end-to-end test with both the C tamer and the Briev tamer,
comparing outputs:
...
# Briev tamer
./tamer/briev_tamer /tmp/test.bounty -o /tmp/out_briev
/tmp/out_briev > /tmp/result_briev.txt

# Compare
diff /tmp/result_c.txt /tmp/result_briev.txt  # should be empty
```
...
- The Briev tamer must produce identical output to the C tamer for the same input
...
| Risk | Mitigation |
|------|------------|
| **Briev's Ptr<Int> support isn't low-level enough** | All pointer operations go through `frgn` calls to the C runtime. Briev code only does integers, booleans, structs, and match expressions — no unsafe pointer work in Briev itself. |
...
| **Briev compiler doesn't support all patterns used** | The tamer's code uses only a restricted subset of Briev: `defn`, `if/match`, `for` loops, `fn` calls, `let` bindings, arithmetic. No generics beyond the needed scope. |
| **Old C tamer breaks during transition** | Keep making `tamer/tamer` as the C version. Create `tamer/briev_tamer` as the Briev version alongside it. Remove the C version only after the Briev version is verified identical. |
...
| Document | Change |
|----------|--------|
| `docs/plans/2026-07-25-bounty-architecture.md` | Update Phase 6 section with briev impl details |
| `docs/architecture/bounty-architecture.md` | Update tamer section to describe Briev compilation |
...
```briev
// 2026-07-25: Tamer VM interpreter in Briev.
// Implements the fetch-decode-execute loop for .lair bytecode.
// Compiled natively via brievc --backend llvm.
// Replaces the C interpreter (tamer/interp.c) with Briev source.
```
