...
`briev build file.ebv --backend llvm` — explicit `--backend` overrides the config file.
...
|------|-----------|--------|----------|
| **Operation** | Backend config file (`llvm-ops.toml`) | Briev syntax: `+`, `==`, `++` | Config lookup: `(op, primitive, bytes)` |
