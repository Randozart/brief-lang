# Ship common language environments — Briev as the central language
...
Ship `briev bindings` / `briev extension` targets for a roster of common
...
changes) and makes Briev the true central language.
...
| Go | bindings | cgo: `import "C"` externs + typed wrappers; String → `C.GoString` (zero-copy read) | portable Go (~/briev-tools) |
| Java | native ext | JNI shim (`JNIEXPORT` wrappers; String → `NewStringUTF`) + Java class with `native` methods | portable Temurin JDK (~/briev-tools) |
| C# | bindings | P/Invoke `[DllImport]` + `Marshal.PtrToStringUTF8` for the composite | guarded (no .NET) |
| Lua | native ext | C module `luaopen_bridge`; String → `lua_pushstring` | Lua built from source (~/briev-tools) |
...
- **Render tests (run everywhere):** assert `briev bindings|extension` produces
...
vs the native C reference (`rank_ref.c`) and the native Briev `.a` call, on the
...
At the current commit: Rust→Briev 1127ns, C→Briev 1092ns, Python native ext
