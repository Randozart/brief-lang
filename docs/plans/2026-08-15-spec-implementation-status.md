...
| 8 | Declarations | In progress | trait/impl + relationship list + coherence + structural conformance (Phase 5); `meld` removed (staged-reject); union untagged overlay + atomic field modifier; init-kind box/spill classification phases 1–4 (2026-08-14); `coll` obj/struct/seq + capacity intrinsics + Grow/Shrink override (2026-08-14/15). **Grow-on-full normative + implemented (2026-08-15)**: default `op Grow` doubles capacity when `len == cap`; the frontend bounded-length analysis strips the guard when provably dead (queue_drain_idio 0.59x; plan 2026-08-15-coll-loop-guard-elimination). Remaining: `Ptr<T>`-backed `coll struct` (documented follow-up) |
