...
| 16 | Literals, ranges, slicing | In progress | width suffixes removed; `#b` Data byte literals + `#r` raw strings + Python slices shipped 2026-08-06; `..=` inclusive range patterns shipped 2026-08-06 (DotDotEq token, `Pattern::RangeInclusive`, interpreter + codegen match lowering); `data[mask]` boolean mask indexing shipped 2026-08-07 (interpreter masked select + typechecker Data typing + runtime gather helper; mask = Bool list literal or `Bool[N]` state field); remaining: standalone `..`/`..=` range *expressions* (as values), general-container masks (`Int[N][mask]`), named selectors, const generics |
