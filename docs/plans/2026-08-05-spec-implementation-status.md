...
| 16 | Literals, ranges, slicing | In progress | width suffixes removed; `#b` Data byte literals + `#r` raw strings + Python slices shipped 2026-08-06; `..=` inclusive range patterns shipped 2026-08-06 (DotDotEq token, `Pattern::RangeInclusive`, interpreter + codegen match lowering); remaining: standalone `..`/`..=` range *expressions* (as values), boolean mask indexing, named selectors, const generics |
