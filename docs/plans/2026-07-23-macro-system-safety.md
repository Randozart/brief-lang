...
Files written via `FileWrite$` at any phase become new source files that are compiled in a separate, clean compiler invocation. A single `briev build` orchestrates both passes transparently, with the guarantee that macro-generated code is NEVER compiled in the same pass as the macro that generated it.
...
briev build bridge.bv --dump-vfs

# CI — write generated files as part of the build
briev build bridge.bv --allow-write
```
...
[!] Plugin 'glue-generator' has changed since last approved.
    ...
    Run `briev audit` to inspect changes, or update macro-lock.toml
```
...
| 7 | Add `briev audit` command (static capability scan) | 2 |
...
Driven by `briev.toml` target profiles, NOT by the macro source code:
...
`SysQuery$` feeds directly into Briev's contract system to create optimizations
...
### Build Configuration (`briev.toml`)
...
```
                      ...
                      │
briev build ──────────┼───► [Pass 2: aws-h100-node]    SysQuery$ reads nvidia_h100.json
                      │
...
| **2c** | **Add target profiles in `briev.toml`** | **2a** |
