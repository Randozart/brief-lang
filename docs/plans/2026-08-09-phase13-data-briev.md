# Phase 13 — Data Briev: validation, canonical serialization, CLI check

**Date:** 2026-08-09
**Status:** Implementation plan
**Normative source:** `spec/SPEC.md` §22 (Data Briev),
`docs/plans/2026-08-05-implement-normative-language-spec.md` §19 (Phase 13)

## 0. Goal

Complete the Data Briev surface: schema validation (§22.5), canonical
serialization (§22.6), `briev check` extension dispatch (§22.6), and the
`.dbvs` migration (§19.5). The parser is mature (`src/dbriev/v2.rs`, 69 tests);
...
| `briev check file.dbv/.dbvl` dispatch | NOT implemented — run_check → check_source (.bv-only) |
| `.dbvs` migration (§19.5) | `.dbvs` rejected + not swept, but stale files remain |

## 2. Scope — 4 slices

### Slice A — Schema validation (`src/dbriev/validate.rs`)
`validate_document(&DbrievDocument) -> Result<(), Vec<String>>` — when a schema
...
### Slice B — Canonical serialization (`src/dbriev/serialize.rs`)
`canonicalize_document(&DbrievDocument) -> String` — deterministic field/key
...
### Slice C — `briev check` extension dispatch
...
| Change | File |
|---|---|
| validate.rs (new) | `src/dbriev/validate.rs`, `src/dbriev/mod.rs` |
| serialize.rs (new) | `src/dbriev/serialize.rs` |
...
- Existing 69 dbriev parser tests stay green.
...
- **Slice C**: `briev check file.dbv|.dbvl` dispatches to `check_data_source`
