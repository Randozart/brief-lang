...
**Rationale**: The LLVM C ABI passes float as float in registers (xmm), but Briev's internal
...
Briev's register model. The `reg_float_cache` maps `%fze → %call_result` so downstream code
...
| C ABI parameter marshaling | Box to i64 + cache bridge | Store direct float in state field | ABI expects float in xmm registers, but Briev's cross-function internal ABI is i64-uniform. The cache preserves the native float for consumption within the same function. |
| Intrinsic float return | Box result to i64 + cache bridge | Return native float directly | Consistent with Briev's internal ABI (all values are i64). Cache recovers native float for downstream computation. |
