...
Current: Briev wins 8, C wins 8 (of 18 benchmarks).
Target: Briev wins 14+, C wins 2-3 (only fannkuch_redux fencepost and
        irreducible gaps).
...
| fannkuch_redux | 1.76× | Pre-existing fencepost bug (6 vs 10 lines). Codegen is correct for what it compiles. | Bug fix in Briev source | ~1.0× |
...
Briev's sequential mutation semantics create serial dependency chains
...
This is CORRECT Briev semantics — `&x0 = ...` mutates x0, subsequent uses
see the new value.
...
But this changes semantics — the C reference does this, but Briev's semantics
...
sequential dependency is a fundamental Briev semantics choice.

The remaining gap (2.12×) is because C auto-vectorizes the three independent
computations. Briev cannot do this because the semantics are different. The
gap is inherent, not a codegen regression.

**Remove from scope**: This optimization changes Briev semantics. Drop it.
...
Briev's sequential semantics create data dependencies between `&`
