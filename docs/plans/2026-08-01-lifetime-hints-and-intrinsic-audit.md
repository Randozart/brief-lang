...
3. **Free-check** — the garbage scheduler's unprovable tail is reclaimed by a
   ...
   contracts; `brievc memcheck` reports every unprovable point and the hint that
   closes it.
4. **C-surface reduction** — the `.bv`/`.ebv` split (Rust's core/alloc/std
   layering, in Briev's file-extension terms): logic moves to `.ebv` pure Briev,
...
- **`.bv` vs `.ebv`** — the same stdlib API has two target variants: `.bv`
  ...
  Briev logic). The import resolver prefers `.bv` on OS targets and `.ebv` on
...
### 1.1 `syntax-highlighter/syntaxes/briev.tmLanguage.json`
- REMOVE the `"macro-words"` repository (`\b[a-zA-Z_][a-zA-Z0-9_]*!` →
  `entity.name.tag.macro-call.briev`) and its `"include": "#macro-words"`.
- REMOVE the `"macros"` repository (`\b(entry|args|print|println|get_env|get_env_int)!` →
  `keyword.control.macro.briev`) and its `"include": "#macros"`.
- ADD standalone `?` and `!` operator rules after the existing `@`/`&` rules,
  same scope name (`storage.modifier.ownership.briev`). Rule order keeps
...
### 1.2 `syntax-highlighter/syntaxes/dbriev.tmLanguage.json`
- ADD the standalone `?`/`!` rules after `@`/`&` (mirroring the dbriev
...
- For heap fields the static proof cannot close (an unordered reader exists),
  ...
  zero count → `__briev_free`. Sound (no premature free — only freed when no
...
C; `brievc memcheck` lists the unprovable points.
...
- Classify every `briev_rt.c` function as **syscall shim** (stays C) vs
  **logic** (moves to `.ebv`): the digit/float formatting
  (`__print_int`/`__print_float` itoa/ftoa), string ops
  (`briev_str_eq`/`band`/`bor`/`bxor`/`bnot`/`concat`/`len`), the allocator.
...
- Introduce `lib/std/*.ebv` implementations of the formatting/string/allocator
  logic in pure Briev (byte loops, digit conversion, a free-list allocator on a
