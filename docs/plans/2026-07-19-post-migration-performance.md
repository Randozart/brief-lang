...
**Fix:** The chunk allocator (`emit_inline_init_stores` / chunk type emission) should use the field's Brief type to determine the LLVM type. Float32 → `float`, Float64 → `double`, Int → `i64`. This restores the Phase 3 behavior where `%StateChunk0 = type { i64, i64, float, float, ... }`.
