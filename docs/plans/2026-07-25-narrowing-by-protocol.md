...
defn f() -> Int { term 42; }
EOF
briefc build --backend llvm /tmp/t1.bv 2>&1
...
EOF
briefc build --backend llvm /tmp/t2.bv 2>&1
llc -O0 /tmp/t2.ll -o /dev/null     # passes

# 3. ShellCmd# compiles and runs
briefc build --backend llvm /tmp/shell.bv 2>&1
clang /tmp/shell.ll lib/runtime/brief_rt.c -o shell
./shell                     # works
```
