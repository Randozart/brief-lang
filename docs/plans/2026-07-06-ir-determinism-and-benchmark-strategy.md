...
and Brief" and "not so deterministic that the compiler can predict at compile
time." No changes needed for the benchmark sources.

### What's left: IR non-determinism

The same Brief source compiled twice produces different LLVM IR because Rust's
