...
Brief has evolved from universal transpilation to a **backend-per-variant** architecture. Each file extension now targets a specific backend:

| Extension | Name | Backend | Role |
|-----------|------|---------|------|
| `.bv` | Brief | LLVM | General-purpose native |
| `.sbv` | Strict Brief | LLVM | Full contract verification |
| `.abv` | Accelerated Brief | SPIR-V via LLVM | GPU acceleration |
| `.rbv` | Rendered Brief | Webstack (WASM+JS) | Web frontend |
| `.srbv` | Strict Rendered Brief | Webstack | Verified view-state isomorphism |
| `.ebv` | Embedded Brief | LLVM (restricted mode) | Bare-metal embedded |
| `.sebv` | Strict Embedded Brief | LLVM (restricted mode) | Verified embedded |
| `.cbv` | Circuit Brief | CIRCT | Hardware (synthesizable) |
| `.dbv` / `.dbvs` / `.dbvl` | Data Brief | Config schema | Configuration/data |
...
**Validation**: `brief build foo.ebv` should compile through LLVM (not error). The `is_embedded` flag causes restricted-mode checks in Phase 3. For now, it's a silent flag.
...
**Validation**: `brief build foo.cbv` should parse and emit `.mlir` via CIRCT backend. If CIRCT tools are not installed, print warning and emit raw `.mlir`.
...
**Validation**: `brief compile foo.ebv` infers `llvm.toml`. `brief compile foo.cbv` infers `circt.toml`.
...
**Validation**: `brief build foo.ebv` does NOT run `B500x` checks. `brief build foo.cbv` DOES run `B500x` checks. All tests pass.
...
- `Statement::TermBang` emits `call void @llvm_brief_halt()` instead of `br label %done`
