...
**Error:** Brief outputs 2 lines, C outputs 1 line.
...
**Symptom:** Brief runs in 0.0006s (600μs, just process overhead), outputs
...
**Functions using #8:**
- `brief_main` (user `defn` definitions)
...
`#0` (no `argmemonly`) or `#3` (no `argmemonly`).  So `@brief_main` and
...
`brief_main`, scan the body for FFI calls that access globals.  If found,
use `#0` instead of `#8`.
...
body (Brief has no hidden mutation through pointers).  The latch uses
...
It is not user-facing code and does not need to satisfy the Brief compiler's
...
5. Benchmark the fixed benchmark(s) individually:
   ```bash
   ./target/release/brief-compiler llvm benchmarks/<name>.bv --out /tmp/test
