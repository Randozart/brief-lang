...
The baseline at commit `8a827db` achieved **all benchmarks MATCH with competitive runtime** — Briev beat C on 9 of 15 runtime benchmarks (ratio 0.01x–0.95x). Two major changes since then have degraded performance:
...
| float_math | 0.0631s | 0.0771s | 0.81x | Briev |
| float_math_nonzero | 0.1920s | 0.1727s | 1.11x | C |
| sparse_dispatch | 0.0060s | 0.0657s | 0.09x | Briev |
| print_loop | 0.0639s | 0.0670s | 0.95x | Briev |
| nbody_newton | 7.4132s | 9.8522s | 0.75x | Briev |
| nbody_sqrt | 3.0046s | 3.5218s | 0.85x | Briev |
| nbody_sqrt_idio | 2.9578s | 4.3184s | 0.68x | Briev |
| fasta | 0.2695s | 0.2636s | 1.02x | C |
| fannkuch_redux | 0.0763s | 0.0789s | 0.96x | Briev |
| mandelbrot | 0.7514s | 0.7538s | 0.99x | Briev |
| kalman_filter_runtime | 0.1876s | 0.1887s | 0.99x | Briev |
| knucleotide | 0.2093s | 0.2060s | 1.01x | C |
| cancel_math | 0.0682s | 0.0672s | 1.01x | C |
| bit_clear | 0.0010s | 0.0009s | 1.11x | C |
| queue_drain | 0.0007s | 0.0632s | 0.01x | Briev |
| queue_drain_sym | 0.0639s | 0.0672s | 0.95x | Briev |
| interval_step | 0.0009s | 0.0669s | 0.01x | Briev |
...
This checks the `TypedRegister.ty` field, which is the Briev type (e.g., `Type::Custom("Float")`). If the identifier lookup returns `TypedRegister { ty: Type::int() }` for a float field (because it loaded `i64` from `%State`), then `is_float` would be false, and the binary op would emit `add` or `sub` instead of `fadd`/`fsub`!
...
Actually, the identifier lookup for local bindings (first branch) uses `self.get_local_type(name)` which returns the actual Briev type from `let_binding_types`. So if a float value was assigned to a `let` binding, it would carry the correct type. But GEP+load from `%State` always returns `Type::int()`.
...
4. `ensure_float_reg` should check `rt.storage` — since CTD+ALU has no explicit "Native" storage flag, the equivalent check is: if the register's LLVM type (from `llvm_type`) is `float` or `double`, return as-is. If it's `i64` but the Briev type is Float, emit trunc+bitcast.
...
fn ensure_float_reg(&mut self, out: &mut String, indent: &str, reg: &TypedRegister) -> String {
    ...
    // Is the Briev type a float? Then we have an i64 boxed value
...
**A1 — Fix identifier type info:** Ensure `Expr::Identifier` returns the correct Briev type for state fields (not always `Type::int()`). This fixes float arithmetic in the loop body.
