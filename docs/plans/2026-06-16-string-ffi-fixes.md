...
## Problem 1 — `__str_concat` uses C strings instead of Brief strings

`__str_concat` in `brief_rt.c` expects `const char*` (C strings). But the generated IR passes Brief string headers (2-slot format: data_ptr + length + characters) as `i64` values via `inttoptr`. `strlen` on a Brief header reads the header bytes as string data → SIGSEGV.

The `safe_cstr` hack masks the crash but produces empty strings and leaks memory.

**Fix:** Rewrite `__str_concat` to operate entirely in Brief string space:
- Signature: `int64_t __str_concat(int64_t a, int64_t b)`
- Validate inputs as Brief headers (data_ptr == header + 16, 0 < len < 1M)
- Allocate result header, copy characters from both inputs
- Return `ptrtoint` of result header

---

## Problem 2 — FFI marshaling doesn't convert between Brief and C strings
...
**Fix:**
- **Inbound** (Brief → C): Extract `data_ptr` from slot 0 of the Brief header before passing to the C function.
- **Outbound** (C → Brief): Wrap the C `char*` return in a Brief header via `cstr_to_brief()` (already exists in `brief_rt.c`).
...
Change signature from `char*(char*, char*)` to `int64_t(int64_t, int64_t)`. Operate on Brief headers:
...
```llvm
; raw = i64 Brief string header pointer
...
```llvm
; %call_result is i8*
%brief = call i64 @cstr_to_brief(i8* %call_result)
; %brief is the Brief string header ptr
```

### Item 4 — Add `cstr_to_brief` declare

**File:** `src/backend/llvm/mod.rs` — declare section

```rust
writeln!(out, "declare i64 @cstr_to_brief(i8*) #1").ok();
```

Also make `cstr_to_brief` in `brief_rt.c` non-static (remove `static`).
...
### Item 7 — Cleanup `brief_rt.c`

- Remove `fprintf(stderr, "DEBUG __int_to_str...` from `__int_to_str`
- Remove `safe_cstr` function and `#include <unistd.h>`
- Remove `__resolve_brief` helper
- Make `cstr_to_brief` non-static (line 1272)

### Item 8 — End-to-end test

```bash
cd ~/Desktop/Projects/officina-cli
cargo build --release -p brief-compiler
./target/release/brief-compiler llvm officina.bv
clang -O3 officina.ll lib/runtime/brief_rt.c -lc -o officina_bin
...
|------|------|---------------|
| `lib/runtime/brief_rt.c` | Rewrite `__str_concat`; remove debug/safe_cstr; make `cstr_to_brief` non-static | ~40 |
| `src/backend/llvm/emit_expr.rs` | FFI arg/return marshaling for String; `Expr::Concat`/`emit_binop` emission | ~30 |
| `src/backend/llvm/mod.rs` | Update `__str_concat` declare; add `cstr_to_brief` declare | ~3 |
