...
## Problem 1 — `__str_concat` uses C strings instead of Briev strings

`__str_concat` in `briev_rt.c` expects `const char*` (C strings). But the generated IR passes Briev string headers (2-slot format: data_ptr + length + characters) as `i64` values via `inttoptr`. `strlen` on a Briev header reads the header bytes as string data → SIGSEGV.

The `safe_cstr` hack masks the crash but produces empty strings and leaks memory.

**Fix:** Rewrite `__str_concat` to operate entirely in Briev string space:
- Signature: `int64_t __str_concat(int64_t a, int64_t b)`
- Validate inputs as Briev headers (data_ptr == header + 16, 0 < len < 1M)
- Allocate result header, copy characters from both inputs
- Return `ptrtoint` of result header

---

## Problem 2 — FFI marshaling doesn't convert between Briev and C strings
...
**Fix:**
- **Inbound** (Briev → C): Extract `data_ptr` from slot 0 of the Briev header before passing to the C function.
- **Outbound** (C → Briev): Wrap the C `char*` return in a Briev header via `cstr_to_briev()` (already exists in `briev_rt.c`).
...
Change signature from `char*(char*, char*)` to `int64_t(int64_t, int64_t)`. Operate on Briev headers:
...
```llvm
; raw = i64 Briev string header pointer
...
```llvm
; %call_result is i8*
%briev = call i64 @cstr_to_briev(i8* %call_result)
; %briev is the Briev string header ptr
```

### Item 4 — Add `cstr_to_briev` declare

**File:** `src/backend/llvm/mod.rs` — declare section

```rust
writeln!(out, "declare i64 @cstr_to_briev(i8*) #1").ok();
```

Also make `cstr_to_briev` in `briev_rt.c` non-static (remove `static`).
...
### Item 7 — Cleanup `briev_rt.c`

- Remove `fprintf(stderr, "DEBUG __int_to_str...` from `__int_to_str`
- Remove `safe_cstr` function and `#include <unistd.h>`
- Remove `__resolve_briev` helper
- Make `cstr_to_briev` non-static (line 1272)

### Item 8 — End-to-end test

```bash
cd ~/Desktop/Projects/officina-cli
cargo build --release -p briev-compiler
./target/release/briev-compiler llvm officina.bv
clang -O3 officina.ll lib/runtime/briev_rt.c -lc -o officina_bin
...
|------|------|---------------|
| `lib/runtime/briev_rt.c` | Rewrite `__str_concat`; remove debug/safe_cstr; make `cstr_to_briev` non-static | ~40 |
| `src/backend/llvm/emit_expr.rs` | FFI arg/return marshaling for String; `Expr::Concat`/`emit_binop` emission | ~30 |
| `src/backend/llvm/mod.rs` | Update `__str_concat` declare; add `cstr_to_briev` declare | ~3 |
