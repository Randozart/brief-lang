...
(`long g_a = 25000000L; (void)g_a;`), while the Brief body gained `print_int#()`
...
- **Brief**: `print_int#(a)` guards in body (observable, non-pure)
- **C reference**: Real 2-thread pthread_barrier program with matching prints
- **What it tests**: "Does Brief's thread pool throughput match C's pthread?"
- **Expected**: Both ~500s at BOUND=50e6 — fair comparison

### `async_counters_idio` (optimizer tag)
- **Brief**: Pure bodies (no prints), multi-txn pure fold → O(1) register pipeline
- **C reference**: Current trivial version (clang folds to `long x = N`)
- **What it tests**: "Does Brief fold pure async loops as aggressively as C?"
