...
with the `<-` arrow restoration and native `ring_push`/`ring_pop` in Briev.
...
`String` and `Ptr<T>` map to LLVM `"ptr"`. Fix: check Briev type first:
...
## Phase 3: Implement `ring_push` / `ring_pop` in Briev

**3a. Write the Briev implementations** (`lib/std/os/ring.bv`)
...
Now that `ring_push`/`ring_pop` are Briev functions, the inline GEP path in
...
| `lib/std/os/ring.bv` | 3a | Native ring_push/ring_pop in Briev |
