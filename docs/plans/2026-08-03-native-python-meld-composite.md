...
1. **Native Python speed without ctypes.** Python→Briev via ctypes is 2033 ns/call
...
2. **Meld-composite boundaries.** The author's insight: a meld
   ...
   One buffer: Briev reads `[len][bytes]`, C reads a nul-terminated `char*`,
...
1. **Lifetime = the state's life.** A composite view is valid from creation
   ...
   handle; while it lives, String/Data memory is stable (Briev values are
   immutable, the arena doesn't move).
2. **Ownership:** Briev owns; hosts borrow read-only for the state's life; each
...
3. **Mutability is declared by the meld, not per language.** Default read-only
   (Briev Strings are immutable — every language must see them read-only). A
...
- `briev extension <bridge> python` — config-driven:
...
- Benchmark: native extension vs ctypes vs C. **Baseline:** Python→Briev ctypes
  2033 ns/call (Python→C 1927). **Target:** ~300–500 ns/call.

  **P1 result (2026-08-03, `feature_hash` count=1000, 200k calls):**
  | path | ns/call | vs |
  |------|---------|----|
  | Python → Briev (ctypes) | 3057 | — |
  | Python → Briev (native ext) | 1297 | **2.4×** |
  | Python → Briev add (ctypes) | 1058 | — |
  | Python → Briev add (native ext) | **179** | **6×** |
...
- **Audit result (2026-08-03): the invariant already holds everywhere.**
  ...
  - Runtime `briev_cstr_to_briev` / `__int_to_str__` — `malloc(len+9)`,
    `buf[8+len]='\0'`.
...
- **Change:** `briev_str_to_c` now returns the in-place data pointer
...
- **Done (2026-08-03).** `lib/glue/c.bv` declares `meld CStr -> String` — the
  ...
  boundary marshalling inserts the delta (`cstr_to_briev`/`str_to_c`) at those
