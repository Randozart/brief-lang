...
`push_field_type` (mod.rs:902) pushes `"i64".to_string()` for ALL state fields, regardless of their Briev type. Float32 values take 4 instructions per access:
```
...
|----------|-----------|--------|
| Identifier resolution (state memory load) | `emit_expr.rs:127-132` | Load with `field_types[idx]`. Remove the `briev_ty == Type::float32()` unboxing block. The load already returns the correct type. |
