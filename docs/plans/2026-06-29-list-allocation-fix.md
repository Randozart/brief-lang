...
An explicit escape analysis in the Briev compiler is possible but adds:
...
LLVM already has this analysis. Leveraging LLVM's pass is zero additional compile-time complexity in Briev, is maintained by the LLVM community, and handles edge cases (partial escape, phi nodes, etc.) that we'd have to reimplement poorly.
...
The current code never `free`s heap-allocated list headers. This is pre-existing (the `alloca`-based pattern also leaked — stack memory is freed on function return, but the data pointed to by the header was never freed). The Briev runtime currently has no garbage collector.
...
Beyond this fix:
1. **Add `free` for heap-allocated list headers** — Briev needs a GC or ownership system. Currently not blocking any feature.
2. **Escape-analysis pass in Briev** — if LLVM's promotion proves insufficient (e.g., partial escape through phi nodes), add a Briev-level analysis that chooses `alloca` vs `malloc` directly. This would be additive — default to `malloc`, override to `alloca` when escape is disproven.
