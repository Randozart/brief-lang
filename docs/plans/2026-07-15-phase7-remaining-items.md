...
- Currently `emit_trg_load` emits `load volatile` from the pointer value
  ...
  - `trg_null_<n>`: call `__brief_warn("unresolved trigger")` or abort
...
**Goal:** Brief documentation for each plugin stage directory explaining
...
**Goal:** Add `brief-compiler register <name>` CLI subcommand for
