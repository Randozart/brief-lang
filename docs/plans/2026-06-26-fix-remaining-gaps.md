...
```bash
cargo test --lib
cargo run --bin briev-compiler -- check examples/inop-sadd.bv
cargo run --bin briev-compiler -- check examples/inop-side-effect.bv
cargo run --bin briev-compiler -- check lib/std/string_builder.bv
```
