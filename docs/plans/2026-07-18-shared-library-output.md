...
│     • Call internal Briev body                                 │
│     • Return result via C ABI                                  │
│     • Teardown arena                                           │
│  3. Skip main loop, reactor tick, runtime init                 │
│  4. Emit __briev_init / __briev_fini (constructor/destructor)  │
...
**CLI:**
```
brievc build --shared example.bv --out component.so
```
...
// No briev_rt.o linked — no runtime threads, no barrier sync.
// No main() expected.
```
...
1. Emit the normal Briev function body (internal name: `briev_impl_f`)
...
entry:
  ...
  store ptr %arena, ptr @__briev_arena_ptr
  store ptr %arena_end, ptr @__briev_arena_end
  store ptr %arena, ptr @__briev_arena_base
  ...
  %result = call i64 @briev_impl_f(ptr %state, i64 %arg0, i64 %arg1)
...
(via the module's `__briev_arena_*` globals, not per-call stack arena).
...
3. **Thread safety** — The per-call stack arena is inherently thread-safe
   ...
   ```llvm
   store ptr %arena_base, ptr @__briev_arena_ptr
   ```
...
No `briev_rt.c` linked. The `.so` has zero external runtime dependencies
...
| `src/backend/llvm/emit_toplevel.rs` | L4 | `__briev_init` / `__briev_fini` constructors |
...
4. **Multiple `.so` loading** — If two Briev `.so` files are loaded into the
