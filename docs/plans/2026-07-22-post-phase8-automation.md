...
protocol path between `briev_type` and `briev_type` — the same type twice.

```rust
compute_protocol_path(briev_type, briev_type, universe)
```
...
let param_paths: Vec<ProtocolStep> = fb.inputs.iter()
    .map(|(_, briev_type)| {
        let foreign_type = lookup_foreign_type(briev_type, target);
...
Where `lookup_foreign_type` maps a Briev type to its foreign counterpart:

```rust
fn lookup_foreign_type(briev_type: &Type, target: &GlueTarget) -> Type {
    match briev_type {
        Type::Custom(name) => target.c_type_map.get(name)
            .map(|s| Type::Custom(s.clone()))
            .unwrap_or_else(|| briev_type.clone()),
        _ => briev_type.clone(),
    }
}
```
...
```bash
# Before: param_paths are all Identity (briev_type == briev_type)
...
briev build pp-types.bv --llvm --optimize-budget 0
grep -c 'realloc\|arena.*grow' target/pp-types.ll  # should be 0

# High budget → bump arena (optimal runtime)
briev build pp-types.bv --llvm --optimize-budget 256
grep -c 'realloc\|arena.*grow' target/pp-types.ll  # should be >0
```
...
in `compile.rs:239-244`. It scans bridge-path frgns, compares Briev type
...
always produces `Identity` paths (since it compares `briev_type == briev_type`).
...
briev build pp-types.bv --llvm --emit-beast all 2>&1
