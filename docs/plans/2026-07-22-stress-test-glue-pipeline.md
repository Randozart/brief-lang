# Stress-Test the GLUE/FFI Pipeline via `briev export`
...
Run `briev export pp-types.bv rust --out /tmp/pp-bridge` end-to-end and have it
produce a working Rust crate that Rust can call with `briev_pp_type_bits("42")`
...
1. **Type mapping** (`c_type_map`) — Briev types to foreign ABI types
...
| `{{name}}` | Function name | `"briev_pp_type_bits"` |
...
|-----|---------------|-----------------|
| **1** | TOML-driven template generation works | `briev export` produces files without hardcoded branches |
...
| **4** | Bridge functions return correct results | `briev_pp_type_bits("42")` returns `"Bits(42)"` |
