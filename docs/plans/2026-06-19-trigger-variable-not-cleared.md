...
```bash
cargo test --lib
./target/release/briev-compiler build ~/Projects/officina-cli/officina.bv
...
./target/release/briev-compiler build officina.bv
...
./target/release/briev-compiler build ~/Desktop/Projects/officina-cli/officina.bv
...
A `trg` is a **mailbox** — the outside world drops a value, Briev reads it. Writing to the `trg` variable mutates the local copy; the external source neither sees nor cares.
...
Same rule applies. a front-end button press emits a one-time signal into a `trg`. Briev can read and optionally write back (`&trg = ...`), but the front end never listens. The mailbox is one-directional regardless.
