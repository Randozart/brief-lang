...
1. **False MISMATCH signals** — C benchmarks auto-vectorize, changing Float32
   association order. Briev stays scalar. F32 precision differences (~1e-7) are
   inherent, not bugs.
...
3. **No vectorization** — The txn loop IR (`%State` struct load/store, branch
   ...
   vectorizer. C auto-vectorizes to `<4 x float>`/`<8 x float>`; Briev emits
...
- For each line, compare `abs(briev - c) < 1e-5` (relative epsilon)
...
1. **State extraction**: Currently Briev emits `load %State` + `extractvalue`
