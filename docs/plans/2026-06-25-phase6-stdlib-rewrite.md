...
declarations guarded by `#!cfg(target_os == "linux")`, then shrink `brief_rt.c`
...
`brief_rt.c` shims. The remaining intrinsics that use C shims are:
...
### Step 3: Remove dead C shims from brief_rt.c (30 min)

Remove the following sections from `brief_rt.c`:
...
- Phase B: all `brief_open` through `brief_fcntl` — direct libc already used in codegen
...
**Files modified:** `lib/runtime/brief_rt.c` (1744 lines → ~200 lines)
...
- Brief benchmarks pass correctness check
- brief_rt.c still links (no removed symbols that are referenced)
