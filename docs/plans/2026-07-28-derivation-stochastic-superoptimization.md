...
**Worktree:** `../briev-compiler-derive`
...
output files and an optional stochastic superoptimization pass. The `briev derive` command
...
| **Enumerative Synthesis** | Depth-bounded expression enumeration | `briev derive` (default) |
| **SMT Synthesis** | SyGuS query to Z3, QF_BV | `briev derive` (fallback) |
...
| **`briev accept`** | Fold synthesized bodies into source | User-initiated (Phase I) |
...
derivation block. This is the primary `briev derive` target.
...
When a function has BOTH a body and a derivation block, every `briev build` evaluates
...
Briev types to QF_BV sorts, emits example constraints, calls Z3, and parses the
...
fn parse_smt_response(
    ...
    // Convert SMT expression to Briev Expr tree
    let briev_expr = smt_to_briev_expr(body_expr, params, ret_type)?;

    Ok(SynthesizedProgram {
        body: vec![Statement::Term { values: vec![Some(briev_expr)], swan_song: None, modifiers: vec![] }],
...
**SMT to Briev expression mapping**:

| SMT expression | Briev expression |
|---------------|-----------------|
| `(bvadd a b)` | `a + b` |
| `(bvsub a b)` | `a - b` |
| `(bvmul a b)` | `a * b` |
...
/// Format a synthesized program as a readable Briev body string.
...
### Step E.3 — `briev derive` CLI command

**File**: `src/main.rs`, `src/derive/cli.rs`

**What**: Add `briev derive <file>` command with flags:

```
briev derive <file>           # Phase 1+2: enumerative + SMT → foo.derive.bv
briev derive --stochastic     # Phase 3: also run MCMC → foo.opt.bv
briev derive --iterations N   # MCMC iterations (default: 10000)
briev derive --temperature T  # MCMC initial temperature (default: 1.0)
briev derive --enumerative-depth N  # Max enumerative depth (default: 5)
briev derive --all            # Process all transitive imports
```

**CLI handler**:

```rust
/// 2026-07-28: Phase E.3 — `briev derive` command handler.
...
**Tests**:
- `test_derive_command_basic`: Run `briev derive foo.bv` → `foo.derive.bv` exists
...
### Step E.4 — `briev accept` subcommand (user-approved folding)

**File**: `src/main.rs`, `src/derive/accept.rs` (new)

**What**: An explicit user command that folds a doppelganger's synthesized bodies
back into the source `.bv` file. The compiler NEVER mutates source — `briev accept`
is an intentional user action after reviewing the generated `foo.derive.bv`.

```
briev accept <file>              # fold foo.derive.bv bodies into foo.bv
briev accept <file> --opt        # fold from foo.opt.bv instead (MCMC result)
briev accept <file> --all        # accept all derivation blocks in file
```
...
**Why separate from `briev derive`**: The derive step is automatic and lossless
...
**Relation to assertion mode**: After acceptance, `briev build foo.bv` runs in
...
```rust
/// 2026-07-28: Phase E.4 — `briev accept` command.
...
pub fn handle_accept_command(
    ...
    if !shadow_path.exists() {
        return Err(format!("no doppelganger found at '{}' — run 'briev derive' first", shadow_path.display()));
    }
...
- If no doppelganger exists, the error message tells the user to run `briev derive` first.

**Nesting check**: The handler validates inputs, finds the shadow, calls the
replacement helper, writes — one `?` chain, depth ≤ 2.

**Tests**:
- `test_accept_basic`: Run `briev accept foo.bv` after derive → `foo.bv` has bodies inlined
- `test_accept_opt`: Accept from `foo.opt.bv` → `foo.bv` has MCMC-optimized bodies
- `test_accept_no_shadow`: No `foo.derive.bv` → error message points to `briev derive`
...
5b. `briev derive` CLI with flags        (Step E.3)
5c. `briev accept` subcommand            (Step E.4)
```
...
rules are stored in a single `config/meta-vocab.dbv` file (Data Briev `.dbv` format
...
`dbriev::v2::parse_document_quoted`. A `MetadataRegistry` in `src/backend/metadata.rs`
...
### Step G.1 — Data Briev vocabulary file
...
and `dbriev::v2::parse_document_quoted()`. The `parse_document_quoted` variant is
...
2. `parse_document_quoted(&content).expect("...")` parses into `DbrievDocument`
...
## Phase I — CLI Completion (`briev derive` flags + `briev accept`)

**Goal**: Complete the phase E sub-steps that were deferred. Phase E committed
the doppelganger infrastructure and a bare `briev derive` command, but omitted
flag parsing (`--stochastic`, `--iterations`, etc.) and the `briev accept`
subcommand. Phase I finishes both.

### Step I.0 — `briev derive` CLI Flags

**File**: `src/derive/cli.rs`, `src/main.rs`

**What**: Add flag parsing to the `briev derive` command:

```
briev derive <file>                # enumerative + SMT → foo.derive.bv
briev derive --stochastic <file>   # also run MCMC → foo.opt.bv
briev derive --iterations N        # MCMC iterations (default: 10000)
briev derive --temperature T       # initial temperature (default: 1.0)
briev derive --enumerative-depth N # max depth (default: 5)
briev derive --all                 # process all transitive imports
```
...
### Step I.1 — `briev accept` Subcommand

**File**: `src/derive/accept.rs` (new), `src/main.rs`, `src/derive/mod.rs`

**What**: Add `briev accept <file>` that folds doppelganger bodies back into
the source `.bv` file. The compiler NEVER mutates source — `briev accept` is
...
- `test_accept_no_shadow`: no `.derive.bv` → error points to `briev derive`
...
| E | `docs/architecture/features/derivation-blocks.md` — doppelganger system + `briev accept` |
...
| I | `docs/architecture/features/derivation-blocks.md` — `briev accept` docs |
...
```
 ...
 9. Phase I: CLI completion (`briev derive` flags + `briev accept`)                   ✓ (cb02b43b+)
