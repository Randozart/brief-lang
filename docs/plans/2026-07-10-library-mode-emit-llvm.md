...
Add a `--library` flag to `briev build` that produces a reusable LLVM IR module
...
```
briev build --library bridge.bv --out ./out
  → out/bridge.ll   (library-mode LLVM IR — #export wrappers + __briev_init_state)
```

Combined with:
```
briev export bridge.bv rust --out ./out
...
- `briev build --llvm` already produces a `.ll` file and exits before linking
...
- `emit_library_shim()` exists in `src/backend/llvm/emit_toplevel.rs:1971` —
  emits `__briev_init_state()` (allocates `%State`, returns ptr) and
...
6. **init_state**: `define dso_local i64 @__briev_init_state()` — allocates
...
- Manual: `briev build --library examples/glue-macro.bv --out /tmp/glue-test`
  → verifies `.ll` contains `@__briev_init_state` and no `@main`
- Manual: `briev build --library examples/glue-macro.bv --llvm` → should be
...
- **`bridge-exports.dbvl` auto-generation** as part of `--library` — currently
  handled separately by `briev export`
