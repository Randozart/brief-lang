...
(written inline in Brief source).
...
| **User** | `$(Stage @ N) { ... }` inline in `.bv` files | Any Brief programmer | Declared in source file | `@ normal` (500) default |
...
3. Compiler installation path: `<executable_dir>/../share/brief/plugins/`
...
brief-compiler build --disable-plugin prelude main.bv

# Disable all system plugins
brief-compiler build --disable-plugin '*' main.bv

# Load an additional user plugin from a path
brief-compiler build --enable-plugin ./my-plugin.bv main.bv
```
...
The body of a `$(Stage)` block is **regular Brief code** run at compile time.
The compiler:
...
All other Brief constructs (`let`, `if`, `defn`, type constructors, etc.) work
...
```
Asserts a structural property of the AST. The `condition` is a Brief
...
```bash
brief-compiler register my-project/utils /home/user/projects/utils/utils.bv
brief-compiler unregister my-project/utils
brief-compiler list-registry   # lists all registered modules
```
...
config at `~/.config/brief/registry.toml` with `--global`.
...
test_register_cli_command:
  // brief-compiler register name path → adds entry to registry
  // Subsequent import <name> resolves to that path

test_unregister_cli_command:
  // brief-compiler unregister name → removes entry
...
test_compile_with_prelude_plugin:
  // Compile a minimal Brief file with system plugins enabled
  // Verify prelude imports are present in the resolved AST

test_compile_with_disable_plugin:
  // brief-compiler build --disable-plugin prelude minimal.bv
