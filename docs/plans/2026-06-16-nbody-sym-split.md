...
For nbody_sqrt, the Briev version computes energy every iteration (10 extra `sqrt#` calls
...
1. Briev outputs an extra periodic line at count=0 that C doesn't have
...
This makes the Briev compute energy exactly once, at convergence — matching C's
...
Creates a variant where both Briev and C compute energy every iteration.
...
- ADD in-loop energy computation + fprintf every 5M matching the Briev
...
for (count = 0; count < total; count++) {
    ...
    // ENERGY COMPUTATION every tick (new — matches Briev idiomatic)
    float energy = 0.0f;
    #define EPAIR(ia, ib) { \
...
3. Build + run each changed benchmark:
   ```bash
   cargo build --release --bin briev-compiler
   ./target/release/briev-compiler llvm benchmarks/knucleotide.bv --out /tmp
