...
got Briev callable from C, Python, and Rust, with callbacks and host cancellation.
...
of — the mechanism Briev was designed around.

**Briev has no type layouts.** A type is `(protocol, metadata)`, nothing else.
...
3. **Casting emits the delta, not the chain.** The casting graph resolves the
   *chain* (BFS finds the minimal number of hops); codegen emits the *delta* —
   the minimal incremental operations. Briev adopts whatever operations are most
...
2. **Single binding → one call.** `CStr → String` emits exactly one call
   (`cstr_to_briev`); `String → CStr` emits `str_to_c`. The delta transform,
...
`[len][data]`, which is wrong). `briev_cstring_concat` was added to the
...
`cstr_to_briev`/`str_to_c` bindings, and the boundary types `CStr`/`CFloat`/
...
**Marshalling: DONE.** `name as String` for a CStr param emits `cstr_to_briev`
(and `s as CStr` emits `str_to_c`) — the graph-resolved binding calls. Briev's
...
`greet` marshals a C string through `briev_cstr_to_briev`/`briev_str_to_c`,
...
proto C_String: #String {
    CastTo(#String<UTF8>) = cstr_to_briev(#L);   // nul-terminated → [len][data]
...
The `cstr_to_briev`/`str_to_c` frgns to `lib/runtime/briev_rt.c` are declared
here (they already exist in the runtime). Verify:
- `find_path(CStr → String)` = one `cstr_to_briev` call.
...
- Body marshalling is ordinary Briev casts (`(String)name`) → the
...
- `pp-types.bv`, `examples/glue-host/rank.bv`, `callback.bv` migrate to boundary
  signatures (`CStr`, `CDouble`); the manual `cstr_to_briev`/`str_to_c` calls
...
- **P3 (export boundary):** DONE — `resolve_protocol` uses a type→protocol
  ...
  `CStr ⇄ String` casts into the graph's binding calls (`cstr_to_briev`/
  `str_to_c`).
