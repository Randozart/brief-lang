...
**Branch:** `feat/out-observability` (worktree `../briev-compiler-out`)
**Related:**
...
Prove Briev can hold its own as a **systems language without dependencies**: every
C-expressible piece of the runtime becomes Briev-native. C shrinks to the
...
1. **The `out` keyword** — the stdlib-side observability pin. It lets stdlib
   ...
   permits formatting/logic to move out of the C runtime and into Briev without
   being dead-code-eliminated.
2. **The native-stdlib migration** — moving the pure-logic half of
   `lib/runtime/briev_rt.c` (digit→string, string byte-ops, `str_to_int`, file
   read/write) into Briev, and creating the first real `.ebv` freestanding
   target.
...
Briev's compiler **always maximizes optimization** — even if that means folding a
...
- **NOT `out export`** — whatever `export` returns IS the output boundary for the
  ...
  also called from Briev, there are two versions: the exported one (ABI-pinned)
  and a Briev twin that may be optimized into other bodies.
...
Briev and need `out` to survive DCE.
...
- Fix `std/string.bv` `to_string` to use the casting path (or a Briev digit
  loop — see Phase 4).
- **Creates the `.ebv` seam:** the lane *implementation* becomes swappable —
  `.bv` uses `ExtCall("int_to_str")` (C), `.ebv` uses a Briev digit loop.
...
**Goal:** prove Briev can hold its own as a systems language without C — a real
`.ebv` freestanding target with a **heap** (static bump arena, no `@malloc`),
String/Data support, and the string-conversion stdlib implemented in Briev.
...
- The documented `.ebv` vision REQUIRES a heap and Strings:
  - `docs/plans/2026-08-03-data-briev-config-and-board-hardware-map.md:81` —
    `.ebv` is explicitly `Malloc#`→ bump allocator, `Print#`→ pure Briev
...
`.cbv`. Briev should PROVIDE an allocator on bare metal, not forbid Strings.
...
- **Latent link-error bug found+fixed** (`70f596f9`): the casting graph declares
  10 `ExtCall` string-lane symbols; only `int_to_str` existed in `briev_rt.c`.
...
- `lib/std/conversions.ebv` provides the cast-lane symbols (`int_to_str`,
  ...
  `str_to_bool`, `bool_to_str`, `str_first_char`, `char_to_str`) as **Briev
...
- The `prelude` plugin (`plugins/parsed/prelude.bv`) imports `std/os/*`,
  `std/io.bv`, `std/env.bv` — these transitively import `std/briev_rt.bv`
  which does `import "link/briev_rt.c"`, pulling `briev_rt.o` into EVERY build
...
- For a true freestanding `.ebv`, the prelude must be a **C-free variant**
  ...
  `.ebv` build must skip `briev_rt.c` in `collect_extra_objects`. This is a
...
compiles, links, and runs with `briev_rt.c` NOT linked (freestanding).
...
- **Formatting moves to stdlib** — the Phase 4 digit loops (`int_to_str`,
  `float_to_str`). The `.ebv` print path is pure Briev formatting + a raw
  `write` syscall.
...
`../briev-compiler-glue-host`) is landing `2026-08-03-glue-folders-node-bridge.md`.
...
4. First real `.ebv` stdlib exists; `is_embedded_extension` wired; `briev_rt.c`
