# Full System Analysis: SLP, SROA, and Attribute-Gated Optimization in the Briev Compiler
...
The Briev compiler has 19 benchmarks whose all-time-best performance is spread across six
...
| Benchmark | Best Ratio | Briev Time | C Time | Era | Commit | Configuration |
|-----------|-----------|------------|--------|-----|--------|--------------|
...
| float_math_nonzero | 0.1656s | 0.1675s | **0.98x** | Briev |
| sparse_dispatch | 0.0500s | 0.0610s | **0.81x** | Briev |
...
| fasta | 0.2099s | 0.2109s | **0.99x** | Briev |
| fannkuch_redux | 0.0653s | 0.0657s | **0.99x** | Briev |
| mandelbrot | 0.6569s | 0.6528s | 1.00x | ~tie |
| kalman_filter_runtime | 0.1813s | 0.1790s | 1.01x | C |
| knucleotide | 0.1883s | 0.1909s | **0.98x** | Briev |
| cancel_math | 0.0626s | 0.0614s | 1.01x | C |
| bit_clear | 0.0001s | 0.0002s | **0.50x** | Briev |
...
| float_math_nonzero | 0.1611s | 0.1620s | **0.99x** | Briev |
| sparse_dispatch | 0.0551s | 0.0604s | **0.91x** | Briev |
| print_loop | 0.0568s | 0.0559s | 1.01x | C |
| nbody_newton | 10.6217s | 7.8560s | 1.35x | C |
| nbody_sqrt | 2.2434s | 2.6339s | **0.85x** | Briev |
| nbody_sqrt_idio | 2.3270s | 3.4561s | **0.67x** | Briev |
| fasta | 0.1987s | 0.1980s | 1.00x | ~tie |
| fannkuch_redux | 0.0599s | 0.0612s | **0.97x** | Briev |
| mandelbrot | 0.6317s | 0.6277s | 1.00x | ~tie |
| kalman_filter_runtime | 0.1741s | 0.1725s | 1.00x | ~tie |
| queue_drain | 0.0601s | 0.0612s | **0.98x** | Briev |
| queue_drain_sym | 0.0575s | 0.0588s | **0.97x** | Briev |
...
| Benchmark | Briev | C | Ratio | Winner |
|-----------|-------|---|-------|--------|
| ring_buffer | 0.0664s | 0.0666s | **0.99x** | Briev |
| float_math | 0.0626s | 0.0739s | **0.84x** | Briev |
...
|---|---------|-----------|---------|
| 0 | `init_state`, `__briev_init_state` | `nofree norecurse nosync nounwind memory(argmem: write)` | State initializer — write-only |
| 1 | `__briev_member_fn` | `nofree nosync nounwind` | Member function (may read/write heap) |
...
has no FFI — only pure Briev operations on %State. So `argmem:readwrite` is accurate.
...
numbering scheme that caused a DIFFERENT feature (#1 on `__briev_member_fn`) to be applied
to reactor_tick.
