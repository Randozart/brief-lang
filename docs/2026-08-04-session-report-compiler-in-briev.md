# Session Report — Compiler-in-Briev (2026-08-04)

**Scope:** the `compiler-in-briev` branch (worktree `../briev-compiler-dogfood`),
P1–P5, merged into main here. Two compiler passes are now written in Briev,
compiled by `brievc` itself, and loaded through the GLUE C ABI at runtime — the
...
pass rewritten in Briev, compiled to a native library by `brievc`, and linked
in at runtime via dlopen** — the same way a host language calls a Briev bridge.
...
| Phase | Commit | What |
|-------|--------|------|
| Plan | `ea6c4687` | `docs/plans/2026-08-04-compiler-in-briev-dogfood-ffi.md` |
...
| P3 | `ede34733` | root `build.rs` + `briev_pass.rs` dlopen loader (Option 1: runtime dlopen) |
| P4 | `ede34733` | both production call sites route through `briev_pass::compute_export_needs_state` |
| P5 | `a23df1ab` | `soa_reorder.bv` (second pass) + `reader.bv` (shared scanner) + recipe doc |

The pattern: Rust serializes a **tagged Data Briev projection**; Briev decides;
build.rs compiles the pass with a prebuilt brievc; briev_pass.rs dlopens it and
...
first build has no brievc yet (self-hosted bootstrap) and every pass falls back
to its Rust reference. Transition tests assert Briev == Rust on a corpus.

## 3. Verified Briev language facts that shaped the passes
...
- **Dynamic String slices return the whole array** (emit_expr.rs:992); **String
  ...
  generic `T`**. The passes therefore use `briev_str_substr` + `char_at` frgns
...
8. **Dynamic String slicing had no runtime implementation** — added
   `briev_str_substr` (substring) and `briev_str_char_at` (Int byte, no
   allocation) to briev_rt.c. (`ef5c476f`)
...
the state handle first; the `briev bindings` header is the source of truth.

## 6. Verification

- `needs_state_compute` == `compute_export_needs_state` on boundary (0),
  node_bridge (31), cancel (1), rank (2), bench (2) — asserted by
  `tests/c_driver_needs_state.rs` and `briev_pass.rs`'s unit test (which also
  asserts the dlopen path was USED, not the fallback).
- `soa_reorder_compute` == `reorder_fields` on `nbody_newton.bv` — asserted by
  `soa_reorder.rs::briev_reorder_matches_reference_on_nbody` and
  `briev_pass.rs::soa_pass_matches_reorder_fields`.
...
- Dynamic String slices still return the whole array (worked around via the
  `briev_str_substr` frgn); a real fix would make slices construct substrings.
...
- string.bv does not type-check (legacy bodies: `StrBytes#`, `List+`, method
  calls on String) — surfaced by the error-not-swallowed fix; the in-Briev
...
`docs/architecture/compiler-in-briev.md` is the how-to: the model, a 6-step
