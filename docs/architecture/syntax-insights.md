# Briev Syntax Insights

## Termination: `term`, not `return`

Briev does NOT have a `return` keyword. The correct way to produce a value
...
but is NOT a parseable Briev keyword. It's an internal IR construct used by
...
Briev is not Rust. Expressions in a `defn` body are NOT implicit return values.
...
Briev does NOT have `if` expressions. `Expr::If(cond, then, else_)` exists
...
The correct Briev pattern for conditional execution is `when`:
...
2. `when` guards are the idiomatic Briev conditional pattern
