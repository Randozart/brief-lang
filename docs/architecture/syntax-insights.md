# Brief Syntax Insights

## Termination: `term`, not `return`

Brief does NOT have a `return` keyword. The correct way to produce a value
...
but is NOT a parseable Brief keyword. It's an internal IR construct used by
...
Brief is not Rust. Expressions in a `defn` body are NOT implicit return values.
...
Brief does NOT have `if` expressions. `Expr::If(cond, then, else_)` exists
...
The correct Brief pattern for conditional execution is `when`:
...
2. `when` guards are the idiomatic Brief conditional pattern
