...
C functions that serve as compiler-internal implementations of Brief
intrinsics follow the `__name__` naming convention (double underscore
both sides). This distinguishes them from:

| Convention | Meaning | Example |
|---|---|---|
| `name` | Brief intrinsic call syntax | `print#(msg)` |
...
| `brief_name` | **DEPRECATED** — old intrinsic C convention | `brief_read_file(...)` |

## Rules

1. Every `__name__` C function corresponds to a Brief intrinsic callable
   via `name#(args)` syntax.
2. String parameters use `const char*` — the LLVM intrinsic handler loads
   `hdr[0]` (the data pointer) from the Brief string struct before calling.
...
4. Return values are `int64_t` (a Brief-packed value: pointer for strings,
