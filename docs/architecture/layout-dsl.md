...
The Layout DSL is a declarative binary grammar for describing the physical arrangement of bits in any Briev type. It replaces the old opaque `codec` concept with a formal pattern language that the compiler can reason about, validate, transform, and prove.
