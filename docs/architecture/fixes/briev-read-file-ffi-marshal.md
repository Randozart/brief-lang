# Fix: briev_read_file FFI marshaling mismatch (C string vs Briev header)

**Date**: 2026-06-14
**Author**: OpenCode (diagnosis from officina-cli agent)
**Bug**: briev_read_file crashes with corrupt malloc call (size = 0x6E6174737265646F)

## Root Cause

The LLVM backend marshals `String` as `i8*` (C string pointer) when crossing the
FFI boundary. This is documented at `briev_rt.c:376`:
...
However, `briev_read_file` was written to accept a **Briev internal header pointer**
(the old format: `[data_ptr, len, chars...]` stored as `int64_t[]`):

```c
// Old (broken) convention:
int64_t briev_read_file(int64_t path_ptr) {
...
### 1. `lib/runtime/briev_rt.c` — Rewrite `briev_read_file` to C string convention

```c
char* briev_read_file(const char* path) {
    if (!path) return NULL;
...
%raw = call i64 @briev_read_file(i8* %fp)
%result = ptrtoint i8* %raw to i64   ; back to i64 for SSA
```

### 3. `src/backend/llvm/emit_toplevel.rs` — Update declaration

Changed `declare i64 @briev_read_file(i64)` to `declare ptr @briev_read_file(ptr)`
...
After fix, `read_file#("system/understands.dbv")` in a briev program compiled with
