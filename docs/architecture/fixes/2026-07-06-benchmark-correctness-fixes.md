...
`bit_clear` produces output `0` but C reference produces `1`. The Briev
...
the printed values are 5M, 10M, 15M...  Briev's guard fires BEFORE the
...
- nbody_sqrt: non-deterministic → 0.74x (Briev wins, deterministic)
