# The GLUE FFI Architecture — Brief, the central language
...
`learn-brief/07-ffi.md` (tutorial),
...
Brief compiles to a native library that any host language calls at **native
speed**. The FFI is:
...
- **Config-driven.** Every language is a folder `lib/glue/<lang>/` — a `glue.dbvl`
  ...
  `brief bindings|export|extension <bridge> <lang>` resolves `lib/glue/<lang>/`
...
**Mental model:** a host calling `brief.feature_hash(...)` should feel like
...
```bash
briefc build examples/glue-host/bench.bv --library --out build/
...
| Function | Purpose |
|----------|---------|
| `BriefState* __brief_init_state(void)` | Allocate + init the process-global state; returns its address |
| `void __glue_release(BriefState* state)` | Release (no-op today; arena is process-lifetime) |
| `void __brief_set_cancel(BriefState*, int32_t)` | Raise the process-global cancel flag |
| `void __brief_clear_cancel(BriefState*)` | Clear it |

`__brief_init_state` returns a **module-global** state (not a stack address —
...
|---------|---------|----------------|
| `brief bindings <bridge> <lang>` | Declarative bindings | C header, C# P/Invoke class |
| `brief export <bridge> <lang>` | A language package/wrapper | Go package, Java class, Rust crate |
| `brief extension <bridge> <lang>` | A **native extension** (compiled shim) | Python `.so`, Node `.node`, Java JNI `lib*.so`, Lua C module |

`brief extension` builds the bridge library, renders the shim from the
...
- Every export is declared in the shim as `__brief_export_<name>` with an
...
### 4.1 `frgn` — call a foreign function from Brief
...
The first identifier is the **C/runtime symbol**; `as` gives the Brief name used
...
- **`bindings.*`** — declarative files rendered by `brief bindings`
  (C header, C# class). Whole-file templates use `{{ffi_decls}}`
  (from `bindings.ffi_template`).
- **`templates.*`** — package files rendered by `brief export`, with
  `{{exports}}` (from `fn_template`) for per-export wrappers.
- **`native.*`** — the extension shim rendered by `brief extension`:
...
toolchain discovery pattern: PATH first, then `~/brief-tools/<tool>-*`.
...
|----------|--------|---------|-----------------|
| C | bindings | `brief bindings <b> c` | `c_driver_boundary` |
| C++ | bindings | same C header (`extern "C"`) | `c_driver_cpp` |
| Rust | bindings | `brief export <b> rust` | `rust-host` crate |
| Python | native ext | `brief extension <b> python` | `c_driver_python` |
| Node | native ext | `brief extension <b> node` | `c_driver_node` |
| Go | cgo package | `brief export <b> go` | `c_driver_go` |
| Java | native ext (JNI) | `brief extension <b> java` + `brief export <b> java` | `c_driver_java` |
| Lua | native ext (C module) | `brief extension <b> lua` | `c_driver_lua` |
| C# | bindings (P/Invoke) | `brief bindings <b> csharp` | `c_driver_csharp` |
...
| Lua | 1162 | 12309 | **0.09** | Brief 11× |
| Python | 1179 | 229794 | **0.01** | Brief 195× |
| Node | 1282 | 190498 | **0.01** | Brief 149× |
...
**Dispatch** (Brief `add` vs the host's pure-internal `add`): C 1.07, C++ 1.09,
...
whose toolchain is present — **Gate A** (real work: Brief vs native
`feature_hash`) and **Gate B** (dispatch: Brief vs native internal `add`), 3
...
The gate asserts (generous canaries): every present host's Brief `feature_hash`
