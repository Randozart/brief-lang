...
```
briefc build --shared component.bv --out build/
  → component.ll (LLVM IR)
  → clang -O3 -shared -fPIC component.ll lib/runtime/brief_rt.c -o component.so -lm
  → component.so
```
...
4. Emits `__brief_init` / `__brief_fini` via `llvm.global_ctors`
5. Links `brief_rt.c` for runtime functions (malloc, syscall wrappers, etc.)
...
it (minimum 8 bytes, zero-initialized) or use `__brief_init_state` to
