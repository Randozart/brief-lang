# Accelerated Briev (`.abv`) — Native GPU Compilation

**Date added:** 2026-06-18 (renamed from Graphic Briev 2026-06-19)  
**Phase:** 1 (StrictMode, type validation, SPIR-V output)  
**Status:** Active — canonical GPU file format  
**Also known as:** "Briev Accel"

---

## Purpose

Accelerated Briev is a dedicated file extension that always compiles to GPU
...
The compilation is triggered automatically by `briev-compiler build file.abv`.
...
|-------------|----------------|
| `briev-compiler build file.abv` | ✅ Auto-detected, GPU path |
| `briev-compiler llvm file.abv` | ✅ Auto-detected (via `is_gpu_extension`) |
| `briev-compiler check file.abv` | ✅ Auto-detected (via `is_gpu_extension`) |

## CLI

```text
$ briev-compiler build file.abv
Building Accelerated Briev (.abv) file: compiling via GPU...

$ briev-compiler llvm file.abv --gpu-backend opencl
