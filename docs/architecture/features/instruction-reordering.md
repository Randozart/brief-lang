...
Modern CPUs extract ILP via out-of-order execution, but the window is narrow (~352 µops on Zen 4, ~512 on Golden Cove). Brief has more information than LLVM — contracts expose precise read/write sets — so reordering at the Brief IR level gives LLVM better material to schedule.
