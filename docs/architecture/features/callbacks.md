...
of status from Brief code." C#-events/delegates-style.

## What was built

A host passes a function pointer into an exported Brief function; Brief calls
...
5. **Test:** `tests/c_driver_callback.rs` (toolchain-guarded) — C passes
   `doubler`/`plus_one`, Brief calls them, returns 42/42.

## Status / follow-ups

- C path verified end-to-end (`briefc build --library` + C driver).
