# Host Cancellation — cancel a long-running Briev call
...
A host thread raises a process-global atomic flag; a long-running Briev loop
...
Host side (C):
```c
__briev_set_cancel(st, 1);   // from another thread
int64_t r = cancellable_sum(st, 2000000000);  // returns early
__briev_clear_cancel(st);
```

## Pieces

1. **Intrinsics:** `CancelRequested#() -> Bool` loads `@__briev_cancel_flag`
   (seq_cst); `ClearCancel#()` stores 0. Interpreter returns `false` (no
   host in-process).
2. **Shim exports:** `__briev_set_cancel(ptr %state, i32)` /
   `__briev_clear_cancel(ptr %state)` in `emit_library_shim`; declared in the
   C bindings header.
...
- **Process-global flag** (`@__briev_cancel_flag`): one Briev instance per
...
- Remove the `CancelRequested#`/`ClearCancel#` signatures + intrinsics arms,
  `@__briev_cancel_flag` global, and the shim `__briev_set_cancel`/
  `__briev_clear_cancel` functions.
