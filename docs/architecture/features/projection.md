...
The `:>` operator (`expr :> Target(args)`) extracts compile-time metadata or applies a type-defined projection. It is the primary mechanism for type-directed operations in Brief.
