# Accelerated Brief (`.abv`) — Native GPU Compilation

**Date added:** 2026-06-18 (renamed from Graphic Brief 2026-06-19)  
**Phase:** 1 (StrictMode, type validation, SPIR-V output)  
**Status:** Active — canonical GPU file format  
**Also known as:** "Brief Accel"

---

## Purpose

Accelerated Brief is a dedicated file extension that always compiles to GPU
...
The compilation is triggered automatically by `brief-compiler build file.abv`.
...
|-------------|----------------|
| `brief-compiler build file.abv` | ✅ Auto-detected, GPU path |
| `brief-compiler llvm file.abv` | ✅ Auto-detected (via `is_gpu_extension`) |
| `brief-compiler check file.abv` | ✅ Auto-detected (via `is_gpu_extension`) |

## CLI

```text
$ brief-compiler build file.abv
Building Accelerated Brief (.abv) file: compiling via GPU...

$ brief-compiler llvm file.abv --gpu-backend opencl
