...
Every `brief build` re-verifies the synthesized body against the examples
...
The developer reviews the output, then runs `brief accept` to fold the
...
- `brief derive --stochastic <file>` — also run MCMC superoptimizer
- `brief accept <file>` — fold `.derive.bv` bodies back into source
- `brief build <file>` — compile with assertion verification
