# Graphic Brief (`.gbv`) — Native GPU Compilation
...
Graphic Brief is a dedicated file extension that always compiles to GPU
(SPIR-V). Unlike `.bv` with `#gpu` or `--gpu-offload`, `.gbv` is implicit —
no flags or directives needed. It enforces type/intrinsic restrictions at
compile time so every valid `.gbv` file is guaranteed to produce valid SPIR-V.

## Syntax

`.gbv` uses the same syntax as `.bv`. Key differences are enforced by the
...
```
file.gbv → parser (StrictMode::Gpu) → import resolver →
           typechecker (GPU validation) → LlvmBackend.generate() →
           CPU IR + embedded SPIR-V blob + standalone .spv file
```

The compilation is triggered automatically by `brief-compiler build file.gbv`.
The `is_gpu_extension()` check in `main.rs` detects `.gbv` and:
...
| Entry point | `.gbv` support |
|-------------|----------------|
| `brief-compiler build file.gbv` | ✅ Auto-detected, GPU path |
| `brief-compiler llvm file.gbv` | ✅ Auto-detected (via `is_gpu_extension`) |
| `brief-compiler check file.gbv` | ✅ Auto-detected (via `is_gpu_extension`) |

## CLI

```text
$ brief-compiler build file.gbv
Building Graphic Brief (.gbv) file: compiling via GPU...

$ brief-compiler llvm file.gbv --gpu-backend opencl
Compiling to LLVM IR: file.gbv (GPU — .gbv)
```
...
| `src/main.rs` | `is_gpu_extension()`, `run_build` `.gbv` arm |
