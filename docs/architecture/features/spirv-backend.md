...
`src/analysis/gpu_cost.rs`. The runtime C library is `lib/runtime/briev_gpu_rt.c`.
...
```
Briev source → codegen (CPU IR) → collect_gpu_kernel():
  ...
  6. SPIR-V blob embedded as @briev_kernel_N byte array in .rodata
  7. At runtime: briev_gpu_launch() via Vulkan or OpenCL
```
...
| Briev expression | Integer (i64) | Float (f32) |
|-----------------|---------------|-------------|
...
| Briev intrinsic | SPIR-V LLVM IR |
|----------------|----------------|
...
The runtime's `briev_gpu_launch` accepts `grid_y` and `grid_z` parameters,
