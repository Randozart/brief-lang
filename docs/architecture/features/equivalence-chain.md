...
| **Briev function** | `defn fn ... { ... }` | Interpreter | No |
...
(Briev functions, examples) have no target — they are available on any
target.
...
|-----------|-------------------|
| **Ref (Briev defn)** | Briev interpreter via `eval_expr` + `eval_block`. Handles `let` bindings, `term` returns, and `TermReturn` early-return pattern. Params bound by position. |
...
- Body is evaluated via the Briev interpreter
- Let bindings, contracts, all Briev features supported
...
| **Inline asm** | Target-annotated bodies in the chain. Cross-verified against a Briev reference at compile time. |
