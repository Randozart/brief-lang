...
types. The body uses concrete LLVM types (all Briev values are `i64` at
...
The body uses raw LLVM IR with Briev-flavored syntax.
...
The fallback is a single Briev expression or block. It provides the reference
...
- User `defn main` is renamed to `briev_main` to avoid collision with `define i32 @main()`
...
| `src/backend/llvm/emit_toplevel.rs` | `emit_inop()`, `term`→`ret` lowering, `briev_main` rename |
