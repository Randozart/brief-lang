...
```
 ...
 defn add_i64     ┌──────────────┐       ┌──────────────┐       ┌──────────────┐
   a: Int         │  Frontend    │       │  .dbvl       │       │  brief-llvm  │
...
| No prefix | All backends (standard Brief) | `bytes`, `alignment` |

Backends MUST NOT read keys with other backends' prefixes. For example,
`brief-llvm` must never read `circt_op`, and `brief-circt` must never
...
| Metadata key is not a Brief reserved word | ✅ Yes | Parser: reserved word check |
...
### 4.2 LLVM Backend Responsibilities (`brief-llvm`)
...
```brief
// A single Brief function that compiles to three different targets.
...
| `brief-llvm` | `"rdtsc"` is valid x86 instruction; registers exist | `circt_op`, `wasm_op`, `interpreter_impl` |
| `brief-circt` | `"hw.cycle_count"` is valid CIRCT op | `llvm_asm`, `wasm_op` |
| `brief-webstack` | `"i64.const 0"` is valid WASM | `llvm_asm`, `circt_op` |
...
A developer writing a custom backend (e.g., `brief-spirv` for Vulkan
...
brief compile src/main.bv --archive build/main.dbvl

# 2. Run LLVM backend — validates llvm_* keys
brief-llvm build/main.dbvl --output build/a.out

# 3. Run CIRCT backend — validates circt_*, hls_* keys (same archive)
brief-circt build/main.dbvl --output build/design.v

# 4. Run custom compliance checker — validates pii_*, gdpr_* keys
brief-compliance build/main.dbvl --output build/compliance-report.json
```
