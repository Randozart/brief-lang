...
1. **`brief link <library>`** — Analyzes a foreign `.so`/`.a` via `nm`, extracts T (text) symbols, cross-references each against the compiler's `Intrinsic` enum. Known intrinsics become `intrinsic_call#()` wrappers; unknowns become `frgn` skeletons.

2. **`brief export <bridge.bv> <language>`** — Parses the bridge, extracts `#export`/`frgn`/`meld` declarations, serializes them as D-Brief Lines (bare comma-separated), looks up the language adapter in `glue.dbvl`, invokes the adapter's `$!macro`, which calls `emit_file#()` to write wrapper source files.
