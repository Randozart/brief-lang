...
Metropolitan FFI is Briv's integrated memory negotiation system that allows Briv to negotiate shared memory positions with foreign languages (C, Rust, Python, etc.).
...
- **Request region** (Briv → Foreign)
- **Response region** (Foreign → Briv)
...
```rust
use briv_compiler::ffi::metropolitan::MetropolitanHub;
...
```c
// Generated by Briv Compiler - ml_inference.h
#include <stdint.h>
#include <stdatomic.h>
...
```rust
// Generated by Briv Compiler - ml_inference.rs
...
```python
# Generated by Briv Compiler - ml_inference.py
import mmap
import ctypes
import time
...
### Traditional FFI
```
Briv → Marshal → Syscall → Foreign → Marshal → Syscall → Briv
...
### Metropolitan FFI
```
Setup: Briv ↔ Negotiate ↔ Foreign  (one-time, O(1))

Per call:
Briv → Write → Signal → Foreign → Process → Signal → Read → Briv
