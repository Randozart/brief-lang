...
Metropolitan FFI is Briev's integrated memory negotiation system that allows Briev to negotiate shared memory positions with foreign languages (C, Rust, Python, etc.).
...
- **Request region** (Briev → Foreign)
- **Response region** (Foreign → Briev)
...
```rust
use briev_compiler::ffi::metropolitan::MetropolitanHub;
...
```c
// Generated by Briev Compiler - ml_inference.h
#include <stdint.h>
#include <stdatomic.h>
...
```rust
// Generated by Briev Compiler - ml_inference.rs
...
```python
# Generated by Briev Compiler - ml_inference.py
import mmap
import ctypes
import time
...
### Traditional FFI
```
Briev → Marshal → Syscall → Foreign → Marshal → Syscall → Briev
...
### Metropolitan FFI
```
Setup: Briev ↔ Negotiate ↔ Foreign  (one-time, O(1))

Per call:
Briev → Write → Signal → Foreign → Process → Signal → Read → Briev
