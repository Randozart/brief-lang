...
- **CLI argv capture** (3a): every loop-engine `main` is
  `main(i32 %argc, ptr %argv)` storing into `@__briev_argc`/`@__briev_argv`;
...
- **Flat scripting** (4): `defn main()` and bare top-level lets run exactly
  once via a synthesized `node __script_main` (fixes the dead `briev_main`).
...
- **Content equality + bitwise** (B1): `Eq`/`Ne` compare payload bytes via
  `briev_str_eq`; `& | ^ ~` operate on content via
  `briev_str_band/bor/bxor/bnot`.
- **Content view + encoding door** (B2): `#String → #Bit` = buffer address
  (`ptrtoint`); `#Bit → #String` = UTF8 wrap via `briev_bits_to_str`
  (header materialized by construction); `CastFrom(#Bit)` overrides.
- **Length ops** (B3): `x.^Len` = UTF8 char count (`briev_char_len`);
...
- **`!>` metadata syntax**: the `<~` metadata form was removed (writing `<~`
  ...
  briev, and architecture docs updated.
...
2. **SSO tag bit corrupted String addresses** (B1): OR-1 static tag on literal
   stores made `briev_str_eq` read a misaligned header.
...
7. **`briev_main` dead code** (Phase 4): `defn main` was emitted but never
