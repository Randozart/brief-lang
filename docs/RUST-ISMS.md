# Rust-isms to Avoid in Brief

This document tracks patterns from Rust that accidentally leaked into Brief code but violate Brief's philosophy.
...
### Why This Violates Brief Philosophy

1. **Contracts, Not Crashes**: Brief uses contracts to guarantee safety at compile-time, not runtime checks
2. **No `Never` Type**: Brief has no `!` (diverges) type - every function must return
...
defn compile_file(path: String) [path.len() > 0][result.is_ok() || result.is_err()] -> Result<String, String>
```
