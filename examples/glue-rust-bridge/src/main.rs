...
//
// Calls Briev-exported functions through the GLUE protocol.
// The bridge object is compiled by build.rs and linked statically.
//
// Each Briev export compiles to an LLVM function with signature:
//   i64 @<name>(ptr %state, i64 %arg0, i64 %arg1, ...)
//
// The first argument is the Briev state pointer (opaque).
// Call __briev_init_state() once before any export to initialize.
...
extern "C" {
    fn __briev_init_state() -> *mut std::ffi::c_void;
...
fn main() {
    ...
    let state = unsafe { __briev_init_state() };
    println!("  Briev runtime initialized (state=0x{:x})", state as usize);
