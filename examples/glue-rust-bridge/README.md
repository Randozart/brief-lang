...
Call Briv-exported functions from Rust via the GLUE protocol.
...
4. `main.rs` declares `extern "C"` functions matching the LLVM signatures,
   calls `__briv_init_state()` once, then calls exports directly
...
extern "C" {
    fn __briv_init_state() -> *mut c_void;  // returns state ptr
