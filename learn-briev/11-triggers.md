...
In Briev, triggers (`trg`) represent **external events** that can change at any time. They are the bridge between your verified state machine and the unpredictable outside world.
...
```briev
// Hardware trigger (Embedded Briev) — whole-target form
...
- **Runtime**: `runtime/briev_rt.c` provides `volatile char __io_pending`, `volatile int64_t __timer_1hz`, etc.

The `@ link` mechanism is **zero-magic**: the compiler knows nothing about the symbol name. It emits whatever name you provide. The runtime/OS provides the definition. This lets any C/assembly symbol be used as a Briev trigger.

**Supported trigger types** for `@ link`:
| Briev Type | Runtime C Type | LLVM IR Type |
|-----------|----------------|-------------|
...
Briev accepts multiple forms for trigger declarations:
...
- **Briev**: `trg!` warned "async rollback risk here"
...
The Briev compiler treats trigger variables as **volatile**:
...
Briev's reactor uses a **two-tier execution model** to avoid wasted FFI side effects:
...
Briev provides standard system triggers in `lib/std/system.bv`. These use `@ link` bindings to runtime symbols:
...
// Compile:     briev build --llvm program.bv
// Produces program.ll + briev_rt.c, then links to binary.
// Link:        ld program.o briev_rt.o -o program
```
...
The LLVM backend is the primary compilation target for event-driven Briev programs.

### Compile and Link

```bash
# Build (produces .ll + links with briev_rt)
briev build --llvm program.bv --out output/

# Or manually:
briev build --llvm program.bv --out output/
llc output/program.ll -filetype=obj -o output/program.o
cc -c runtime/briev_rt.c -o output/briev_rt.o
ld output/program.o output/briev_rt.o -o program
```

The `build --llvm` writes the embedded `briev_rt.c` source to the output directory, compiles it with `cc`, and prints the final `ld` command.

### Runtime Source

`runtime/briev_rt.c` is a single C file that provides:
- **`@ link` global definitions**: `volatile` variables in section `briev_trg` for signal handlers, timers, stdin
...
|----------|-----------|--------|
| Linux | `epoll` + `signal` | `runtime/briev_rt.c` |
| macOS/BSD | `kqueue` + `signal` | `runtime/briev_rt.c` |
| ARM bare-metal | `WFI` instruction | `runtime/briev_rt.c` |
| x86 bare-metal | `STI; HLT` | `runtime/briev_rt.c` |
| WASM | `memory.grow` yield | `runtime/briev_rt.c` |
| Fallback | `nanosleep` (1ms) | `runtime/briev_rt.c` |
