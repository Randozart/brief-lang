...
> **Operator quick reference:** Briev's operators fall into three groups
...
once via the same synthesized node (it is renamed to `briev_main`).
...
Instead of `if/else`, Briev uses `when` guards:
...
Briev supports pipe chaining (`|>`) for chaining function calls in dataflow
