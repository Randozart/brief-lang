...
./target/debug/briev-compiler check counter.bv

# Compile to a native binary (LLVM → machine code)
./target/debug/briev-compiler build counter.bv

# Web frontend (TypeScript + view bindings)
./target/debug/briev-compiler build counter.rbv

# Embedded/microcontroller binary
./target/debug/briev-compiler build counter.ebv

# SPIR-V GPU kernel
./target/debug/briev-compiler build counter.abv

# CIRCT hardware description
./target/debug/briev-compiler build counter.cbv

# Validate Data Briev configuration
./target/debug/briev-compiler build counter.dbv

# Emit LLVM IR instead of a binary
./target/debug/briev-compiler build --llvm counter.bv
...
The **Metropolitan FFI** is the umbrella architecture for Briev's cross-language interoperability, comprising two mechanisms:

- **GLUE** (compile-time bridge generation) — exports Briev functions as native wrappers for any language, driven by protocol-path BFS in `lib/glue.toml`
...
The GLUE bridge system exports Briev functions as native wrappers for any language, driven entirely by `lib/glue.toml`:

Briev can export functions to any language via the GLUE (Generated Language Universal Exchange) bridge system, part of the broader **Metropolitan FFI** architecture:

```briev
// Export a function for Rust/Python/Node
export defn briev_pp_type(n: String) -> String {
    term pp_type(n);
};
```
...
briev export bridge.bv rust --out ./rust-crate   # Compilable Rust crate with safe wrappers
briev export bridge.bv python --out ./py-module  # ctypes Python module
briev export bridge.bv node --out ./node-module  # ffi-napi Node.js module
```

**How it works — Protocol-Driven, Not Type-Driven:**
- The TOML config maps protocol categories (`#String`, `#Int`, `#Float`) to language-native types — not Briev internal types
- The BFS protocol path optimizer finds the cheapest transform between Briev's representation and the target language's representation
...
**The GLUE pipeline:**
```
.bv → briev build --llvm → .ll (real function bodies, no stubs)
    → llc → .o → cc → .so → loaded via libloading/ctypes/ffi-napi
    → 7μs per call (C FFI), 6.5μs per call (Briev GLUE) — 3.5% slower, both correct
```
...
Metropipe provides zero-copy communication between Briev processes and foreign programs via shared memory:
...
- **661 lines** in `src/ffi/metropipe_cli.rs` (`briev metrod connect`)
...
5. **[11-triggers.md](learn-briev/11-triggers.md)** - Triggers and reactive I/O (`trg`, `trg!`)
6. **[13-projections.md](learn-briev/13-projections.md)** - `:>` metadata projections (Width, Endian, Codec, Ops)
7. **[15-custom-types.md](learn-briev/15-custom-types.md)** - Custom type definitions with operator declarations
...
- [learn-briev/15-custom-types.md](learn-briev/15-custom-types.md) - Custom type definitions guide
...
# Check a Briev file
./target/debug/briev-compiler check examples/counter.rbv

# Check in strict mode
./target/debug/briev-compiler check --strict counter.sbv
```
...
- ~11,300 lines of Briev self-hosted compiler
...
│   │   ├── metro_cli.rs          # `briev metrod connect` CLI (661 lines)
...
│   │
│   ├── dbriev/                   # Data Briev (DBVS) subsystem
...
│   │   └── ...                   # 15+ modules total
│   │
│   └── compiler/                 # Briev Self-Hosted Compiler
...
- [x] `.rbv` Briev-as-default format (no `<script>` wrapper)
...
- [x] `docs/architecture/bits-thesis.md`, `learn-briev/15-custom-types.md`
...
- [x] **Bridge benchmark**: Python ↔ C vs Briev (3.5% slower, all correct)
