...
<sub>(Briv is pronounced and was formerly known as 'Brief', but that name was overrepresented. Name can also be spelled as 'Brīv' for the long "ee" sound, or even Briiv, if that's your thing)</sub> 
### TL;DR
...
If you've gotten this far, I thank you for reading, and I hope you will have enjoyed your *Briv* time here so far.
...
Most programming languages are built around _operations in sequence_. Briv describes the _sequence of operations_ - the spatial connections between logical states.

*   **Logic as a Map:** Briv defines a world where roads exist all at once. The "sequence" is then can call o _connection_, not the _timing_.
...
**Deep Dive:** There are several .md documents scattered across the repo with random ideas on optimizing the language. Some are outdated, some aren't, but they should show the development of the Briv philosophy over time, and ways in which the topological approach has allowed backend optimization not otherwise available.
...
### Briv doesn't need you to be correct, it just needs you to be right.

The contract logic often just requires you to declare either the precondition or postcondition, not both. Contracts are simultaneously specification AND optimization input. In most languages, types/specs are safety rails that constrain what you can do. In Briv, they're also what the optimizer feeds on. The more you declare, the more the compiler can prove, and the faster your program runs. The file extension system (.bv → warnings, .sbv → hard errors) embodies the idea that you opt into strictness as your understanding deepens. Partial contracts compile with warnings. Full contracts with strict mode compile with proofs. This is a choice that distinguishes Briv from total languages (Coq, Agda) where you must prove everything upfront, and from mainstream languages where you prove nothing.
...
- **LLVM target** (`.bv`, `.ebv`): `frgn from "c"` resolved via `briv_rt.c`.
...
This is an odd one I discovered I could do while optimizing Briv. In most languages, a precondition, assertion or some other safety wrapper is you doing the compiler or even just the runtime a favor to prevent messy logic from crashing the program. In Briv, the contract *is* the optimization input. The more you declare, the more the compiler proves, and the faster your program runs. Safety enables speed.
...
There is no `if/else` in Briv. There are guarded blocks: `[condition] { body }`. This is not an omission. A guard forces you to ask "what must be true for this to execute?" rather than "which branch do I take?" If it feels harder than `if`, that is because you are specifying an invariant instead of a jump. The friction is the point. Operators that alter normal flow are marked with `!`: `term!` exits the program, `trg!` fires a hardware trigger, `sync!` forces a barrier, `$!` marks a high-power macro with access to `compile#`, `gensym#`, and `error#`. The `!` is the language saying "this is not a normal operation." If it feels heavy, good. It should. The strict variants (`.sbv`, `.cbv`) exist precisely to add friction. Sugar is banned, full contracts are required. You opt into strictness as your understanding deepens. The compiler does not let you take shortcuts when the material (hardware, safety) cannot afford them.
...
Briv's operators are organized into three conceptual groups:
...
./target/debug/briv-compiler check counter.bv

# Compile to a native binary (LLVM → machine code)
./target/debug/briv-compiler build counter.bv

# Web frontend (TypeScript + view bindings)
./target/debug/briv-compiler build counter.rbv

# Embedded/microcontroller binary
./target/debug/briv-compiler build counter.ebv

# SPIR-V GPU kernel
./target/debug/briv-compiler build counter.abv

# CIRCT hardware description
./target/debug/briv-compiler build counter.cbv

# Validate Data Briv configuration
./target/debug/briv-compiler build counter.dbv

# Emit LLVM IR instead of a binary
./target/debug/briv-compiler build --llvm counter.bv
...
The **Metropolitan FFI** is the umbrella architecture for Briv's cross-language interoperability, comprising two mechanisms:

- **GLUE** (compile-time bridge generation) — exports Briv functions as native wrappers for any language, driven by protocol-path BFS in `lib/glue.toml`
...
The GLUE bridge system exports Briv functions as native wrappers for any language, driven entirely by `lib/glue.toml`:

Briv can export functions to any language via the GLUE (Generated Language Universal Exchange) bridge system, part of the broader **Metropolitan FFI** architecture:

```briv
// Export a function for Rust/Python/Node
export defn briv_pp_type(n: String) -> String {
    term pp_type(n);
};
```
...
briv export bridge.bv rust --out ./rust-crate   # Compilable Rust crate with safe wrappers
briv export bridge.bv python --out ./py-module  # ctypes Python module
briv export bridge.bv node --out ./node-module  # ffi-napi Node.js module
```

**How it works — Protocol-Driven, Not Type-Driven:**
- The TOML config maps protocol categories (`#String`, `#Int`, `#Float`) to language-native types — not Briv internal types
- The BFS protocol path optimizer finds the cheapest transform between Briv's representation and the target language's representation
...
**The GLUE pipeline:**
```
.bv → briv build --llvm → .ll (real function bodies, no stubs)
    → llc → .o → cc → .so → loaded via libloading/ctypes/ffi-napi
    → 7μs per call (C FFI), 6.5μs per call (Briv GLUE) — 3.5% slower, both correct
```
...
Metropipe provides zero-copy communication between Briv processes and foreign programs via shared memory:
...
- **661 lines** in `src/ffi/metropipe_cli.rs` (`briv metrod connect`)
...
5. **[11-triggers.md](learn-briv/11-triggers.md)** - Triggers and reactive I/O (`trg`, `trg!`)
6. **[13-projections.md](learn-briv/13-projections.md)** - `:>` metadata projections (Width, Endian, Codec, Ops)
7. **[15-custom-types.md](learn-briv/15-custom-types.md)** - Custom type definitions with operator declarations
...
- [learn-briv/15-custom-types.md](learn-briv/15-custom-types.md) - Custom type definitions guide
...
# Check a Briv file
./target/debug/briv-compiler check examples/counter.rbv

# Check in strict mode
./target/debug/briv-compiler check --strict counter.sbv
```
...
- ~11,300 lines of Briv self-hosted compiler
...
│   │   └── ...                   # 15+ modules total
│   │
│   └── compiler/                 # Briv Self-Hosted Compiler
...
- [x] `.rbv` Briv-as-default format (no `<script>` wrapper)
...
- [x] `docs/architecture/bits-thesis.md`, `learn-briv/15-custom-types.md`
...
- [x] **Bridge benchmark**: Python ↔ C vs Briv (3.5% slower, all correct)
