...
Build the optimization framework described in `docs/design/determinism-and-optimization-frontier.md` into the Brief compiler. Each phase is independent (later phases depend on earlier ones) and verifiable against the existing test suite.
...
| 2.4 | **Integration** — wire enumeration into the compilation pipeline, controlled by budget | `src/main.rs`, `src/backend/mod.rs` | `brief-compiler llvm --optimize-budget 1000 input.bv` works end-to-end |
...
2. IIR benchmark — Brief maintains ≥ C speed (currently 0.14s vs 0.22s)
