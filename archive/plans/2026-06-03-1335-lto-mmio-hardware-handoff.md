...
1. **LTO gap**: `briev_rt.c` is compiled as machine code (`cc -c -O2`). The Briev IR goes through `opt -O3 → llc`. They're linked as ELF objects. Zero cross-module inlining — `__print_int`, `__wait_for_event`, thread pool primitives are forever opaque calls.
...
3. **Hardware handoff gap**: No path from Vivado's `system.xsa`/`xparameters.h` to compiler-visible addresses. The DBVS alias system exists (`src/dbriev/`) but never flows into LLVM codegen.
...
### Problem
`briev_rt.c` is compiled as machine code (`cc -c -O2`), then linked as ELF with the Briev-generated `.o`. The `opt -O3` pass sees only the Briev IR — `__print_int`, `__wait_for_event`, and thread pool barriers are opaque `call @symbol` instructions with zero inlining opportunity.

### Fix
Replace `cc -c` with `clang -c -emit-llvm`, merge with `llvm-link` before `opt`:

```
Before:  cc -c -O2 briev_rt.c → briev_rt.o
         opt -O3 program.ll → program.opt.ll
         llc -filetype=obj program.opt.ll → program.o
         cc -O2 program.o briev_rt.o → a.out

After:   clang -c -emit-llvm -O2 briev_rt.c → briev_rt.bc
         llvm-link program.bc briev_rt.bc → merged.bc
         opt -O3 merged.bc → merged.opt.bc
         llc -filetype=obj merged.opt.bc → merged.o
         cc -O2 merged.o → a.out
```

`opt -O3` on the merged module inlines `__print_int`'s `fprintf`, constant-folds FFI call arguments, and eliminates dead code across the Briev↔C boundary.

### Implementation (`src/main.rs`, `run_llvm_compile()`)
- Detect `clang` and `llvm-link` at pipeline start
- If available: `clang -c -emit-llvm -O2 briev_rt.c -o briev_rt.bc`
- Then: `llvm-link program.bc briev_rt.bc -o merged.bc`
...
- Parse the `.dbv` into `DbrievProgram`
...
- No changes to register/alias semantics in `src/dbriev/` (parser/AST/engine are correct)
...
- `briev build --hw-handoff system.xsa --target zcu4ev.dbv` produces correct MMIO code
