...
Briev exploits this to fold loops into O(1) stores, proving counter convergence
at compile time. This is legitimate optimization but leaves a blind spot: **how
does Briev perform when inputs are genuinely runtime-variable?**

With runtime-variable inputs:
- Briev cannot fold loops (no constant bound to store)
- Briev cannot precompute final values (no compile-time state space)
- Briev falls through to its runtime dispatch: reactor ticks, pre/fire/post, 
...
1. `runtime/briev_rt.c` adds `volatile int64_t __runtime_bound` — a C global
2. Constructor function `__briev_read_env_bound()` reads `BOUND` from environment
...
### Step 1: `runtime/briev_rt.c` — Add environment global + constructor
...
__attribute__((constructor)) static void __briev_read_env_bound(void) {
...
Mode B — runtime (--mode runtime):
  ...
  2. For each run:
     ...
     b. /usr/bin/time ./benchmark_runtime [briev]
...
Output:
  benchmark: ring_buffer (runtime, n=50)
    briev: mean=0.045s median=0.044s min=0.042s max=0.052s σ=0.0023s
    c:     mean=0.042s median=0.041s min=0.040s max=0.048s σ=0.0018s
    briev/c ratio: 1.07×  (Briev is 7% slower)
...
| File | Action | Lines |
|------|--------|-------|
| `runtime/briev_rt.c` | EDIT | +5 (global) +12 (constructor) |
...
|-----------|--------------------|--------------------|--------------|
| iir_filter | Field + Briev: 0.00s C: 0.10s | Both: actual loop | Briev's runtime dispatch overhead |
| ring_buffer | Both: 0.00s | Both: actual loop | Briev's trigger/reactor tick overhead |
| async_counters | Both: 0.00s | Both: actual loop | Thread pool + dispatch overhead |
| precompute_sum | Both: 0.00s | Both: actual loop | Briev's chain execution overhead |
