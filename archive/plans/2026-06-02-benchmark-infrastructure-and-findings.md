...
`float_math` and `float_math_nonzero` had `#!exit count == total` — only `count` was live. The dead-field elimination pass classified ALL float fields (`x0`, `x1`, `x2`, `p00`…`p22`) as dead, collapsing the entire 50M-iteration body to O(1) `store i64 50000000`. C references actually ran the loop. This created a false Briv-wins impression.
...
**Harness**: `benchmarks/build_and_bench.sh` now auto-compiles and caches `/tmp/briv_bench_timer` on first run.
...
| "float_math/nonzero are O(1) — Briv beats C" | Dead-field elim ate the body. C actually runs the 50M loop. Briv falls behind when the loop isn't folded. |
...
| Benchmark | Briv | C | Ratio | Who wins |
|-----------|-------|---|-------|----------|
| iir_filter | 0.0333s | 0.1526s | 0.21x | **Briv** (O(1) fold) |
...
| const_heavy | 0.0007s | 0.0548s | 0.01x | **Briv** (7x faster) |
