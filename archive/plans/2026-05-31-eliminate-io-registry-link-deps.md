...
The C runtime (`briev_rt.c`) is one possible provider of symbols — but the compiler should not know or
care which ones exist. The linker resolves symbols; the Briev source declares them. Nothing in between.
...
### New Mechanism: `import "link/briev_rt.o"`

```
import "link/briev_rt.o";   // tells the compiler driver: compile briev_rt.c and link briev_rt.o
```
...
instead of resolving as a Briev module, the compiler driver is told to include the
corresponding artifact at link time.

For `briev_rt.o` specifically, the compiler driver knows that `briev_rt.c` lives at
`runtime/briev_rt.c` (embedded via `include_str!`). It writes the C source, compiles it
...
4. The suffix (.o/.a) cleanly distinguishes "link this" from "import this Briev file"
...
pub struct LinkDependency {
    pub path: String,          // e.g., "briev_rt.o", "lib/mysensor.a"
    pub is_bundled_rt: bool,   // true if path == "briev_rt.o" → use BRIEV_RT_SOURCE
}
```
...
if last.ends_with(".o") || last.ends_with(".a") {
    let is_bundled_rt = last == "briev_rt.o";
...
if dep.is_bundled_rt:
    fs::write(briev_rt.c, BRIEV_RT_SOURCE)
    cc_status = cc -c -O2 briev_rt.c -o briev_rt.o [+ -DBRIEV_THREAD_POOL]
    link_objects.push(briev_rt.o)
else:
...
**4.1 New file: `lib/std/briev_rt.bv`**
...
This is the ENTIRE replacement for `io_registry.rs` — a pure Briev file with zero compiler-side knowledge.
`import "link/briev_rt.o"` tells the compiler driver to include the runtime. The trigger declarations
...
Or keep the direct `@ link` declaration — it already works. The import from `briev_rt.bv` avoids
...
in the source files, so the single `cargo run --bin briev-compiler -- llvm benchmarks/${name}.bv --out benchmarks` command works for all benchmarks.
...
**6.3 Add** — New tests:
- `test_link_dependency_parsed` — verifies `import "link/briev_rt.o"` produces `TopLevel::LinkDependency` with `is_bundled_rt = true`
...
in the compiler driver: "I already wrote `briev_rt.c` and compiled `briev_rt.o` — skip the second one."
A `HashSet<String>` of seen link paths is sufficient.

### Circular link deps

Not possible — link deps don't produce Briev items. They don't recurse through the import resolver.

### User links a missing .o

The compiler driver emits `error: link dependency 'foo.o' not found` if the file doesn't exist on disk.
For `briev_rt.o` specifically, the source is embedded — it always exists.

### What about selfhost compilation?

The selfhost compiler (`lib/compiler/`) doesn't use `#io`, `@ link`, or `briev_rt`. No impact.
...
### User types `import "link/briev_rt.o"` but also passes `--link-rt`
...
| Stdlib file named `briev_rt.bv` but not a standard .bv library | It IS a standard library file — it imports `link/briev_rt.o` AND declares triggers. `from "std/briev_rt.bv"` works exactly like any other stdlib import. |
...
| `briev_rt.bv` triggers have `is_wake = true`, but `#io` also set it | Identical. Both paths produce `is_wake = true`. |
