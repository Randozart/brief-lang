...
propagation (state vector + covariance matrix) — and stress-test Briev's struct-SSA
...
- Matches Briev exactly: local float variables, same constants, same loop structure
...
### Key Finding
**Briev is register-pressured at ~12 float fields.** Solutions for future optimization:
...
| Briev runtime ≈ C (within 20%) | ❌ (1.87× slower) |
