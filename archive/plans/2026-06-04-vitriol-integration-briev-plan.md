# Briev Compiler — VITRIOL Integration Plan

**Date:** 2026-06-04 08:14 UTC
**Status:** Design complete — awaiting vectorization syntax (`@`, `<-`, `...`) implementation
**Parent:** VITRIOL × Briev Master Plan (`../VITRIOL/.opencode/plans/vitriol-briev-integration-master-plan-2026-06-04.md`)

---

## 1. Overview

This document covers all changes to the **Briev Compiler** required for the VITRIOL integration. The compiler gains three new capabilities:
...
| **0.3** | Map Briev address spaces → SPIR-V addrspaces: `Ddr4`→CrossWorkgroup(1), `FpgaInternal`→Function(4), `Mmio`→Uniform(2), stack→Private(5) | `src/analysis/address_space.rs` |
...
| **1.3** | Briev's `range.rs` proves activation bounds from type constraints or preconditions | Already works |
| **1.4** | Briev's `region.rs` classifies matmul as `Pure` + bounded → chain composition folds the inner loop | Already works |
| **1.5** | Briev's `proof_engine.rs` verifies LUT lookup safety (bounds checking on activation values) | Already works |
| **1.6** | Compile `lut_matmul.bv` → `liblut_matmul.so` via `briev build --target x86_64` | Existing |
...
The LUT matmul source, written in post-vectorization-syntax Briev:
...
The Briev-compiled `liblut_matmul.so` exposes:
...
These are declared in Briev via:
...
- Master plan: `../VITRIOL/.opencode/plans/vitriol-briev-integration-master-plan-2026-06-04.md`
...
The Briev compiler changes must happen in this order (dependencies within the compiler):
