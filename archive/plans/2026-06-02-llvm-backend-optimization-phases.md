...
|-----------|--------|-------|---|-------|
| float_math (zero) | 0.452s | **0.011s** | 0.05s | **Briev wins (4.5× faster)** |
| float_math_nonzero | 0.486s | **0.380s** | 0.17s | **2.24×** |
| iir_filter | 0.172s | ~0.17s | 0.17s | ~tie |
| const_heavy | 0.006s | 0.006s | 0.04s | Briev wins |
...
For Briev's deterministic model where all float values are initialized to known
...
- Signed zero is irrelevant (Briev doesn't distinguish +0/-0)
...
|-----------|--------|-------|---|---------------|--------|
| float_math (zero) | 0.452s | **0.011s** | 0.05s | **Briev wins** | ✅ Alloca+SROA eliminated zero matrix |
...
| const_heavy | 0.006s | 0.006s | 0.04s | Briev wins | ✅ Already O(1) |
