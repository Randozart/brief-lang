...
Brief's optimizer eliminates dead code. A value is "dead" if no FFI call consumes it. This is correct — a program that produces no observable effect IS dead code.
...
## 10. Learn Brief Documentation (`docs/learn/liveness.md`)
...
Brief's optimizer eliminates code that produces no observable effect. This is correct — a program that doesn't print, write files, or make network calls is dead code.
...
### Phase 7: Learn Brief + SPEC Update

- Write `docs/learn/liveness.md`
- Update `spec/SPEC.md` with output type algebra (§ new section)
- Update `spec/old_docs/language_specs/v4-brief-lang-spec.md` to v5
