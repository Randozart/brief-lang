...
bench_self_term() {
    ...
    echo ""
    echo "=== $name ==="
    local briev_start=$(date +%s.%N)
    BOUND=50000000 ./benchmarks/"${name}" >/dev/null 2>&1
    local briev_end=$(date +%s.%N)
    local briev_time=$(printf "%.4f" $(echo "$briev_end - $briev_start" | bc))
    echo "  Briev: ${briev_time}s"
...
- Briev: 0.0558s (50M iterations)
- C: 0.0019s (pure-counter O(1))
- The 0.0558s includes runtime startup overhead; Briev's loop is O(1) after SCEV optimization
...
**Problem**: `acc = acc + C00 + ... + C19` is a linear recurrence (`acc += 21000` per iter). LLVM SCEV reduces it to `acc = 21000 * BOUND` (O(1) mul). Loop eliminated. Briev shows 0.00s.
...
Both Briev and C now run the full 50M loop with a non-linear `sdiv` per iteration. Neither compiler can cheat.
...
2. Baseline: All 7 benchmarks produce non-zero Briev timings (except precompute_sum which is legitimately O(1))
...
| Benchmark | Briev | C | Note |
|-----------|-------|---|------|
| iir_filter | ~0.15s | ~0.10s | Briev SSA loop vs C native loop |
...
| sparse_dispatch | >5s or crash | ~0.05s | alwaysinline bloat makes Briev pathological |
...
- **sparse_dispatch**: Briev's 8-precondition + 8-body call dispatch is ~100× slower than C's `switch(count%8)`. This is expected — Briev evaluates all 8 preconditions every tick, while C jumps directly. The benchmark shows WHERE Briev needs improvement: indirect dispatch (computed goto or jump table) instead of call-chain dispatch.
- **const_heavy**: After fixing, this measures pure arithmetic throughput. Briev and C should be similar since both generate `sdiv` + `add` per iteration through `opt -O2`.
