...
when `briev build` was switched to use the LLVM backend (previous session).
...
3. **No defensive guard on `post:` label** (emit_toplevel.rs ~457):
   Minor — valid Briev always ends bodies with `term`. Not fixing this cycle.
...
4. **Verify**: `cargo build && briev build officina.bv` passes `opt`
