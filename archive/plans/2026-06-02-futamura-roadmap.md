...
Briev's strict transactional state-machine model makes it uniquely suited to realize the **Futamura Projections** — an unbroken chain from self-hosting to a universal target-agnostic meta-compiler. The end state: a compiler that can analyze any Briev program and emit the absolute optimal hardware or software implementation, then generalize to compile any language expressed as a state machine.

### Why Briev Succeeds Where MIX (1985) Failed
...
**Verification after fix:** `./briev-compiler selfhost lib/compiler/lexer.bv lib/compiler/main.bv` tokenizes a briev source file correctly.

### Phase B2: Turtle Compiler (Acceleration Path)

Instead of writing the full parser and typechecker before seeing results, write a **Turtle compiler** — a minimal Briev compiler that handles only:
...
**Turtle is just enough to compile the IIR filter and Kalman filter.** It proves the self-hosting pipeline works end-to-end with a small, verifiable codebase (~300 lines of Briev vs ~1500 for the full compiler).

```briev
// Turtle.bv: Minimal Briev compiler (conceptual)
...
**Milestone:** `./briev-compiler selfhost lib/compiler/turtle.bv benchmarks/iir_filter.bv -o a.ll && clang a.ll briev_rt.o -O2 -lm && ./a.out` produces correct output.

### Phase B3: LUT Optimizer in Briev (Demonstration Path)
...
The Briev versions are shorter because they don't need memory management, error recovery, or complex trait dispatch — the state machine handles everything.
...
The compiled Briev compiler is a **pure state machine with zero heap allocation:**

| Compiler phase | When compiled via Briev's LLVM backend |
|---------------|--------------------------------------|
...
Stage 1 is the Briev compiler compiled by Rust. Stage 2 is the Briev compiler compiled by Stage 1. **Stage 2 is faster because:**
...
The ultimate goal: any Briev program with bounded state and trigger spaces compiles into a flat lookup table. The program doesn't compute — it looks up the predetermined future.
...
| Capability | Briev | C | Advantage |
|-----------|-------|---|-----------|
| Self-compilation | `stage0 compiles llvm.bv → stage1` | Impossible | **Briev only** |
| Compound optimization | Each cycle optimizes the optimizer | Fixed `clang -O2` | **Briev only** |
...
| ROM-based FSM synthesis | One IR → hardware or software | Two separate toolchains | **Briev only** |
...
Briev's self-compiling compiler:
1. **Round 1:** Briev compiler compiles itself through the full optimization pipeline
...
C compiles code into a binary that runs. Briev compiles code into a binary that runs, then compiles itself into a better binary that runs faster. **The optimization compounds with each cycle.**
...
| B3: Write LUT optimizer in Briev (~200 lines) | 3 days | LUT optimizer produces correct hash params |
