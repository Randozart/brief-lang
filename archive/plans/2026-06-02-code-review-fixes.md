...
- Briev already uses `icmp eq` for the folded loop exit (matches `test + jnz`)
...
- Briev compile: `opt -O2` + `llc -O2` + `clang -O3 -march=native` (link step)
...
| Benchmark | Briev | C | Ratio | Status |
|-----------|-------|---|-------|--------|
| float_math | 0.011s | 0.052s | **Briev 4.5×** | ✅ O(1), beats C |
...
| const_heavy | 0.006s | 0.044s | **Briev 7×** | ✅ |
| kalman_filter | 0.71s | 0.75s | **Briev beats C** | ✅ SLP disabled |
