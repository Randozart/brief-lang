# Compiler Sync Plan: Rust Bootstrap ↔ Briev Self-Hosted
...
The Briev compiler exists in two versions that have diverged:

| Aspect | Rust Bootstrap (`src/`) | Briev Self-Hosted (`lib/compiler/`) |
|--------|------------------------|-------------------------------------|
| Language | Rust | Briev (`.bv`) |
...
**The gap runs both ways.** Rust is ahead on v0.14 language features and production backends; Briev is ahead on AArch64 codegen and has a different WASM architecture.
...
2. **Sync v0.14 features** — parser/AST/typechecker/proof engine from Rust → Briev
3. **Backport AArch64** — from Briev → Rust
4. **Create missing backends** — COBOL, TCL in Briev
...
The Rust compiler's `wasm.rs` (2,022 lines) generates Rust+JS glue → wasm-pack → wasm. The Briev compiler's `wasm.bv` (351 lines) describes direct `.wasm` binary generation. These are **two different targets** sharing one name.

### Decision

Split into two distinct backends:

- **WASM** — Direct WebAssembly binary generation (`.wasm` file). Briev's `wasm.bv` is the authoritative spec.
...
| 1.2 | `src/backend/wasm.rs` | Create new file: direct WASM binary generation. Use Briev's `wasm.bv` as the architectural spec |
...
#### Briev self-hosted (`lib/compiler/`)
...
### WASM Binary Architecture (from Briev wasm.bv)
...
- **Runtime**: Briev reactor loop compiled to WASM loop, branchless guard execution via select, state encoded as linear memory
...
- `briev wasm program.bv` produces valid `.wasm` binary that `wasmtime` or `wasmer` can execute
- `briev webstack program.rbv` produces working browser app (wasm + JS + HTML)
- `cargo build && cargo test --lib` passes

---

## Phase 2: Sync v0.14 Features (Rust → Briev)

Sync these features from the Rust compiler into the Briev self-hosted compiler's parser, AST, typechecker, and proof engine.
...
**Note**: In the Rust compiler, this feature is **parsed into AST but never consumed by typechecker or backends** — only base struct fields are validated/emitted. The Briev self-hosted port matches this behavior.
...
4. **Extension**: `.sbo` (Strict Briev Optimized), `.sebo` (Strict Embedded Briev Optimized), `.srbo` (Strict Rendered Briev Optimized) — or equivalently, `--strict --optimize` CLI flags.
...
**Briev target files**:
- `lib/compiler/backends/mod.bv` — create this file (module registry for Briev backends). Add `supported_hashtags(backend)`, `validate_hashtags(tags, backend)`, and `validate_hashtags_in_program(program, backend, strict)` functions
...
**Key insight:** Briev's proof engine can *prove* a transaction is acyclic via symbolic execution (`execute_statement_symbolic`). If no path contains a loop (`StmtWhile`/`StmtLoop`) or unbounded recursion, the transaction is provably acyclic — a category most languages cannot statically guarantee.
...
- **Future hardware backends:** Acyclic Briev → VHDL/Verilog without handshake states — pure combinational logic.
...
Briev does not solve the halting problem — it structurally discourages it:

1. **No unbounded loop primitives** — `while`, `loop`, `for` do not exist in Briev. The `txn` construct is the only unit of iteration.
...
This is the termination analog of Briev's contract philosophy: make the provable case the default, make the uncertain case explicit and bounded, and eliminate the need for general halting-problem reasoning.

---

## Phase 3: Backport AArch64 (Briev → Rust)

### Current State

| Compiler | File | Lines | Capability |
|----------|------|-------|------------|
| Briev | `backend_aarch64.bv` | 1,654 | Full binary encoding, register allocator, memory layout, 3 optimization passes |
...
| Task | File | Description |
|------|------|-------------|
| 3.1 | `src/backend/aarch64.rs` | Port Briev's instruction enum with binary A64 encodings. All instruction categories: Data Processing (Immediate, Register, Floating-point), Loads/Stores (scalar, SIMD, exclusive), Branches (B/BL/B.cond/CBZ/CBNZ/TBZ/TBNZ), System (MSR/MRS/SVC/DC), Cryptographic |
...
- Regression: existing Briev `backend_aarch64.bv` still compiles and passes its tests

**Phase 3 status**: NOT STARTED. Documented 2026-05-25. Ready to resume.

---

## Phase 4: Create Missing Backends (Rust → Briev)

### 4.1 COBOL Backend

**Rust source**: `src/backend/cobol.rs` (709 lines)

**Briev target**: `lib/compiler/backends/cobol.bv`

Capabilities to port:
- Free-format COBOL source generation (no column restrictions)
- `WORKING-STORAGE SECTION` with PIC clauses for all Briev types
...
**Briev target**: `lib/compiler/backends/tcl_generator.bv`
...
### 5.1 C Backend: Briev → Rust → Briev

| Feature | Rust (908 lines) | Briev (205 lines) | Action |
|---------|-----------------|-------------------|--------|
| FFI bindings/calls/stubs | ✅ | ❌ | Port Rust→Briev |
| Kernel module entry | ✅ | ❌ | Port Rust→Briev |
| MMIO `@link` linkage | ✅ | ❌ | Port Rust→Briev |
| Target spec integration | ✅ | ❌ | Port Rust→Briev |
| Inline asm with clobbers | ✅ | ❌ | Port Rust→Briev |
| Local trigger support | ✅ | ❌ | Port Rust→Briev |
| Alka blocks | ✅ | ❌ | Port Rust→Briev |
| `#on_exit` cleanup | ✅ | ❌ | Port Rust→Briev |
| Error handling (bounds/null) | ✅ | ❌ | Port Rust→Briev |
| State allocation (static/dynamic) | ✅ | ❌ | Port Rust→Briev |
| Test mode | ✅ | ❌ | Port Rust→Briev |
| Hardware register tracking | ✅ | ❌ | Port Rust→Briev |
| Makefile generation | ✅ | ❌ | Port Rust→Briev |
| Basic reactor loop | ✅ | ✅ | (already synced) |
| Guard compilation | ✅ | ✅ | (already synced) |

### 5.2 Rust Backend: Briev → Rust → Briev

| Feature | Rust (783 lines) | Briev (219 lines) | Action |
|---------|-----------------|-------------------|--------|
| Struct/enum/constant definitions | ✅ | ❌ | Port Rust→Briev |
| Standalone definitions (functions) | ✅ | ❌ | Port Rust→Briev |
| Inline asm (core::arch::asm!) | ✅ | ❌ | Port Rust→Briev |
| Local triggers | ✅ | ❌ | Port Rust→Briev |
| Alka blocks | ✅ | ❌ | Port Rust→Briev |
| `#on_exit` cleanup | ✅ | ❌ | Port Rust→Briev |
| Escape/term statement handling | ✅ | ❌ | Port Rust→Briev |
| Type mappings (HashMap, Result, Option, Queue, Stack, HashSet, tuples, generics) | ✅ | ❌ | Port Rust→Briev |
| Vector element-wise ops | ✅ | ❌ | Port Rust→Briev |
| Field access / struct instances | ✅ | ❌ | Port Rust→Briev |
| Pattern match / block expressions | ✅ | ❌ | Port Rust→Briev |
| Tuple destructure | ✅ | ❌ | Port Rust→Briev |
| Slice/multislice | ✅ | ❌ | Port Rust→Briev |
| ForAll/Exists | ✅ | ❌ | Port Rust→Briev |
| Object literals | ✅ | ❌ | Port Rust→Briev |
| Default impl generation | ✅ | ❌ | Port Rust→Briev |
| `main()` entry point | ✅ | ❌ | Port Rust→Briev |
| State struct generation | ✅ | ✅ | (already synced) |
| Basic let/assignment | ✅ | ✅ | (already synced) |

### 5.3 Verilog Backend: Briev → Rust → Briev

| Feature | Rust (1,805 lines) | Briev (488 lines) | Action |
|---------|-------------------|-------------------|--------|
| AXI4-Lite state machine (full handshake FSM) | ✅ | ❌ | Port Rust→Briev |
| Clock divider per reactor speed | ✅ | ❌ | Port Rust→Briev |
| IO/memory mapping with hex-address lookup | ✅ | ❌ | Port Rust→Briev |
| Union type signals (data/err/tag) | ✅ | ❌ | Port Rust→Briev |
| Tuple signals | ✅ | ❌ | Port Rust→Briev |
| BRAM/UltraRAM inference with ram_style attributes | ✅ | ❌ | Port Rust→Briev |
| Vector generate-for loops | ✅ | ❌ | Port Rust→Briev |
| RAM priority encoder multiplexer | ✅ | ❌ | Port Rust→Briev |
| Timeout/watchdog per variable | ✅ | ❌ | Port Rust→Briev |
| Hardware validation (size/bits) | ✅ | ❌ | Port Rust→Briev |
| Testbench generation with VCD | ✅ | ❌ | Port Rust→Briev |
| `generate_with_axi` auto-detection | ✅ | ❌ | Port Rust→Briev |
| Regex-based vector lifting | ✅ | ❌ | Port Rust→Briev |
...
| Optimization (pipelining/retiming) | ❌ | ✅ | Port Briev→Rust |

### 5.4 VHDL Backend: Briev → Rust → Briev

| Feature | Rust (1,042 lines) | Briev (443 lines) | Action |
|---------|-------------------|-------------------|--------|
| Multi-file output (package/top/AXI/clk_div/RAM/FSM/txn/testbench) | ✅ | ❌ | Port Rust→Briev |
| AXI4-Lite slave bridge (full state machine) | ✅ | ❌ | Port Rust→Briev |
| Clock divider component | ✅ | ❌ | Port Rust→Briev |
| RAM inference with attributes | ✅ | ❌ | Port Rust→Briev |
| FSM (type/register/next-state/output) | ✅ | ❌ | Port Rust→Briev |
| PSL assertion comments | ✅ | ❌ | Port Rust→Briev |
| Testbench with clock/reset stimulus | ✅ | ❌ | Port Rust→Briev |
| Type width calculation | ✅ | ❌ | Port Rust→Briev |
| `get_pragma` attribute system | ✅ | ❌ | Port Rust→Briev |
| `is_ram_state` detection | ✅ | ❌ | Port Rust→Briev |
| Entity/architecture / process conversion | ✅ | ✅ | (already synced) |
| Component instantiation | ✅ | ✅ | (already synced) |
| Timing constraints | ❌ | ✅ | Port Briev→Rust |
| Optimization (resource sharing/pipelining/guard merging) | ❌ | ✅ | Port Briev→Rust |

### 5.5 x86_64 Backend: Briev ↔ Rust

| Feature | Rust (598 lines) | Briev (523 lines) | Action |
|---------|-----------------|-------------------|--------|
| Binary encoding (instruction tables, label resolution) | ❌ | ✅ | Port Briev→Rust |
| Register allocator | ❌ | ✅ | Port Briev→Rust |
| Memory layout pass | ❌ | ✅ | Port Briev→Rust |
| Two-pass encoding | ❌ | ✅ | Port Briev→Rust |
| Entry point / Linux syscall exit | ✅ | ❌ | Port Rust→Briev |
| Sequential + parallel reactor | ✅ | ❌ | Port Rust→Briev |
| Branchless guard (CMOV/SETcc) | ✅ | ❌ | Port Rust→Briev |
| Predictive fetch (PREFETCHT0) | ✅ | ❌ | Port Rust→Briev |
| Multi-expr generation | ✅ | ❌ | Port Rust→Briev |
| `collect_data_addresses` for prefetch | ✅ | ❌ | Port Rust→Briev |
...
| Briev compiler builds | `briev build lib/compiler/main.bv` | Briev self-hosted compiler compiles successfully |
| CLI help | `briev help` | Shows all commands including new ones |
| WASM output | `briev wasm file.bv` | Produces valid `.wasm` binary |
| Webstack output | `briev webstack file.rbv` | Produces `.rs` + `.js` + HTML output |
...
| 1 (WASM split) | Briev | 1 (`webstack.bv`) | 2 (`wasm.bv`, `main.bv`) | 3 |
| 2 (v0.14 features) | Briev | 1 (`backends/mod.bv`) | 6 (`token.bv`, `lexer.bv`, `ast.bv`, `parser.bv`, `typechecker.bv`, `proof_engine.bv`) | 7 |
| 3 (AArch64) | Rust | 0 | 1 (`aarch64.rs`) | 1 |
| 4 (missing) | Briev | 2 (`cobol.bv`, `tcl_generator.bv`) | 1 (`main.bv`) | 3 |
...
| Briev self-hosted compiler can't parse its own expanded AST | Low | Rust compiler is the bootstrap — can always compile the Briev compiler with itself after each change |
| Briev proof_engine.bv too small for SIMD checks | Low | SIMD length checking is a standalone function, minimal integration with existing symbolic engine |
| COBOL backend in Briev is a novelty with little practical use | Medium | Port the Rust implementation directly. Keep minimal. Don't over-engineer |
...
│   ├── cobol.rs             (709 lines)
│   ├── x86_64.rs            (598 lines)
│   ├── aarch64.rs           (561 lines — stub, Briev version is authoritative)
...
All 14 v0.14 feature sync phases completed in a single session. The Briev self-hosted compiler (`lib/compiler/`) now has:
