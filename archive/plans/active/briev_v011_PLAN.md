...
Briev-native syntax. Each word must be clear, predictable, inferrable. No magic.

### 3.1 File Type Inference

**Rule:** `.ebv` = embedded briev (no_std implied, MMIO volatility inferred)
**Rule:** `.bv` = standard Briev (full stdlib)
...
**Inference flow:**
1. Briev declares `let state: State @ 0x8000A000`
...
**No explicit `*mut` syntax needed in Briev source**
...
**Rule:** Briev vectors compile to fixed-size arrays in Rust (no Vec)
...
**Status:** Defer - likely expressible in Briev
...
| Bit Range | Briev Type | Rust Type |
|-----------|------------|-----------|
| `@/x1` - `@/x8` | `UInt[N]` | `u8` |
| `@/x1` - `@/x8` | `Int[N]` | `i8` |
| `@/x9` - `@/x16` | `UInt[N]` | `u16` |
...
| `BRIEV_v011_PLAN.md` | This document |
