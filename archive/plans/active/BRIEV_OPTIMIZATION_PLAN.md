...
Briev supports two build modes:

### Full Mode (`briev build`, default)
...
## 3. Fix Briev Self-Hosted Bugs

These bugs prevent Briev from compiling itself. Fixing them is a prerequisite for the self-hosted compiler to be a viable development platform.
...
- **Fix:** Full rewrite in valid Briev. These are 360-500 lines each.

### 3.4 `rust.bv:210`, `c.bv:197` — Rust syntax
- **Bug:** `String::new()` — Rust method syntax
- **Fix:** Replace with Briev-idiomatic string construction.
...
- **Backends:** `compile_to_wasm`, `compile_to_webstack`, `generate_rust` — all emit `"Hello from Briev"` only
- **Fix:** Real implementations matching the Briev self-hosted backend files.
...
### Briev Parser — AST nodes defined but not parsable
- `ExprTuple(List<Expr>)`
...
- `if/else` — Briev token.bv has `KeywordIf`/`KeywordElse`, Rust lexer.rs does not
- `while` loops — no tokens in either
- `for` loops — no tokens in either
- `loop` (infinite) — no tokens in either
- `match/switch` — Rust lexer has `Match`, Briev tokens do not
...
| P3 | Briev parser: missing expr types (Tuple, FieldAccess, etc.) | None | 2-3 sessions | PENDING |
...
- `15742b2` (2026-05-27): Fix Briev bugs (NOT(), range.bv, String::new()), implement stub parsers, wire analyze_program into all backends
