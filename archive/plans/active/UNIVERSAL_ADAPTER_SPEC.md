# Briev Compiler: Universal Transpilation Adapter
...
This document defines the **Target Spec TOML** system for the Briev compiler. The goal is to transform Briev from a multi-flag compiler into a universal transpilation adapter where framework-specific conventions are defined in declarative TOML files rather than hardcoded Rust logic.
...
- **Language Backend** (`c. rs`, `typescript. rs`) → Only generates syntax (translates Briev AST → target language)
...
```bash
briev compile --target linux_ kernel file. bv   # C backend + kernel headers
briev compile --target bare_ metal file. ebv  # C backend + ARM start
```
...
```bash
briev compile --target lib/ffi/profiles/ linux_ kernel. toml file. bv
```
...
```
CLI Layer
  briev compile --target <spec. toml>
         | loads
         v
Target Spec TOML
...
export const BrievApp: React.FC = () => {
"""
...
class BrievViewModel: ObservableObject {
    @Published var state: BrievState
"""
...
Attributes provide target-specific overrides without polluting the core Briev language.
...
- Generate TypeScript from Briev AST using spec
...
- Map Briev state to WGSL address spaces (`var<storage>`, `var<uniform>`)
...
| CLI loads specs | briev compile --target linux_kernel.toml file.bv succeeds |
