...
LLVM already knows how to emit every syscall across every target. Briev
...
| `spawn#` | `popen(cmd)` → read output → return as Briev string | `(cmd: String) -> String` |

### Filesystem (5 + 2 existing)
| Intrinsic | LLVM IR | Briev sig |
|-----------|---------|-----------|
| `read_file#` | `fopen+fread+fclose` → Briev string | `(path: String) -> String` — EXISTS |
| `write_file#` | `fopen+fwrite+fclose` | `(path, data: String) -> Int` — EXISTS |
| `list_dir#` | `opendir+readdir+closedir` → newline-sep Briev string | `(path: String) -> String` |
...
| Intrinsic | LLVM IR | Briev sig |
|-----------|---------|-----------|
| `env_get#` | `getenv(name)` → return as Briev string | `(name: String) -> String` |
...
| `read_stdin#` | `fread(stdin, buf, len)` → return as Briev string | `() -> String` |
...
| `tcp_recv#` | `recv(fd, buf, len, 0)` → return data as Briev string | `(fd: Int) -> String` |
...
3. Briev string helper: for intrinsics returning strings, emit `alloca` + data pointer + length + chars, return `ptrtoint`
...
### Phase 3: Stdlib string/JSON implementations (briev_rt.c)
...
| `src/backend/llvm/emit_expr.rs` | 16 new match arms + Briev string helper |
| `src/main.rs` | Auto-link from `from` paths in LTO pipeline |
| `lib/runtime/briev_rt.c` | Implement __trim, __to_lower, __split, etc. |
