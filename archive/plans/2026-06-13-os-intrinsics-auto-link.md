...
LLVM already knows how to emit every syscall across every target. Brief
...
| `spawn#` | `popen(cmd)` → read output → return as Brief string | `(cmd: String) -> String` |

### Filesystem (5 + 2 existing)
| Intrinsic | LLVM IR | Brief sig |
|-----------|---------|-----------|
| `read_file#` | `fopen+fread+fclose` → Brief string | `(path: String) -> String` — EXISTS |
| `write_file#` | `fopen+fwrite+fclose` | `(path, data: String) -> Int` — EXISTS |
| `list_dir#` | `opendir+readdir+closedir` → newline-sep Brief string | `(path: String) -> String` |
...
| Intrinsic | LLVM IR | Brief sig |
|-----------|---------|-----------|
| `env_get#` | `getenv(name)` → return as Brief string | `(name: String) -> String` |
...
| `read_stdin#` | `fread(stdin, buf, len)` → return as Brief string | `() -> String` |
...
| `tcp_recv#` | `recv(fd, buf, len, 0)` → return data as Brief string | `(fd: Int) -> String` |
...
3. Brief string helper: for intrinsics returning strings, emit `alloca` + data pointer + length + chars, return `ptrtoint`
...
### Phase 3: Stdlib string/JSON implementations (brief_rt.c)
...
| `src/backend/llvm/emit_expr.rs` | 16 new match arms + Brief string helper |
| `src/main.rs` | Auto-link from `from` paths in LTO pipeline |
| `lib/runtime/brief_rt.c` | Implement __trim, __to_lower, __split, etc. |
