...
- Add `frgn __rt_init() -> Void;`, `frgn __rt_poll() -> Void;`, `frgn __rt_wait() -> Void;` to `lib/std/briv_rt.bv` (or a new `lib/std/rt.bv`)
...
## 2026-06-07 Addendum: `trg` owns its runtime — auto-link briv_rt.c
...
2. Also writing `import "link/briv_rt.c";`
...
- Compiler auto-links `briv_rt.c` when any `@ link` trigger is present
- Compiler emits `declare void @__rt_wait()` and `call void @__rt_wait()` when `has_wake_triggers`
- Constructor in `briv_rt.c` handles `__rt_init` automatically
- No `frgn` declarations, no hardcoded strings, no manual C file imports

### Steps

1. **Compiler driver auto-links `briv_rt.c`** (main.rs)
   - During compile pipeline scan, check for any `TopLevel::Trigger` with `LinkRef::Linked(_)`
   - If found, add `briv_rt.c` to link dependencies automatically
   - No changes needed for non-trigger programs (they already don't link briv_rt.c)
...
2. **Remove `__rt_init` and `__rt_poll` from LLVM backend** (llvm.rs)
   ...
   - Constructor in briv_rt.c already calls `__rt_init` at load time
...
4. **Remove `import "link/briv_rt.c"` from all benchmarks** (benchmarks/*.bv)
