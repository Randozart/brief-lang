...
Two systemic inefficiencies in the LLVM backend waste cycles on every benchmark and prevent Briev from decisively beating C on float-heavy workloads.
...
Briev uses `i64` as its universal wire type. Every float operation requires round-tripping through i32→float→i32→i64. Tracing a single Kalman filter iteration:
...
When `emit_binop` sees both operands as compile-time constants, fold them at the Briev compiler level:
...
| Optimization | Briev | C (world-class) |
|-------------|-------|-----------------|
...
| Constant peephole folding | Briev-level folding before LLVM | Clang does this too (no advantage) |
| `emit_exit_expr` unification | Currently duplicated → fixable | N/A |

**C's advantage:** Local variables naturally stay in float registers. Briev's i64 boxing adds overhead that float register promotion eliminates.

**Briev's advantage:** `llvm.assume` on convergent preconditions removes branches that C's optimizer cannot prove are always-taken. The proof engine's convergence proof is information C's compiler fundamentally lacks.
...
| Ratio vs C | +5% (Briev wins) | **+33-40% (Briev wins decisively)** | baseline |
...
9. **Kalman filter benchmark**: Briev beats C by ≥10%
