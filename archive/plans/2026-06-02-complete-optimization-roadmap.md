...
|-----------|-------|---|-------|--------|
| iir_filter | 0.0333s | 0.1526s | 0.21× | **Brief** (O(1) fold) |
...
| const_heavy | 0.0007s | 0.0548s | 0.01× | **Brief** (7× faster) |
...
| Round 2 code-review fixes | __find_from, dbrief pipeline, dataflow extracts, protocol verifier, parser dedup | Round 2 |
...
**Why Brief is uniquely suited**: Composed chains have no side effects, no pointer aliasing, no FFI calls — they are pure mathematical expressions. Equality saturation is designed exactly for this scenario.
...
**Solution**: Brief has a built-in `Interpreter` (`src/interpreter/mod.rs`). Use it for zero-overhead compile-time profiling:

1. **Interpret with profiling mode**: Execute the Brief program in the interpreter with sample inputs (or generate reasonable defaults from state initial values)
...
**Expected impact**: Makes Brief's optimization decisions explicit and auditable. Enables future passes to target specific tiers.
...
Audit all function boundaries and add `noalias`/`nocapture` where Brief's semantics guarantee it. Incremental.
