...
/// VHDL code generator: converts a Briev Program into multi-file VHDL output.
...
impl VhdlGenerator {
    ...
    fn emit_package(&self, program: &Program) -> String {
        ...
        for item in &program.items {
            if let TopLevel::StateDecl(state) = item {
                let vh = self.briev_type_to_vhdl(&state.ty);
        ...
        for item in &program.items {
            if let TopLevel::Constant(c) = item {
                let vh = self.briev_type_to_vhdl(&c.ty);
    ...
    /// Convert a Briev statement to VHDL code with the given indentation level.
    fn statement_to_vhdl(&mut self, output: &mut String, stmt: &Statement, indent: &str) {
        match stmt {
            ...
            Statement::LocalTrigger { name, ty, expr, .. } => {
                let _ty_str = self.briev_type_to_vhdl(ty);
                if let Some(e) = expr {
                    let expr_code = self.expr_to_string(e);
    ...
    /// Convert a Briev Type to its VHDL type string, handling all type variants.
    fn briev_type_to_vhdl(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Vector(inner, dims) => {
                let inner_vhdl = self.briev_type_to_vhdl(inner);
            ...
            Type::Tuple(types) => {
                let fields: Vec<String> = types.iter().enumerate().map(|(i, t)| {
                    format!("        field_{} : {}", i, self.briev_type_to_vhdl(t))
                }).collect();
                format!("record\n{}\n    end record", fields.join(";\n"))
            }
            Type::Union(types) => {
                let max_width = types.iter().map(|t| self.get_type_width(t)).max().unwrap_or(32);
                let field_types: Vec<String> = types.iter().map(|t| self.briev_type_to_vhdl(t)).collect();
    ...
    /// Convert a Briev expression to a VHDL expression string.
