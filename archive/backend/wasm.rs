...
impl WasmBackend {
    ...
    /// Intent: Generate WAT text output for the given Briev program.
...
/// Intent: Map a Briev type to the nearest WASM value type.
fn map_briev_type(val_type: &Type) -> ValType {
    match val_type {
...
/// Intent: Compile a Briev expression into a sequence of WASM opcodes.
...
/// Intent: Compile a list of Briev statements into WASM opcode bytes.
...
/// Intent: Compile an entire Briev Program into a WASM binary module.
...
/// Intent: Generate a WASM module from a Briev program using default configuration.
