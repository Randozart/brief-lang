...
/// Intent: Backend that compiles Briv programs to C with optional inline assembly.
pub struct CBackend {
...
impl CBackend {
    ...
    /// Intent: Generate C source and optional Makefile from a Briv program.
    pub fn generate(&mut self, program: &Program, stem: &str) -> (String, Option<String>) {
        ...
        output.push_str("/* C application - generated by Briv */\n");
        ...
        match entry_style {
            "module_init" => {
                ...
                output.push_str("static int __init briv_init(void) {\n");
                if let Some(entry_name) = entry_txn {
                    output.push_str(&format!("    {}();\n", Self::sanitize_name(&entry_name)));
                }
                output.push_str("    return 0;\n");
                output.push_str("}\n");
                output.push_str("module_init(briv_init);\n\n");

                let exit_txn = self.find_exit_point(program);
                output.push_str("static void __exit briv_exit(void) {\n");
                if let Some(exit_name) = exit_txn {
                    output.push_str(&format!("    {}();\n", Self::sanitize_name(&exit_name)));
                }
                output.push_str("}\n");
                output.push_str("module_exit(briv_exit);\n\n");

                output.push_str("MODULE_DESCRIPTION(\"Generated by Briv\");\n");
    ...
    /// Intent: Convert a Briv statement to C code.
    ...
    /// Intent: Convert a Briv expression to its C representation.
    ...
    /// Intent: Map a Briv type to its C type string.
    ...
    /// Intent: Get the default initialization value for a Briv type.
    ...
    /// Intent: Sanitize a Briv identifier for use as a C name (replace hyphens with underscores).
