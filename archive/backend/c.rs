...
/// Intent: Backend that compiles Briev programs to C with optional inline assembly.
pub struct CBackend {
...
impl CBackend {
    ...
    /// Intent: Generate C source and optional Makefile from a Briev program.
    pub fn generate(&mut self, program: &Program, stem: &str) -> (String, Option<String>) {
        ...
        output.push_str("/* C application - generated by Briev */\n");
        ...
        match entry_style {
            "module_init" => {
                ...
                output.push_str("static int __init briev_init(void) {\n");
                if let Some(entry_name) = entry_txn {
                    output.push_str(&format!("    {}();\n", Self::sanitize_name(&entry_name)));
                }
                output.push_str("    return 0;\n");
                output.push_str("}\n");
                output.push_str("module_init(briev_init);\n\n");

                let exit_txn = self.find_exit_point(program);
                output.push_str("static void __exit briev_exit(void) {\n");
                if let Some(exit_name) = exit_txn {
                    output.push_str(&format!("    {}();\n", Self::sanitize_name(&exit_name)));
                }
                output.push_str("}\n");
                output.push_str("module_exit(briev_exit);\n\n");

                output.push_str("MODULE_DESCRIPTION(\"Generated by Briev\");\n");
    ...
    /// Intent: Convert a Briev statement to C code.
    fn statement_to_c(&mut self, output: &mut String, stmt: &Statement) {
        match stmt {
            ...
            Statement::SyncBlock { body } => {
                for stmt in body { self.statement_to_c(output, stmt); }
            }
    ...
    /// Intent: Convert a Briev expression to its C representation.
    ...
    /// Intent: Map a Briev type to its C type string.
    ...
    /// Intent: Get the default initialization value for a Briev type.
    ...
    /// Intent: Sanitize a Briev identifier for use as a C name (replace hyphens with underscores).
