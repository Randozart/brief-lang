...
`Char → String` casts, creating a hidden dependency on `briev_rt.c`.
...
**What to remove**:
- `__chr_to_str` from `lib/runtime/briev_rt.c` (9 lines)
...
```briev
// briev_rt.c provides these C functions:
...
- All `__trg_*` C wrappers from `briev_rt.c`
