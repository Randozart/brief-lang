...
Wire the Rust interpreter to `lib/compiler/main.bv` so that `main.bv` can compile Briev programs — producing its own backend output (initially stubs, eventually real C/Rust).
...
A new subcommand:
```
briev-compiler selfhost <file.bv> [--backend c|rust] [--verbose]
```
...
3. `briev-compiler selfhost examples/counter.rbv` — produces backend output (stub or real)
...
- **Phase 7**: `gcc` the C output → standalone Briev compiler