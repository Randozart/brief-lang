...
Wire the Rust interpreter to `lib/compiler/main.bv` so that `main.bv` can compile Briv programs — producing its own backend output (initially stubs, eventually real C/Rust).
...
A new subcommand:
```
briv-compiler selfhost <file.bv> [--backend c|rust] [--verbose]
```
...
3. `briv-compiler selfhost examples/counter.rbv` — produces backend output (stub or real)
...
- **Phase 7**: `gcc` the C output → standalone Briv compiler