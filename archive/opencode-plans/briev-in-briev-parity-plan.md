# Briev-in-Briev Parity Plan

## Goal
Bring `parser.bv`, `typechecker.bv`, `ast.bv`, `token.bv`, and `proof_engine.bv` in the self-hosted Briev compiler up to parity with the Rust compiler's `Expr::Term` handling.
...
The Briev-in-Briev compiler has none of this. It does pure type inference, not symbolic execution.
