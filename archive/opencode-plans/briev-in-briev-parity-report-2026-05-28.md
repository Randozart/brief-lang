# Briev-in-Briev Self-Hosting Status
...
Make all 18 `.bv` files in `lib/compiler/` parse, type-check, and eventually compile Briev code through the self-hosted compiler.
...
1. **Named struct literals unsupported**: `TypeName { field: val }` is NOT valid Briev syntax. Only anonymous `{ field: val }` (parsed as `Expr::ObjectLiteral`) works.
