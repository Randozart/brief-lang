...
1. **Briev-in-Briev parser (`parser.bv`)**: Does not yet recognize `term` in postconditions — it still expects `result`. When the self-hosted compiler reaches parity with the Rust parser, this needs updating.
