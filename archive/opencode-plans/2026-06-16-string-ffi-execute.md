...
| Brief type | Was (i64 boxed) | Now (native) |
|------------|----------------|--------------|
| `String` / `Data` | `i64` (ptrtoint of header) | `i8*` (pointer to Brief 2-slot header) |
...
- **Both operands compile-time constant** → fold at compile time (single global Brief header)
- **One or both runtime** → always-allocate inline concat (malloc + memcpy + header setup)
- **No buffer reuse** — Brief compiler has no ownership/lifetime analysis (would need to be built from scratch)
...
**1c. String constant globals → Brief headers**
...
/// Both %a and %b are i8* (Brief header pointers).
...
Currently marshals params (lines 290, 327) with `inttoptr i64 %raw to i8*` for String params. With native types, String params are already `i8*`, so we just pass them through directly. BUT we still need to extract `data_ptr` from the Brief header — the C function expects `char*`, not `i8*` (Brief header pointer).

Wait — this is the key issue. Even with native `i8*` for strings, the `i8*` is a **Brief header pointer**, not a C string pointer. The C function expects `char*` pointing to the actual string data. So we still need to extract slot 0.
...
to:
```llvm
; NEW: %raw is i8* (native Brief header pointer)
...
For return marshaling: when C returns `i8*` and Brief expects String → wrap in Brief header via `strlen` + `malloc` + memcpy inline.
...
- `Intrinsic::ReadFile` → currently `call ptr @brief_read_file(ptr %fp)` then `ptrtoint ptr %raw to i64` — change to just use ptr directly as `i8*`
...
Cover:
- Brief string format (2-slot header)
...
/path/to/brief-compiler llvm officina.bv
clang -O3 officina.ll /path/to/brief_rt.c -lc -o officina_bin
...
1. "compiler: native LLVM types for strings (i8*), bools (i1/i8), chars (i32)"
   Steps 1-19, Step 20. All emit_*.rs + mod.rs + brief_rt.c changes.
