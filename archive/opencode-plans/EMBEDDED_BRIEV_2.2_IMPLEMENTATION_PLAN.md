# Embedded Briev 2.2 Implementation Plan

## 1. Goal
Implement the features for Embedded Briev 2.2, unifying logic across Core (.bv), Rendered (.rbv), and Embedded (.ebv) variants with high-velocity hardware primitives.
...
- [ ] **AST:** 
    ...
    - Add `Slice`, `Index`, `Stride` variants to `Expr`.
    - Refine `Union` and `Result` types in variable declarations.
...
- [ ] **Mandatory Error Handling:** Detect `Union` type in assignment; block `term` if `Error` variant path is unhandled by guards (`[result is Error]` or `[result Err(e)]`).
...
## 5. Cross-Variant Integration & Build (Phase 5)
