...
pub enum OpImpl {
    ...
    Defn(String),      // Briev defn by name
...
- LLVM: attempts to lower via `llvm.fadd.bf16` if available, otherwise
  uses the inop body (which compiles to plain Briev→LLVM)
...
### Performance: Runtime (Compiled Briev Programs)
...
This is fragile — it matches on LLVM type strings, not Briev types. A
