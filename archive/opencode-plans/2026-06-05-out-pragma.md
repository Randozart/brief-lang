...
In Briev, the compiler is already running at full speed by default — inlining, folding, precomputing, dead-field-eliminating — with maximum zealotry. **Pragmas are the programmer's way to request the compiler calm down on a specific point.** Not "help me optimize" but "I understand you can prove this is dead, but I need it alive anyway."
...
This is teachable in one sentence: **"Briev runs at full speed by default. A pragma is a request to the compiler to hold back its zealotry on a specific point."** The programmer holds the authority — the compiler defers.

## The Problem

After LTO merges `briev_rt.c` with the program IR, LLVM sees `fprintf(stderr, ...)` in the inlined body of `__print_int`. LLVM's GlobalOpt recognizes `fprintf` via TargetLibraryInfo as a stdio write to `stderr`. Since nothing in the merged module ever re-reads `stderr`'s buffer, LLVM proves the write is dead and eliminates the entire call chain. Same for `__print_float` and `__putchar`.
...
Briev already has native syntax for distinguishing call kinds:
...
A Briev programmer writes:
```briev
import { __print_int } from "std/briev_rt.bv";
...
The `#out` modifier emits `memory(write)` on the ***individual declaration***, not on the shared attribute group. This survives LTO: even after `llvm-link` merges `briev_rt.bc`, the program's declaration side keeps `memory(write)`. LLVM knows the call "might write to memory that's later read" and preserves it.
...
| `lib/std/briev_rt.bv` | Add `#out` to output function declarations |
| `runtime/briev_rt.c` | Restore `static inline` on runtime helpers |
