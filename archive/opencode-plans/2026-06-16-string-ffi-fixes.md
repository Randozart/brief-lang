...
Strings are `i8*` (pointer to Briev 2-slot header). Bool is `i1` (SSA) / `i8` (memory). Char is `i32`. Only Int/UInt remain `i64` (they ARE integers).
...
### Step 1 — String constants → global Briev headers
**File:** `src/backend/llvm/mod.rs`  
**Lines:** 947-950 (string constant emission)  
**Before:** `[N x i8] c"..."` C-string globals  
**After:** `<{i64, i64, [N x i8]}>` Briev-header globals (data_ptr, length, chars)  
...
### Step 3 — Cleanup `briev_rt.c`
**File:** `lib/runtime/briev_rt.c`  
**Remove:** `__str_concat` function, `safe_cstr` function, `__resolve_briev` helper, debug `fprintf(stderr, "DEBUG...")`  
**Keep:** `cstr_to_briev` (used internally by `__int_to_str` etc.)
...
- BUT: since strings are now `i8*` natively, it's: `bitcast i8* %raw to i64*` → `load i64, i64* %hp` → `inttoptr i64 %dp to i8*` — wait, `%raw` IS already `i8*` (the Briev header pointer). So we just bitcast to `i64*` and load slot 0.
...
```bash
cd ~/Desktop/Projects/briev-compiler
cargo test --lib
cargo build --release
```
...
| `lib/runtime/briev_rt.c` | Remove dead functions | 30 |
...
```llvm
; %a and %b are i8* (Briev header pointers)
...
### FFI param: Briev string → C string
```llvm
; %str is i8* (Briev header pointer)
...
### FFI return: C string → Briev string (inline)
```llvm
...
; %result is the Briev header pointer (i8*)
```
...
1. **`feat: native LLVM types for strings (i8*), chars (i32), bools (i1/i8)`**  
   Compiler core: all emit_expr.rs, emit_stmt.rs, mod.rs changes + briev_rt.c cleanup  
...
3. **`fix: officina-cli migration to intrinsics + native types`**  
   ...
   Test: compile officina with `briev-compiler llvm`
