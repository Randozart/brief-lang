# Self-Hosted (Briev-in-Briev) LLVM Backend

## Overview

The self-hosted LLVM backend lives at `lib/compiler/backends/llvm.bv`. It mirrors the Rust backend's logic but emits `.ll` text using Briev's `StringBuilder` pattern.

## Key Translations

| Rust Feature | Briev Equivalent |
|--------------|------------------|
...
1. **Parse-only mode** — `Briev-compiler selfhost counter.bv --target llvm` runs without crashing
