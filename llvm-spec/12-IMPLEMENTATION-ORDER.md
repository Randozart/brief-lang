...
**Goal**: `briev llvm file.bv` runs and produces a `.ll` file.
...
**Goal**: `briev-compiler selfhost file.bv --target llvm` produces the same `.ll` as `briev llvm file.bv`.

| Step | What | Details |
|------|------|---------|
| 7.1 | `llvm.bv` — module header + `%State` type | Mirror Phase 0 in Briev |
