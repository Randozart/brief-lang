# Briev LLVM Backend — Change Log & Design Journal
...
- **Root cause:** `@briev_string_to_cstr` heap-allocates but never frees — infinite leak in reactor loop.
...
- **Pipeline:** Briev → LLVM IR → `llc -march=nvptx64` → `.ptx` → `ptxas` → SASS
