...
Briev → LLVM IR → GPU-native virtual assembly → Physical machine code:
...
Both paths are viable because LLVM IR is target-independent. The same Briev source compiles to both.

## 2. Four GPU Optimizations Briev Enables (That CUDA Cannot)
...
**In Briev:** The SMT proof engine analyzes array indexing expressions:
```briev
...
**In Briev:** The `noalias` model guarantees disjoint thread memory regions:
```llvm
...
**In Briev:** The dataflow analysis proves read/write patterns:
...
**In Briev:** The guard-to-`select` conversion (`05-CONTRACT-TO-METADATA.md`) flattens branches:
```llvm
; Instead of:
...
```bash
# Step 1: Compile Briev to LLVM IR
briev-compiler llvm kernel.bv --out output.ll
...
- Must wait for Briev-in-Briev self-hosting (Phase 7) before Briev can compile its own GPU kernels