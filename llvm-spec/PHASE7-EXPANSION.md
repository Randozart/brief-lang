# Phase 7 Expansion Plan: Full LLVM IR Emission in Brief
...
The backend needs state across calls. In Brief, this uses closure-passing patterns (unlike Rust's struct fields):
...
brief-compiler llvm tests/fixtures/counter.bv --out /tmp/r/
llc /tmp/r/counter.ll -o /dev/null

# llvm.bv compiles through itself
brief-compiler llvm lib/compiler/backends/llvm.bv --out /tmp/r/
