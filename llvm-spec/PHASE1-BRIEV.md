# Phase 1 Briev: Basic Transaction Emission
...
**Expected LLVM:** The precondition expression `counter < 10` emits `icmp slt` + `zext` to produce an `i64` from the `i1` comparison. The `zext` is needed because Briev's expression system uses `i64` for Bool values (1 = true, 0 = false).
...
```bash
briev-compiler llvm tests/fixtures/phase1/full_txn.bv --out /tmp/p1/
...
- **Bool values are i64 in Briev's expression system** but i8 in `%State`. Transactions must `trunc` stores and `zext` loads. The Phase 0 implementation handles this, but the `let` binder must preserve the i64 type for chained arithmetic.
