...
Brief's three first-class safety features are not a runtime tax — they are
...
| **Brief** | `[pre][post]` | Proven at compile-time → converted to `!range` + `@llvm.assume` → **code gets faster** |
...
This is the core architectural insight of Brief's LLVM backend: **safety
...
pub struct LlvmBackend {
    ...
    /// Tracks the logical Brief type of each SSA register, for use at
...
| 5A | 484-494 | Unification dominance violation — `%upX` defined only in the matching arm but inserted into `self.let_bindings` globally; the `def_l` (default) path never defines it, yet `merge_l` references it | Change `def_l` to `unreachable` (Brief guarantees the pattern must match or the program panics) |
...
Not correctness-critical, but realizes Brief's optimization vision.
...
If strings need to be mutable (unlikely in Brief's semantics), fall back to
...
| Brief Type | LLVM Type | Storage in %State | i64 Representation |
|-----------|-----------|-------------------|-------------------|
| `Int` | `i64` | `i64` | Direct |
| `UInt` | `i64` | `i64` | Direct |
...
2. **`noalias` as a language guarantee.** Brief's prohibition on arbitrary
