...
Briev prohibits arbitrary pointers. There is no `&` operator, no pointer arithmetic, no reference escaping. Every `rstruct` field access is a named GEP on the global `%State*`. Therefore:
...
```llvm
; Briev: &count = count + 1; &count = count * 2;
