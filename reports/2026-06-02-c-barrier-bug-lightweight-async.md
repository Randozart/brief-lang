...
Runtime input fuzzing exposed a latent bug in Brief's thread pool barrier and an
...
The actual thread pool barrier in `runtime/brief_rt.c` is a custom portable
...
**This is fundamentally a C runtime bug, not a compiler bug.** Brief's LLVM IR
is correct — the generated `call void @brief_barrier_release()` and
`call void @brief_barrier_wait()` are exactly right. The C implementation of
...
main:
  init_state()
  brief_thread_pool_init(2, ...)   // 50µs overhead
  tick:
...
1. Skips `brief_thread_pool_init()` in `emit_main()` / `emit_enum_main()`
2. Skips `brief_barrier_release()` / `brief_barrier_wait()` in the main loop
...
Brief's compiler is so good at optimization that it never exercised its own
...
| `runtime/brief_rt.c` | ~20 | `__get_env_int()` function + barrier generation fix |
| `lib/std/env.bv` | NEW | `frgn __get_env_int(name: String) -> Int` |
| `benchmarks/*_runtime.bv` | 4 NEW | Runtime-variant Brief benchmarks |
