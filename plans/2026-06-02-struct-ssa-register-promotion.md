...
Brief's folded-loop codegen currently accesses every state field through
...
Brief can match this by loading the entire `%State` struct once per loop
...
| Benchmark | Before (Brief) | After (Brief) | C | Notes |
|-----------|---------------|--------------|---|-------|
