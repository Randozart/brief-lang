...
./brief-compiler llvm benchmarks/nbody_newton.bv --out benchmarks --optimize-budget 2048`
...
**Problem**: Brief outputs result to stderr; C returns it as exit code.
These are different mechanisms and produce different numbers (Brief "10",
...
use different algorithms. The Brief version does more work (multiple fields,
...
- `queue_drain_sym.bv` — mirrors C step-for-step using Brief features
- `queue_drain_idio.bv` — Brief-native pattern (contract-proven, reactive)
...
1. `BOUND=5` run — confirm Brief and C produce identical stderr output
