...
New architecture: **stdlib implemented entirely in Brief** using arrow syntax (`<-`), projections (`:>`), and `uni` pattern matching — zero Rust-side string matching for collection operations.
...
After this, `Ok(value)` in Brief source never hits any FFI registry — it's caught here.
...
Note: After Step 5, `Ok(value)` in Brief source will dispatch natively via enum variant lookup, never hitting FFI. The `__builtin_Result_Ok` frgn can be deleted once that's verified.
