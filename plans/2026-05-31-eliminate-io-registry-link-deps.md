...
The C runtime (`brief_rt.c`) is one possible provider of symbols — but the compiler should not know or
care which ones exist. The linker resolves symbols; the Brief source declares them. Nothing in between.
...
### New Mechanism: `import "link/brief_rt.o"`

```
import "link/brief_rt.o";   // tells the compiler driver: compile brief_rt.c and link brief_rt.o
```
...
instead of resolving as a Brief module, the compiler driver is told to include the
corresponding artifact at link time.

For `brief_rt.o` specifically, the compiler driver knows that `brief_rt.c` lives at
`runtime/brief_rt.c` (embedded via `include_str!`). It writes the C source, compiles it
...
4. The suffix (.o/.a) cleanly distinguishes "link this" from "import this Brief file"
...
pub struct LinkDependency {
    pub path: String,          // e.g., "brief_rt.o", "lib/mysensor.a"
    pub is_bundled_rt: bool,   // true if path == "brief_rt.o" → use BRIEF_RT_SOURCE
}
```
...
if dep.is_bundled_rt:
    fs::write(brief_rt.c, BRIEF_RT_SOURCE)
    cc_status = cc -c -O2 brief_rt.c -o brief_rt.o [+ -DBRIEF_THREAD_POOL]
    link_objects.push(brief_rt.o)
else:
...
**4.1 New file: `lib/std/brief_rt.bv`**
...
This is the ENTIRE replacement for `io_registry.rs` — a pure Brief file with zero compiler-side knowledge.
`import "link/brief_rt.o"` tells the compiler driver to include the runtime. The trigger declarations
...
Or keep the direct `@ link` declaration — it already works. The import from `brief_rt.bv` avoids
...
in the source files, so the single `cargo run --bin brief-compiler -- llvm benchmarks/${name}.bv --out benchmarks` command works for all benchmarks.
...
**6.3 Add** — New tests:
- `test_link_dependency_parsed` — verifies `import "link/brief_rt.o"` produces `TopLevel::LinkDependency` with `is_bundled_rt = true`
...
in the compiler driver: "I already wrote `brief_rt.c` and compiled `brief_rt.o` — skip the second one."
A `HashSet<String>` of seen link paths is sufficient.

### Circular link deps

Not possible — link deps don't produce Brief items. They don't recurse through the import resolver.

### User links a missing .o

The compiler driver emits `error: link dependency 'foo.o' not found` if the file doesn't exist on disk.
For `brief_rt.o` specifically, the source is embedded — it always exists.

### What about selfhost compilation?

The selfhost compiler (`lib/compiler/`) doesn't use `#io`, `@ link`, or `brief_rt`. No impact.
...
### User types `import "link/brief_rt.o"` but also passes `--link-rt`
...
| Stdlib file named `brief_rt.bv` but not a standard .bv library | It IS a standard library file — it imports `link/brief_rt.o` AND declares triggers. `from "std/brief_rt.bv"` works exactly like any other stdlib import. |
...
| `brief_rt.bv` triggers have `is_wake = true`, but `#io` also set it | Identical. Both paths produce `is_wake = true`. |
