# Briev Compiler - Agent Guidelines (Historical Record)
...
- **Selfhost** (deferred): `cargo run --bin briev-compiler -- selfhost <file.bv>` — self-hosting compiler relies on `list_append`/`get` magic; to be migrated to `<-` arrow syntax in Part C. Currently broken — magic handlers removed.
...
### File Types
- **.bv** - Briev (standard Briev file)
- **.rbv** - Rendered Briev (Briev + View, compiles to web frontend. Designed for web specifically. Like `.tsx` is to `.ts`)
- **.ebv** - Embedded Briev (Briev with less OS based abstractions, and more oriented towards bare metal and embedded programming)
- **.dbv/.dbvs/.dbvl** - Data Briev (configuration with schema and lines, think `.xml` compared to `.xmls` and `.json` compared to `.jsonl`)
...
Briev's liveness model follows from one principle: **a value is live if an FFI call consumes it.** Every program must eventually interact with the world — print to stdout, write to a file, send a network packet. These are all `frgn` calls.
...
Briev benchmarks answer one question: **"Can Briev compute X with competitive performance vs C?"** — not "Does Briev have feature Y?" Implement the benchmark's **semantic goal** using Briev's idioms, not a line-by-line port.

Example: knucleotide counts k-mer frequencies over a DNA sequence. C uses `int counts[4^k]` + string indexing. Briev encodes the sequence as an Int rolling hash and dispatches through 64 guarded state-field blocks — same output, different encoding. Both compute the same frequency distribution.
...
If a Briev benchmark beats C by an implausible margin, first suspect the C reference has been hobbled (volatile, unused return, artificial liveness). Fix the C reference, don't celebrate the win. The only valid victory is Briev vs C on symmetric, structurally-live programs.
...
Every C reference uses the same observable output mechanism as the Briev version.
Both get `-O3 -ffast-math` from the same clang. No `volatile`, no unused variables.
Any performance asymmetry is a signal of a missing Briev optimization — fix the compiler, not the C code.
...
Briev is a **general-purpose programming language**. The interpreter proves this — it already supports the full expression language including lists, strings, structs, enums, pattern matching, hash maps, and FFI. The standard library (`lib/std/`) has 26 modules covering strings, collections, math, I/O, JSON, HTTP, encoding, shared memory, and more.

### How Briev Works (Correct Model)

Briev's computational primitive is the **reactive transaction** (`node`). A transaction has:
- A **precondition** (guard): `[x > 0 && y < N]`
- A **postcondition** (contract): `[x == N]`
- A **body**: `{ &x = x + 1; &y = y * 2; }`

The compiler's job is to analyze the transaction graph and emit code for the most efficient execution path. This is NOT a niche reactive DSL — it IS how Briev expresses computation. Loops are transactions with bounded convergence (`[count < N][count == N]`). Recursion is a transaction chain with proved termination. Every optimization (purity folding, dead-field elimination, SROA, SLP vectorization) applies because the compiler has enough information from contracts to prove correctness.

### Misconceptions to Avoid

| Wrong | Correct |
|-------|---------|
| "Briev is a reactive state machine DSL" | Briev is a general-purpose language. Transactions are the computational primitive — they ARE loops, iteration, and recursion. |
| "Briev has no arrays/strings/collections" | The interpreter supports `List<T>`, `String`, `HashMap<K,V>`, `HashSet<T>`, `Stack<T>`, `Queue<T>`, `StringBuilder`. The stdlib has 26 modules including `collections.bv`, `string.bv`, `char.bv`, `json.bv`, etc. |
| "Briev can't do tree/heap benchmarks" | The interpreter supports recursive enum types (e.g., `enum Tree { Node(Tree, Tree), Leaf }`), struct instances, field access, and match expressions. |
| "Briev needs malloc/FFI for buffers" | No. The compiler proves bounds from contracts at compile time and allocates accordingly. The programmer writes proofs, the compiler handles memory. |
...
**Conclusion**: The interpreter supports Briev as a general-purpose language. Recursive-defn safety is the only meaningful gap.
...
8. **Never weaken C benchmarks**: Every asymmetry between Briev and C is a signal of a missing Briev optimization. Never hobble C with `volatile` or artificial liveness hacks. Fix Briev to match or beat C's optimization.
...
10. **Benchmarks on our own terms**: Briev benchmarks compare end-to-end results (Input X → Output Y). The compiler chooses the optimal execution path. Adding features for benchmarks is fine IF they add value to the language. Never add features solely to run benchmarks.

## Self-Hosting Pipeline

The Briev-in-Briev compiler lives in `lib/compiler/`. The Rust interpreter runs it via:
```
briev-compiler selfhost <file.bv>
```
...
**Current**: 400 tests pass. Phases 1-4 complete plus Phase 5 (DBVS import pipeline), N3 (PGO), and N2 (equality saturation). Briev wins or ties on 8 of 9 benchmarks. print_loop at 1.63× of C.
...
| Benchmark | Path | Briev | C | Ratio |
|-----------|------|-------|---|-------|
| iir_filter | Dead-field elim + pure counter | **0.001s** | 0.084s | **Briev wins** |
...
| float_math | alloca+SROA + fast-math + -O3 | **0.004s** | 0.006s | **Briev ~1.5×** |
| float_math_nonzero | alloca+SROA + fast-math + -O3 + AVX | **0.162s** | 0.165s | **Briev ~1.02×** |
| sparse_dispatch | Dispatch-chain collapse | 0.001s | 0.001s | ~tie |
| const_heavy | Integer arithmetic (sdiv) | 0.001s | 0.034s | **Briev wins** |
| print_loop | **FFI-based structurally-live** | **0.030s** | 0.049s | **Briev 1.63×** |
| nbody_newton | Custom Newton sqrt inlined vs C sqrtf | **3.62s** | 9.73s | **Briev 2.7×** |
| nbody_sqrt | Both use sqrtf, Briev loses on call overhead | 6.96s | 3.23s | C 2.15× |
...
| knucleotide | Rolling 2-bit hash + FFI output | **0.188s** | 0.194s | **Briev 0.97×** |
...
- **Phase 5a**: Thread pool primitives in `runtime/briev_rt.c` — portable barrier (mutex+cond+counter, works on macOS), `briev_thread_pool_init/release/wait/shutdown`, gated behind `#if defined(BRIEV_THREAD_POOL)`
- **Phase 5b**: Builtin declares (`briev_thread_pool_init`, `briev_barrier_release`, `briev_barrier_wait`)
...
- **Phase 5f**: `main.rs` link step — detects `@llvm.thread_pool`, adds `-DBRIEV_THREAD_POOL -lpthread`
...
- **Step 5f**: `main.rs` link step — detects `@llvm.thread_pool`, adds `-DBRIEV_THREAD_POOL -lpthread`
- **Step 6**: Eliminate `io_registry.rs` and `#io` pragma — replaced by `import "link/briev_rt.o"` auto-dependency mechanism
  ...
  - New `lib/std/briev_rt.bv` — declares all `@ link` triggers as pure Briev code
  - `lib/std/system.bv` rewritten to import from `briev_rt.bv` (no more `#io`)
...
| kalman_filter | SLP hazard + opt -O2 pipeline | 0.71s | 0.75s | **Briev beats C by ~5%** |

### .gitignore / Infrastructure Cleanup
- All benchmark build artifacts (`*.o`, `*.ll`, binaries, generated `briev_rt.c`) now ignored
...
- **Problem**: C compiler proved IIR filter's non-volatile float delay-line state (x1/x2/y1/y2) is never observed, eliminated all 50M biquad iterations, leaving only `volatile long count` incq loop (0.09s). Briev emitted the full body verbatim (0.15s).
- **Root Cause**: Briev had no liveness analysis — every state store was emitted regardless of whether the field value was ever consumed.
...
- **Result**: Briev ties or beats C on all 4 benchmarks when both get equal compiler optimization
...
- **`try_lto_pipeline()`** in `src/main.rs`: Compiles `briev_rt.c` to LLVM bitcode via `clang -c -emit-llvm`, merges with program IR via `llvm-link`, runs `opt -O3` on the merged module. Enables inlining of `__print_int`, `__wait_for_event`, and thread pool barriers into Briev loops.
...
- **`__print_int` + `__print` added to `runtime/briev_rt.c`**: Plain C functions (`int64_t __print(const char*)`, `int64_t __print_int(int64_t)`, `void __exit(void)`). `fputs` to stdout, `fprintf(stderr, "%lld\n")`, `exit(0)`.
...
- **Kalman verification**: n=12, C=72, T=12, K=18, R=16, W=4 → peak=28 ≥ 16 → SLP disabled. Briev 0.71s vs C 0.75s (Briev beats C by ~5%).
...
- Included via `import "link/<lib>/<lib>.c"` in Briev source; compiled to bitcode via LTO
...
- `test_xxhash_compiles`, `test_yyjson_compiles`, `test_briev_json_compiles`
...
- `docs/BRIEV_3.0_SPEC.md` — added Section 11
- `learn-briev/05-data-types.md` — added Section 8
- `learn-briev/README.md` — updated TOC
- `lib/runtime/briev_rt.c` — unchanged (pre-existing change)
...
**Why it's on the edge**: `__chr_to_str` is a C function in `briev_rt.c` that
the user never declared with `frgn`. If `briev_rt.c` isn't linked, the program
...
### Goal
Rename `.gbv` → `.abv` (Graphic Briev → Accelerated Briev / "Briev Accel") and
`.hebv` → `.cbv` (Hardware Embedded Briev → Circuit Briev / "Briev Circuit")
across the entire codebase, including alternative names for all tiers.

### Changes

**Rust source files:**
- `src/main.rs`: All 5 `.gbv` → `.abv`, 3 `.hebv` → `.cbv`, "Graphic Briev" → "Accelerated Briev",
  "Hardware Embedded Briev" → "Circuit Briev" in help text, error messages, comments
...
**Documentation:**
- `AGENTS.md`: All file types updated with `.abv`/`.cbv` + alternative names ("Briev Accel",
  "Briev Render", "Briev Embed", "Briev Circuit", "D-Briev"/"Briev Data"); sugar rules updated
- `README.md`: Table expanded with `.abv` and `.cbv` rows + alternative names
- `docs/architecture/features/graphic-briev.md` → `accelerated-briev.md` (content fully updated)
- `docs/plans/2026-06-18-graphic-briev.md`: Updated with `.abv` and completion status
- `docs/plans/2026-06-18-gpu-io-intrinsics.md`: Updated `.gbv` → `.abv`, `test_gbv` → `test_abv`
- `syntax-highlighter/syntaxes/briev.tmLanguage.json`: `fileTypes` array expanded to all 12 extensions

**Not changed (intentionally):**
- Internal GPU-mode names (`StrictMode::Gpu`, `is_gpu_extension`, `with_gpu_mode`) — these
  describe the compilation mode, not a brand name. Regular Briev can declare GPU-accelerated
  code without a `.abv` file.
- `learn-briev/` and `docs/reference/BRIEV_LANGUAGE_REFERENCE.md` and `spec/SPEC.md` — had
  zero old references
...
Officina-cli is a Briev terminal application that takes keyboard input. The Char→String cast
...
- `learn-briev/12-pragmas.md` — Annotation Arrow + `#hashtag` shorthand
