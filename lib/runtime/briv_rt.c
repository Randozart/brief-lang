/*
 * briv_rt.c — Minimal runtime for Briv LLVM backend
 *
 * 2026-07-15: Stripped ~70 briv_* wrapper functions that were replaced
 ...
 * the two remaining intrinsics: briv_syscall, briv_sysconf.
 */
...
#ifndef _BRIV_INT_DEFINED
#define _BRIV_INT_DEFINED
#if defined(__LP64__) || defined(_WIN64)
typedef int64_t briv_int;
#else
typedef int32_t briv_int;
#endif
#endif
...
char* briv_str_to_c(const char* handle) {
    ...
    // (CStr → String is marshalled through cstr_to_briv at the boundary), so
...
/// Caller should free via briv_free_briv_str().
char* briv_cstr_to_briv(const char* c_str) {
    if (!c_str) return 0;
...
char* briv_str_substr(const char* s, int64_t a, int64_t b) {
    if (!s) return 0;
...
// briv_str_substr — a 1-byte allocation per scan step was corrupting the heap
// under long recursion (the frgn String-return path, see BUGS.md). No
// allocation.
int64_t briv_str_char_at(const char* s, int64_t i) {
    if (!s) return 0;
    int64_t len = *(const int64_t*)s;
...
// [len: i64][bytes], heap-allocated, freed via briv_free_briv_str) and a C
...
char* bool_to_str(int64_t b) {
    return briv_cstr_to_briv(b ? "true" : "false");
}
...
int64_t str_first_char(const char* s) {
    ...
    // are valid UTF8; continuation bytes are skipped the same way briv_char_len
    // counts them).
...
/// Free a Briv string allocated by briv_cstr_to_briv or similar.
void briv_free_briv_str(void* handle) {
...
// length header + payload into a fresh heap buffer. This is NOT briv_cstr_to_briv
...
// briv_free_briv_str. Sub-protocols override the lane via CastFrom(#Bit).
char* briv_bits_to_str(const char* bits) {
    if (!bits) return 0;
...
int64_t briv_char_len(const char* str) {
    if (!str) return 0;
...
int64_t briv_str_eq(const char* a, const char* b) {
    if (a == b) return 1;
    if (!a || !b) return 0;
...
// briv_free_briv_str. These are the runtime half of the #String bitwise
// defaults; the compiler emits a call instead of treating String ptrs as ints.
static char* briv_str_bitop(const char* a, const char* b, int op) {
    if (!a) return 0;
    int64_t la = *(const int64_t*)a;
...
char* briv_str_band(const char* a, const char* b) { return briv_str_bitop(a, b, 0); }
char* briv_str_bor(const char* a, const char* b)  { return briv_str_bitop(a, b, 1); }
char* briv_str_bxor(const char* a, const char* b) { return briv_str_bitop(a, b, 2); }
char* briv_str_bnot(const char* a)                { return briv_str_bitop(a, 0, 3); }
...
// (heap-allocated; caller frees via briv_free_briv_str).
// The compiler's emitted `main(i32 %argc, ptr %argv)` stores its arguments
...
// briv_free_briv_str).
extern int32_t __briv_argc;
extern void* __briv_argv;
...
char* __argv_get(int64_t i) {
    if (!__briv_argv || i < 0 || i >= __briv_argc) {
        return briv_cstr_to_briv("");
    }
    char* s = ((char**)__briv_argv)[i];
    return briv_cstr_to_briv(s);
}
...
int64_t __argv_has(const char* flag_bstr) {
    char* c_flag = briv_str_to_c(flag_bstr);
    if (!c_flag) return 0;
    int64_t found = 0;
    if (__briv_argv) {
        for (int64_t i = 1; i < __briv_argc; i++) {
            if (strcmp(((char**)__briv_argv)[i], c_flag) == 0) {
                found = 1;
                break;
            }
        }
...
char* __argv_value(const char* flag_bstr) {
    char* c_flag = briv_str_to_c(flag_bstr);
    if (!c_flag) return briv_cstr_to_briv("");
    char* result = NULL;
    if (__briv_argv) {
        for (int64_t i = 1; i + 1 < __briv_argc; i++) {
            if (strcmp(((char**)__briv_argv)[i], c_flag) == 0) {
                result = ((char**)__briv_argv)[i + 1];
                break;
            }
        }
    }
    free(c_flag);
    if (!result) return briv_cstr_to_briv("");
    return briv_cstr_to_briv(result);
}

// The first non-flag token in argv[1..] — the subcommand. `<prog> --verbose
// build` → "build"; "" if none. Honors $BRIV_ENTRY_CMD (test/embedded path
// without argv) as the sole environment fallback.
char* __argv_command(void) {
    const char* env_cmd = getenv("BRIV_ENTRY_CMD");
    if (env_cmd && env_cmd[0]) {
        return briv_cstr_to_briv(env_cmd);
    }
    if (__briv_argv) {
        for (int64_t i = 1; i < __briv_argc; i++) {
            const char* tok = ((char**)__briv_argv)[i];
            if (tok[0] != '-') {
                return briv_cstr_to_briv(tok);
            }
        }
    }
    return briv_cstr_to_briv("");
...
char* __getenv_briv(const char* key_bstr) {
    char* c_key = briv_str_to_c(key_bstr);
    if (!c_key) return 0;
...
int64_t briv_syscall(int64_t num, int64_t a1, int64_t a2, int64_t a3, int64_t a4, int64_t a5, int64_t a6) {
    return syscall((long)num, (long)a1, (long)a2, (long)a3, (long)a4, (long)a5, (long)a6);
}

int64_t briv_sysconf(int64_t name) {
...
int64_t __print(const char* msg_bstr) {
    char* c_msg = briv_str_to_c(msg_bstr);
...
void briv_thread_pool_shutdown(void);
...
//   3. briv_thread_pool_shutdown joins all workers.
...
int64_t __read_file__(const char* path_bstr) {
    // 2026-08-03 (P2): briv_str_to_c returns the IN-PLACE data pointer (the
    // composite) for heap Strings — arena-owned, must NOT be freed.
    char* c_path = briv_str_to_c(path_bstr);
    if (!c_path) return -1;
...
int64_t __write_file__(const char* path_bstr, const char* data_bstr) {
    ...
    char* c_path = briv_str_to_c(path_bstr);
    char* c_data = briv_str_to_c(data_bstr);
...
int64_t briv_ttyname(int64_t fd) {
...
void __watchdog_fail(void) {
    fprintf(stderr, "briv: required watchdog fired\n");
    exit(1);
}

// 2026-08-01 (D2): garbage-scheduling calibration. The scheduler's scheduled
// frees route through __briv_free so a benchmark can assert frees == allocs
// (no premature free, no leak). __briv_free_count() is the observable getter.
static long __briv_free_total = 0;

void __briv_free(void* p) {
    if (p) __briv_free_total++;
    free(p);
}

long __briv_free_count(void) {
    return __briv_free_total;
}
...
char* briv_cstring_concat(const char* a, const char* b) {
...
uint8_t* briv_mask_select(const uint8_t* data, const int64_t* mask, int64_t mask_len) {
...
/// Typed Boolean mask select over an i64-slot array (a Briv vector state
...
int64_t* briv_mask_select64(const int64_t* data, int64_t data_len,
