#!/usr/bin/env python3
"""GLUE native-speed benchmark: Briev (.so) vs C (.so) via ctypes.
...
across three paths:
  ...
  Python -> Briev .so   (GLUE c_abi path, generated library)
...
def main():
    ...
    briev = load(os.path.join(LIB_DIR, "librank.so"))
    ref = load(os.path.join(LIB_DIR, "librank_ref.so"))

    # GLUE library ABI: __briev_init_state() -> handle; exports take it first
    # when they need state (feature_hash does — it runs a runtime loop).
    briev.__briev_init_state.restype = ctypes.c_void_p
    st = briev.__briev_init_state()
    briev.feature_hash.argtypes = [ctypes.c_void_p, ctypes.c_int64, ctypes.c_int64]
    briev.feature_hash.restype = ctypes.c_int64
    briev.add.argtypes = [ctypes.c_int64, ctypes.c_int64]
    briev.add.restype = ctypes.c_int64
    ...
    seed = 42
    bh = briev.feature_hash(st, count, seed)
    ch = ref.feature_hash_c(count, seed)
    if bh != ch:
        print(f"FATAL: output mismatch briev={bh} c={ch}")
        sys.exit(1)
    print(f"output: feature_hash({count}, {seed}) = {bh} (briev == c)")
    ...
    bm, bmean = bench(briev.feature_hash, st, count, seed)
    cm, cmean = bench(ref.feature_hash_c, count, seed)
    am, amean = bench(briev.add, 3, 4)

    print(f"  Python -> Briev feature_hash : median={bm:7.0f} ns  mean={bmean:7.0f} ns")
    print(f"  Python -> C    feature_hash : median={cm:7.0f} ns  mean={cmean:7.0f} ns")
    print(f"  Python -> Briev add (pure)  : median={am:7.0f} ns  mean={amean:7.0f} ns")
    ratio = cm / bm if bm else float("nan")
    print(f"\n  Briev vs C per-call overhead: {ratio:.2f}x (C faster when < 1)")

    print("\nnote: the Briev/C gap is ctypes marshalling + state-arg overhead,"
