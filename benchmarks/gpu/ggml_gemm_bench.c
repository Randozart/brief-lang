// ggml_gemm_bench.c — M2 anchor (plan 2026-09-01-m2-gemm): 4096³ fp32 GEMM
...
int main(int argc, char** argv) {
    ...
    for (uint64_t j = 0; j < K * N; j++) bd[j] = (float)(j % 5) * 0.5f;
    ...
    if (cuda != NULL) {
        ...
        struct ggml_tensor* db = ggml_new_tensor_2d(dctx, GGML_TYPE_F32, K, N);
    ...
    printf("fingerprint: ggml mul_mat F32 M=%llu N=%llu K=%llu iters=20\n",
