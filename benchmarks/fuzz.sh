#!/usr/bin/env bash
# Fuzz Runner for Brief Optimization Benchmarks
#
# Two modes:
...
#     brief: mean=0.045s median=0.044s min=0.042s max=0.052s sigma=0.0023
#     c:     mean=0.042s median=0.041s min=0.040s max=0.048s sigma=0.0018
#     ratio: 1.07x (brief is 7% slower)
...
COMPILER="./target/release/brief-compiler"
if [ ! -x "$COMPILER" ]; then
...
bench_runtime() {
    ...
    # Compile once
    local brief_src="benchmarks/${BENCH}_runtime.bv"
    local c_src="benchmarks/${BENCH}_runtime_c.c"
    local brief_bin="benchmarks/${BENCH}_runtime_brief"
    ...
    $COMPILER llvm "$brief_src" --out /tmp --optimize-budget 256 2>&1 | tail -1
    clang -O3 -march=native "/tmp/${BENCH}_runtime.ll" -o "$brief_bin" -lm 2>&1 | tail -1
    ...
    for i in $(seq 1 "$RUNS"); do
        ...
        # Run Brief
        local bt=$( (export BOUND="$bound"; /usr/bin/time -f "%e" "$brief_bin" 2>&1) 2>/dev/null | tail -1)
        brief_times+=("$bt")
    ...
    local b_stats=($(compute_stats "${brief_times[@]}"))
    local c_stats=($(compute_stats "${c_times[@]}"))

    local brief_mean="${b_stats[0]}" brief_median="${b_stats[1]}" brief_min="${b_stats[2]}" brief_max="${b_stats[3]}" brief_sigma="${b_stats[4]}"
    local c_mean="${c_stats[0]}" c_median="${c_stats[1]}" c_min="${c_stats[2]}" c_max="${c_stats[3]}" c_sigma="${c_stats[4]}"

    local ratio=$(awk "BEGIN { if ($c_mean > 0) print $brief_mean / $c_mean; else print 0 }")

    echo "    brief: mean=${brief_mean}s median=${brief_median}s min=${brief_min}s max=${brief_max}s sigma=${brief_sigma}"
...
bench_compile_time() {
    ...
    local brief_tmpl="benchmarks/${BENCH}.bv"
    local c_tmpl="benchmarks/${BENCH}_c.c"
    local brief_src="/tmp/${BENCH}_fuzz.bv"
    local c_src="/tmp/${BENCH}_fuzz.c"
    local brief_bin="/tmp/${BENCH}_fuzz_brief"
    local c_bin="/tmp/${BENCH}_fuzz_c"

    if [ ! -f "$brief_tmpl" ]; then
        echo "  ERROR: $brief_tmpl not found"
        return 1
    fi
    ...
    for i in $(seq 1 "$RUNS"); do
        ...
        sed "s/const [A-Z][A-Za-z]*: Int = [0-9]*;/const N: Int = $bound;/" \
            "$brief_tmpl" > "$brief_src" 2>/dev/null || \
        sed "s/const total: Int = [0-9]*;/const total: Int = $bound;/" \
            "$brief_tmpl" > "$brief_src" 2>/dev/null || true
        ...
        # Compile Brief
        $COMPILER llvm "$brief_src" --out /tmp --optimize-budget 256 2>&1 | tail -1
        local llfile="/tmp/$(basename "$brief_src" .bv).ll"
        clang -O3 -march=native "$llfile" -o "$brief_bin" -lm 2>&1 | tail -1
        ...
        # Time
        local bt=$(/usr/bin/time -f "%e" "$brief_bin" 2>&1 | tail -1)
        brief_times+=("$bt")
    ...
    echo "    brief: mean=${b_stats[0]}s median=${b_stats[1]}s min=${b_stats[2]}s max=${b_stats[3]}s sigma=${b_stats[4]}"
