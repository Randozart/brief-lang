// nbody_sqrt_idio_c — 5-body gravity sim, energy computed every iteration
// Idiomatic C reference: matches nbody_sqrt_idio.bv exactly.
// Energy (10 extra sqrt calls) computed each tick inside the loop.
//
// clang -O3 -march=native -ffast-math -o benchmarks/nbody_sqrt_idio_c \
//     benchmarks/nbody_sqrt_idio_c.c -lm
...
int main(void) {
    ...
    for (; count < total; count++) {
        ...
        // Energy computation every iteration — matches idiomatic Briev
        float energy = 0.0f;
        #define EPAIR(ia, ib) { \
            float dx = bx[ia] - bx[ib]; \
            float dy = by[ia] - by[ib]; \
