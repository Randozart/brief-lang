...
| **Brief GLUE** | `brief_test_cstr_roundtrip(input_ptr: Int) -> Int` from `pp-types.bv` | Python → ctypes → `.so` → Brief → Py |
...
| `benchmarks/bridge/bridge_tests.bv` | Brief bridge test file |
| `benchmarks/bridge/Makefile` | Build: C `.so`, Brief `.so`, optionally Rust `.so` |
...
1. Builds all three `.so` variants:
   ...
   brief export bridge_tests.bv rust --out /tmp/br/
