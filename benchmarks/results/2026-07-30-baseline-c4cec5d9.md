...
| float_math | .0065s | .0733s | .08× | Briev | MATCH |
| float_math_nonzero | .2137s | .1670s | 1.27× | C | MATCH |
| sparse_dispatch | .0513s | .0612s | .83× | Briev | MATCH |
| print_loop | .0001s | .0586s | 0× | Briev | MATCH |
| **nbody_newton** | **6.9434s** | **8.2763s** | **.83×** | **Briev** | **MATCH** |
| nbody_sqrt | 2.1625s | 2.8141s | .76× | Briev | MATCH |
| nbody_sqrt_idio | 2.6355s | 3.6471s | .72× | Briev | MATCH |
| fasta | .2094s | .2100s | .99× | Briev | MATCH |
| fannkuch_redux | .0621s | .0639s | .97× | Briev | MATCH |
...
| cancel_math | .0004s | .0641s | 0× | Briev | MATCH |
| bit_clear | .0002s | .0002s | 1.00× | ~tie | MATCH |
| queue_drain | .0030s | .0623s | .04× | Briev | MATCH |
| queue_drain_sym | .0004s | .0608s | 0× | Briev | MATCH |
| queue_drain_idio | 0s | .0604s | 0× | Briev | MATCH |
| interval_step | .0629s | .0640s | .98× | Briev | MATCH |
| bridge_glue | done | — | — | — | SKIP |
| bridge_multi | done | — | — | — | PASS |

## Summary

**Primary achievement:** nbody_newton improved from 1.22× C to **0.83× C** (Briev beats C by 17%) by fixing the `split_hoistable` safety check to:
...
- **mandelbrot** (1.04× C, briev output differs from C)
- **knucleotide** (1.20× C, briev output differs from C)
...
- Briev-level LICM (hoist loop-invariant let-bindings)
