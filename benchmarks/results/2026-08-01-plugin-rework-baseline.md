...
**Worktree:** `../briev-compiler-plugin-rework`, branch `feat/plugin-macro-rework`
...
**Baseline reference worktree:** `../briev-compiler-baseline` at `d6c6c818`
...
| float_math | .0436s | .0720s | .60× | Briev | MATCH |
| float_math_nonzero | .1590s | .1662s | .95× | Briev | MATCH |
| sparse_dispatch | .0502s | .0615s | .81× | Briev | MATCH |
| print_loop | .0362s | .0609s | .59× | Briev | MATCH |
| nbody_newton | 7.6315s | 9.1267s | .83× | Briev | MATCH |
| nbody_sqrt | 2.4004s | 3.1238s | .76× | Briev | MATCH |
| nbody_sqrt_idio | 2.9454s | 3.9006s | .75× | Briev | MATCH |
| fasta | .2422s | .2267s | 1.06× | C | MATCH |
| fannkuch_redux | .0604s | .0641s | .94× | Briev | MATCH |
| mandelbrot | .7117s | .6932s | 1.02× | C | MATCH |
| kalman_filter_runtime | .1522s | .1780s | .85× | Briev | MATCH |
| knucleotide | .1894s | .1911s | .99× | Briev | MATCH |
| cancel_math | .0503s | .0590s | .85× | Briev | MATCH |
| bit_clear | .0005s | .0004s | 1.25× | C | MATCH |
| interval_step | .0597s | .0596s | 1.00× | ~tie | MATCH |
| telemetry_stream | .1919s | .1987s | .96× | Briev | MATCH |
| pid_control | .3432s | .3486s | .98× | Briev | MATCH |
| matrix_pipeline | .4630s | .7407s | .62× | Briev | MATCH |
| accumulator_flush | .1072s | .1489s | .71× | Briev | MATCH |
...
The runtime helpers (`briev_rt.c:178-199`) are `always_inline` and the harness
links `.ll` + `briev_rt.c` with `-O3 -flto`, so these inline into `main`'s hot
...
```llvm
%t0 = call i64 @__getenv_int(ptr %t2)       ; briev_rt.c: `__getenv_int(int64_t key_bstr)`
%t0 = call ptr @__getenv_briev(ptr %t2)
```

`ptr` is the 8-byte ABI equivalent of the C `int64_t` param. briev_rt.c
`__getenv_int`/`__getenv_briev` (lines 137-160) decode the length-prefixed Briev
...
`briev-compiler` (renamed to `brievc` long ago) — baseline builds would fail.
Fixed to `brievc`. Also `build_and_bench.sh` `bridge_multi` node bench failed
