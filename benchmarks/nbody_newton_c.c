// nbody_newton_c — 5-body gravity sim with Newton's method sqrt (C reference)
// Mirrors nbody_newton.bv exactly: Newton iterations, 10 unrolled pairs, f32.
// Fair test: both sides use same algorithm.
//
// clang -O3 -march=native -ffast-math -o benchmarks/nbody_newton_c benchmarks/nbody_newton_c.c
...
int main(void) {
    ...
    for (; count < total; count++) {
        // 10 unrolled pairs — identical structure to nbody_newton.bv
    ...
    // Energy: compute fresh distances with Newton sqrt
    float energy = 0.0f;
    #define EPAIR(ia, ib) { \
