...
fn generate_with_exports(items: &[crate::ast_new::TopLevel], _file_path: &str) -> Result<String, String> {
    ...
    for item in items {
        match item {
            crate::ast_new::TopLevel::Export(export) => {
                if let crate::ast_new::TopLevel::Definition(defn) = export.inner.as_ref() {
                    emit_export_wrapper(defn, &mut out)?;
                }
            }
            crate::ast_new::TopLevel::Definition(defn) => {
...
fn emit_definition(defn: &crate::ast_new::Definition, out: &mut String) -> Result<(), String> {
    use std::fmt::Write;
...
fn emit_export_wrapper(defn: &crate::ast_new::Definition, out: &mut String) -> Result<(), String> {
    use std::fmt::Write;
