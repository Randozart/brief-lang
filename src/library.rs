...
fn parse_and_check(file_path: &str, source: &str) -> Result<Vec<brief_compiler::ast::TopLevel>, String> {
    ...
    let lexer = brief_compiler::lexer::Token::lexer(source);
    let tokens: Vec<(brief_compiler::lexer::Token, std::ops::Range<usize>)> = lexer
        .map(|r| (r.unwrap(), 0..0))
        .collect();
...
fn generate_with_exports(items: &[brief_compiler::ast::TopLevel], _file_path: &str) -> Result<String, String> {
    ...
    for item in items {
        match item {
            brief_compiler::ast::TopLevel::Export(export) => {
                if let brief_compiler::ast::TopLevel::Definition(defn) = export.inner.as_ref() {
                    emit_export_wrapper(defn, &mut out)?;
                }
            }
            brief_compiler::ast::TopLevel::Definition(defn) => {
...
fn emit_definition(defn: &brief_compiler::ast::Definition, out: &mut String) -> Result<(), String> {
    use std::fmt::Write;
    let params: Vec<String> = defn.parameters.iter()
        .map(|(name, ty)| format!("{} %{}", brief_compiler::backend::llvm::types::lower_type(ty), name))
        .collect();
...
fn emit_export_wrapper(defn: &brief_compiler::ast::Definition, out: &mut String) -> Result<(), String> {
    use std::fmt::Write;
