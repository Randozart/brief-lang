...
use crate::ast::top::{CastDirection, CastEdge, CastBinding, OperatorDef, ProtocolDef, TopLevel};
...
// 2026-07-23: Proves CastTo/CastFrom round-trip identity and
// cross-op equivalence using defn inlining + symbolic/SMT.
...
pub fn verify_protocol_roundtrip(
    ...
    // Find matching CastTo/CastFrom with same target
    ...
    match (forward_body, inverse_body) {
        (Some(fwd), Some(inv)) => {
            // Build symbolic composition: inv(fwd(x))
            let sym_input = {
...
pub fn verify_crossop_equivalence(
    ...
    let (to_edge, from_edge) = match (to, from) {
        (Some(t), Some(f)) => (t, f),
        _ => return Ok(()), // no round-trip path to compare against
    };
    ...
    for op in &pd.cross_ops {
        ...
        // Find defn bodies for CastTo/CastFrom
        let to_fn = to_edge.binding.as_ref().map(|b| &b.fn_name).unwrap();
        let from_fn = from_edge.binding.as_ref().map(|b| &b.fn_name).unwrap();
