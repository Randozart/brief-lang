...
impl Annotator {
    ...
    fn collect_calls_from_expr(&self, expr: &Expr, calls: &mut Vec<(String, usize)>) {
        match expr {
            ...
            | Expr::Gt(l, r) | Expr::Ge(l, r) | Expr::Or(l, r) | Expr::And(l, r) => {
                self.collect_calls_from_expr(l, calls);
                self.collect_calls_from_expr(r, calls);
            }
    ...
    pub fn annotate_program(&self, program: &Program) -> String {
        ...
        let cleaned_comments: Vec<&Comment> = program.comments.iter()
            .filter(|c| {
                let text = &c.text;
    ...
    fn type_to_string(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Union(types) => types.iter().map(|t| self.type_to_string(t)).collect::<Vec<_>>().join(" | "),
            Type::ContractBound(inner, _) => self.type_to_string(inner),
        }
    }

    fn format_definition(&self, defn: &Definition) -> String {
        let params: Vec<String> = defn.parameters
            .iter()
    ...
    fn format_state_decl(&self, state: &StateDecl) -> String {
        ...
        format!("let {}: {}{};\n", state.name, self.type_to_string(&state.ty), init)
    }

    fn format_constant(&self, const_decl: &Constant) -> String {
        format!("const {}: {} = {};\n", const_decl.name, 
            self.type_to_string(&const_decl.ty), self.format_expr(&const_decl.expr))
    }

    fn format_body(&self, stmts: &[Statement]) -> String {
        stmts.iter().map(|s| self.format_statement(s, 1)).collect::<Vec<_>>().join("")
    }
    ...
    fn format_statement(&self, stmt: &Statement, indent: usize) -> String {
        ...
        match stmt {
            ...
            Statement::Assignment { is_owned, name, expr } => {
                ...
                format!("{}{}{} = {};\n", spaces, prefix, name, self.format_expr(expr))
            }
            ...
            Statement::Unification { name, pattern, expr } => {
                format!("{}{}({}) = {};\n", spaces, name, pattern, self.format_expr(expr))
            }
        }
    }
