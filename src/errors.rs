    ...
    pub fn format(&self, source: &str, file_name: &str) -> String {
        ...
        if let Some(span) = &self.span {
            output.push_str(&format!(
                ...
                self.title,
                self.code,
                if self.code.is_empty() { "".to_string() } else { ", ".to_string() },
                file_name,
                span
            ));
        } else {
        ...
        if let Some(span) = &self.span {
            if self.severity == Severity::Error {
                ...
                if span.line > 0 && span.line <= lines.len() {
                    ...
                    output.push_str(&format!("{} | {}\n", padding, " ".repeat(span.column.saturating_sub(1)) + "^"));
                }
            }
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TypeError::TypeMismatch { expected, found, context, .. } => {
                write!(f, "type mismatch: expected {} for {}, found {}", expected, context, found)
            }
            ...
            TypeError::InvalidOperation { operation, type_name, .. } => {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ProofError::UnreachableState { transaction, .. } => {
                write!(f, "transaction '{}' has unreachable precondition", transaction)
            }
            ProofError::PostconditionUnsatisfiable { transaction, .. } => {
                write!(f, "transaction '{}' postcondition cannot be satisfied", transaction)
            }
            ...
            ProofError::MutualExclusionViolation { txn1, txn2, .. } => {
                write!(f, "transactions '{}' and '{}' have unsafe concurrent access", txn1, txn2)
            }
            ...
            ProofError::PostconditionMutationViolation { transaction, .. } => {
                write!(f, "transaction '{}' postcondition references mutated state incorrectly", transaction)
            }
        }
    }
}
