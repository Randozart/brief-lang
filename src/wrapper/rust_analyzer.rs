...
fn parse_rust_function(line: &str) -> Option<AnalyzedFunction> {
    ...
    if !params_str.trim().is_empty() {
        for param in params_str.split(',') {
            ...
            if parts.len() >= 2 {
                ...
                parameters.push((p_name, rust_type_to_briev(&p_type)));
            }
        }
    }
    ...
    let return_type = if let Some(arrow_pos) = rest.find("->") {
        ...
        rust_type_to_briev(&ret[..ret_end].trim())
    } else {
        "Void".to_string()
    };
...
/// Convert Rust type to Briev type
fn rust_type_to_briev(rust_type: &str) -> String {
    ...
    match t {
        ...
        _ => {
            if t.starts_with("Vec<") {
                ...
                format!("List<{}>", rust_type_to_briev(inner))
            } else if t.starts_with("Option<") {
                let inner = &t[7..t.len() - 1];
                format!("Maybe<{}>", rust_type_to_briev(inner))
            } else {
                format!("Custom({})", t)
            }
        }
...
/// Convert Rust function to Briev frgn sig format
