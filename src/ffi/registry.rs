...
fn resolve_location_to_impl(location: &str) -> Option<ForeignFn> {
    let func: fn(Vec<Value>) -> Result<Value, RuntimeError> = match location {
        // ===== Built-in collection/type operations (__builtin namespace) =====
        // HashMap
        "__builtin.HashMap.new" => |args| Ok(Value::HashMap(HashMap::new())),
        "__builtin.HashMap.insert" => |args| {
            if let (Value::HashMap(map), Value::String(key)) = (&args[0], &args[1]) {
                let mut map = map.clone();
                map.insert(key.clone(), args[2].clone());
                Ok(Value::HashMap(map))
            } else { Err(RuntimeError::TypeMismatch("HashMap::insert expects (HashMap, String, Value)".into())) }
        },
        "__builtin.HashMap.get" => |args| {
            if let (Value::HashMap(map), Value::String(key)) = (&args[0], &args[1]) {
            ...
            } else { Err(RuntimeError::TypeMismatch("HashMap::get expects (HashMap, String)".into())) }
        },
        "__builtin.HashMap.contains_key" => |args| {
            if let (Value::HashMap(map), Value::String(key)) = (&args[0], &args[1]) {
                Ok(Value::Bool(map.contains_key(key)))
            } else { Err(RuntimeError::TypeMismatch("HashMap::contains_key expects (HashMap, String)".into())) }
        },
        "__builtin.HashMap.remove" => |args| {
            if let (Value::HashMap(map), Value::String(key)) = (&args[0], &args[1]) {
                let mut map = map.clone();
                map.remove(key);
                Ok(Value::HashMap(map))
            } else { Err(RuntimeError::TypeMismatch("HashMap::remove expects (HashMap, String)".into())) }
        },
        "__builtin.HashMap.len" => |args| {
            if let Value::HashMap(map) = &args[0] { Ok(Value::Int(map.len() as i64)) }
            else { Err(RuntimeError::TypeMismatch("HashMap::len expects HashMap".into())) }
        },
        "__builtin.HashMap.is_empty" => |args| {
            if let Value::HashMap(map) = &args[0] { Ok(Value::Bool(map.is_empty())) }
            else { Err(RuntimeError::TypeMismatch("HashMap::is_empty expects HashMap".into())) }
        },
        "__builtin.HashMap.keys" => |args| {
            if let Value::HashMap(map) = &args[0] {
                let keys: Vec<Value> = map.keys().map(|k| Value::String(k.clone())).collect();
                Ok(Value::List(keys))
            } else { Err(RuntimeError::TypeMismatch("HashMap::keys expects HashMap".into())) }
        },
        "__builtin.HashMap.values" => |args| {
            if let Value::HashMap(map) = &args[0] {
                let values: Vec<Value> = map.values().cloned().collect();
                Ok(Value::List(values))
            } else { Err(RuntimeError::TypeMismatch("HashMap::values expects HashMap".into())) }
        },

        // HashSet
        "__builtin.HashSet.new" => |_args| Ok(Value::HashSet(std::collections::HashSet::new())),
        "__builtin.HashSet.insert" => |args| {
            if let (Value::HashSet(set), Value::String(item)) = (&args[0], &args[1]) {
                let mut set = set.clone();
                set.insert(item.clone());
                Ok(Value::HashSet(set))
            } else { Err(RuntimeError::TypeMismatch("HashSet::insert expects (HashSet, String)".into())) }
        },
        "__builtin.HashSet.contains" => |args| {
            if let (Value::HashSet(set), Value::String(item)) = (&args[0], &args[1]) {
                Ok(Value::Bool(set.contains(item)))
            } else { Err(RuntimeError::TypeMismatch("HashSet::contains expects (HashSet, String)".into())) }
        },
        "__builtin.HashSet.remove" => |args| {
            if let (Value::HashSet(set), Value::String(item)) = (&args[0], &args[1]) {
                let mut set = set.clone();
                set.remove(item);
                Ok(Value::HashSet(set))
            } else { Err(RuntimeError::TypeMismatch("HashSet::remove expects (HashSet, String)".into())) }
        },
        "__builtin.HashSet.len" => |args| {
            if let Value::HashSet(set) = &args[0] { Ok(Value::Int(set.len() as i64)) }
            else { Err(RuntimeError::TypeMismatch("HashSet::len expects HashSet".into())) }
        },
        "__builtin.HashSet.is_empty" => |args| {
            if let Value::HashSet(set) = &args[0] { Ok(Value::Bool(set.is_empty())) }
            else { Err(RuntimeError::TypeMismatch("HashSet::is_empty expects HashSet".into())) }
        },

        // StringBuilder
        "__builtin.StringBuilder.new" => |_args| Ok(Value::StringBuilder(String::new())),
        "__builtin.StringBuilder.append_char" => |args| {
            if let (Value::StringBuilder(buf), Value::Char(c)) = (&args[0], &args[1]) {
                let mut buf = buf.clone();
                buf.push(*c);
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::append_char expects (StringBuilder, Char)".into())) }
        },
        "__builtin.StringBuilder.append_str" => |args| {
            if let (Value::StringBuilder(buf), Value::String(s)) = (&args[0], &args[1]) {
                let mut buf = buf.clone();
                buf.push_str(s);
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::append_str expects (StringBuilder, String)".into())) }
        },
        "__builtin.StringBuilder.append_int" => |args| {
            if let (Value::StringBuilder(buf), Value::Int(n)) = (&args[0], &args[1]) {
                let mut buf = buf.clone();
                buf.push_str(&n.to_string());
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::append_int expects (StringBuilder, Int)".into())) }
        },
        "__builtin.StringBuilder.append_bool" => |args| {
            if let (Value::StringBuilder(buf), Value::Bool(b)) = (&args[0], &args[1]) {
                let mut buf = buf.clone();
                buf.push_str(&b.to_string());
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::append_bool expects (StringBuilder, Bool)".into())) }
        },
        "__builtin.StringBuilder.append_float" => |args| {
            if let (Value::StringBuilder(buf), Value::Float(f)) = (&args[0], &args[1]) {
                let mut buf = buf.clone();
                buf.push_str(&f.to_string());
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::append_float expects (StringBuilder, Float)".into())) }
        },
        "__builtin.StringBuilder.to_string" => |args| {
            if let Value::StringBuilder(buf) = &args[0] { Ok(Value::String(buf.clone())) }
            else { Err(RuntimeError::TypeMismatch("StringBuilder::to_string expects StringBuilder".into())) }
        },
        "__builtin.StringBuilder.clear" => |args| {
            if let Value::StringBuilder(buf) = &args[0] {
                let mut buf = buf.clone();
                buf.clear();
                Ok(Value::StringBuilder(buf))
            } else { Err(RuntimeError::TypeMismatch("StringBuilder::clear expects StringBuilder".into())) }
        },
        "__builtin.StringBuilder.len" => |args| {
            if let Value::StringBuilder(buf) = &args[0] { Ok(Value::Int(buf.len() as i64)) }
            else { Err(RuntimeError::TypeMismatch("StringBuilder::len expects StringBuilder".into())) }
        },
        "__builtin.StringBuilder.is_empty" => |args| {
            if let Value::StringBuilder(buf) = &args[0] { Ok(Value::Bool(buf.is_empty())) }
            else { Err(RuntimeError::TypeMismatch("StringBuilder::is_empty expects StringBuilder".into())) }
        },

        // Stack
        "__builtin.Stack.new" => |_args| Ok(Value::Stack(Vec::new())),
        "__builtin.Stack.push" => |args| {
            if let Value::Stack(stack) = &args[0] {
                let mut stack = stack.clone();
                stack.push(args[1].clone());
                Ok(Value::Stack(stack))
            } else { Err(RuntimeError::TypeMismatch("Stack::push expects Stack".into())) }
        },
        "__builtin.Stack.pop" => |args| {
            if let Value::Stack(stack) = &args[0] {
                let mut stack = stack.clone();
                match stack.pop() {
                    Some(item) => Ok(Value::Enum("Option".to_string(), "Some".to_string(),
                        HashMap::from([
                            ("0".to_string(), item),
                            ("1".to_string(), Value::Stack(stack)),
                        ]))),
                    None => Ok(Value::Enum("Option".to_string(), "None".to_string(), HashMap::new())),
                }
            } else { Err(RuntimeError::TypeMismatch("Stack::pop expects Stack".into())) }
        },
        "__builtin.Stack.peek" => |args| {
            if let Value::Stack(stack) = &args[0] {
                match stack.last() {
                    Some(item) => Ok(Value::Enum("Option".to_string(), "Some".to_string(),
                        HashMap::from([("value".to_string(), item.clone())]))),
                    None => Ok(Value::Enum("Option".to_string(), "None".to_string(), HashMap::new())),
                }
            } else { Err(RuntimeError::TypeMismatch("Stack::peek expects Stack".into())) }
        },
        "__builtin.Stack.len" => |args| {
            if let Value::Stack(stack) = &args[0] { Ok(Value::Int(stack.len() as i64)) }
            else { Err(RuntimeError::TypeMismatch("Stack::len expects Stack".into())) }
        },
        "__builtin.Stack.is_empty" => |args| {
            if let Value::Stack(stack) = &args[0] { Ok(Value::Bool(stack.is_empty())) }
            else { Err(RuntimeError::TypeMismatch("Stack::is_empty expects Stack".into())) }
        },
        "__builtin.Stack.clear" => |args| {
            if let Value::Stack(_) = &args[0] { Ok(Value::Stack(Vec::new())) }
            else { Err(RuntimeError::TypeMismatch("Stack::clear expects Stack".into())) }
        },

        // Queue
        "__builtin.Queue.new" => |_args| Ok(Value::Queue(std::collections::VecDeque::new())),
        "__builtin.Queue.enqueue" => |args| {
            if let Value::Queue(queue) = &args[0] {
                let mut queue = queue.clone();
                queue.push_back(args[1].clone());
                Ok(Value::Queue(queue))
            } else { Err(RuntimeError::TypeMismatch("Queue::enqueue expects Queue".into())) }
        },
        "__builtin.Queue.dequeue" => |args| {
            if let Value::Queue(queue) = &args[0] {
                let mut queue = queue.clone();
            ...
            } else { Err(RuntimeError::TypeMismatch("Queue::dequeue expects Queue".into())) }
        },
        "__builtin.Queue.front" => |args| {
            if let Value::Queue(queue) = &args[0] {
                match queue.front() {
                    Some(item) => Ok(Value::Enum("Option".to_string(), "Some".to_string(),
                        HashMap::from([("value".to_string(), item.clone())]))),
            ...
            } else { Err(RuntimeError::TypeMismatch("Queue::front expects Queue".into())) }
        },
        "__builtin.Queue.len" => |args| {
            if let Value::Queue(queue) = &args[0] { Ok(Value::Int(queue.len() as i64)) }
            else { Err(RuntimeError::TypeMismatch("Queue::len expects Queue".into())) }
        },
        "__builtin.Queue.is_empty" => |args| {
            if let Value::Queue(queue) = &args[0] { Ok(Value::Bool(queue.is_empty())) }
            else { Err(RuntimeError::TypeMismatch("Queue::is_empty expects Queue".into())) }
        },
        "__builtin.Queue.clear" => |args| {
            if let Value::Queue(_) = &args[0] { Ok(Value::Queue(std::collections::VecDeque::new())) }
            else { Err(RuntimeError::TypeMismatch("Queue::clear expects Queue".into())) }
        },

        // Result
        "__builtin.Result.is_ok" => |args| {
            if let Value::Enum(_, variant, _) = &args[0] {
                Ok(Value::Bool(variant == "Ok"))
            } else { Err(RuntimeError::TypeMismatch("Result::is_ok expects Result".into())) }
        },
        "__builtin.Result.is_err" => |args| {
            if let Value::Enum(_, variant, _) = &args[0] {
                Ok(Value::Bool(variant == "Err"))
            } else { Err(RuntimeError::TypeMismatch("Result::is_err expects Result".into())) }
        },
        "__builtin.Result.unwrap" => |args| {
            if let Value::Enum(_, variant, fields) = &args[0] {
                if variant == "Ok" {
                    Ok(fields.get("result").cloned().unwrap_or(Value::Void))
                } else {
                    Err(RuntimeError::TypeMismatch("unwrap on Err".into()))
                }
            } else { Err(RuntimeError::TypeMismatch("Result::unwrap expects Result".into())) }
        },
        "__builtin.Result.unwrap_err" => |args| {
            if let Value::Enum(_, variant, fields) = &args[0] {
                if variant == "Err" {
                    Ok(fields.get("error").cloned().unwrap_or(Value::Void))
                } else {
                    Err(RuntimeError::TypeMismatch("unwrap_err on Ok".into()))
                }
        ...
        "__builtin.Option.None" => |_args| Ok(Value::Enum("Option".to_string(), "None".to_string(), HashMap::new())),

        // String
        "__builtin.String.char_at" => |args| {
            if let (Value::String(s), Value::Int(idx)) = (&args[0], &args[1]) {
                Ok(s.chars().nth(*idx as usize).map(Value::Char).unwrap_or(Value::Char('\0')))
            } else { Err(RuntimeError::TypeMismatch("String::char_at expects (String, Int)".into())) }
        },
