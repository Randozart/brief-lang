...
pub fn describe_mapper_type(info: &MapperInfo) -> &'static str {
    match info.mapper_type {
        MapperType::Briev => "Briev mapper (.bv file)",
...
mod tests {
    ...
    fn test_describe_mapper_type() {
        let briev_info = MapperInfo {
            name: "briev".into(),
            path: PathBuf::from("briev.bv"),
        ...
        assert_eq!(describe_mapper_type(&briev_info), "Briev mapper (.bv file)");
