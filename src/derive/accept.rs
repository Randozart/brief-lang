// ── Phase I.1 — `brief accept` Command ────────────────────────────────
...
/// Handle `brief accept <file>` — fold doppelganger bodies into source.
...
pub fn handle_accept_command(file_path: &str, use_opt: bool) -> Result<(), String> {
    ...
    if !shadow_path.exists() {
        return Err(format!(
            "no doppelganger at '{}' — run 'brief derive' first",
            shadow_path.display()
        ));
    }
    ...
    if replacements.is_empty() {
        return Err("no bodies found in doppelganger — did 'brief derive' succeed?".into());
    }
