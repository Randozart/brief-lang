...
impl WebstackGenerator {
        ...
        for item in items {
            match item {
                ...
                TopLevel::ForeignBinding(fb) => {
                    self.ffi_bindings
                        .insert(fb.effective_briv_name().to_string(), fb.inputs.len());
                    if let Some(impl_code) = &fb.wasm_impl {
                        self.ffi_ts_impl.insert(fb.effective_briv_name().to_string(), impl_code.clone());
                    }
    ...
    fn generate_ts_code(&mut self, items: &[TopLevel], bindings: &[Binding]) -> String {
        ...
        out.push_str("// Generated by Briv Compiler\n");
