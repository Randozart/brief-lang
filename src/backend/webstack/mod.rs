...
// or embeds the Work.
//
// 2026-07-26: DEPRECATED — The WebstackGenerator (TypeScript emitter) is
// superseded by LlvmBackend(wasm32) + GlueWebGenerator for the webstack v2
// WASM-first architecture. This file is kept for the migration period but
// its codegen path is no longer used. New webstack code should use
...
pub struct WebstackGenerator {
    ...
    // 2026-07-26: Deprecated — kept for TS emitter backward compat during migration.
    // Superseded by GlueWebGenerator which uses type-driven WASM import stubs.
    ffi_ts_impl: HashMap<String, String>,
    ffi_ts_setups: std::collections::HashSet<String>,
...
impl WebstackGenerator {
    ...
    pub fn new() -> Self {
        WebstackGenerator {
            ...
            ffi_ts_setups: std::collections::HashSet::new(),
    ...
    pub fn generate(
        ...
        match self.target {
            CodeTarget::Wasm => {
                let ts_code = self.generate_ts_code(items, bindings);
        ...
        for item in items {
            match item {
                TopLevel::StateDecl(decl) => {
                    ...
                    // StateDecl no longer has an expr field; default initializer
                ...
                TopLevel::Trigger(trg) => {
                    ...
                    let signal_type = match &trg.instance {
                        Expr::Identifier(name) if name == "env" => SignalType::Int,
                        _ => SignalType::Int,
                    };
                ...
                TopLevel::ForeignBinding(fb) => {
                    self.ffi_bindings
                        .insert(fb.effective_briev_name().to_string(), fb.inputs.len());
                    if let Some(impl_code) = &fb.wasm_impl {
                        self.ffi_ts_impl.insert(fb.effective_briev_name().to_string(), impl_code.clone());
                    }
    ...
    fn generate_ts_code(&mut self, items: &[TopLevel], bindings: &[Binding]) -> String {
        let mut out = String::new();

        out.push_str("// Generated by Briev Compiler\n");
    ...
    fn statement_to_ts(&mut self, out: &mut String, stmt: &Statement) {
        match stmt {
            ...
            // 2026-08-06 (endprogram plan): process boundary — exit the JS
            // host, not just the current transaction. In browsers this would
            // be the harness's exit hook; in Node it is process.exit.
            Statement::EndProgram(swan_song) => {
                if let Some(swan) = swan_song {
                    self.statement_to_ts(out, &Statement::Expression(swan.clone()));
                }
                out.push_str("if (typeof process !== 'undefined') process.exit(0); throw new Error('endprogram');\n");
            }
    ...
    fn expr_to_ts(&mut self, expr: &Expr) -> String {
        match expr {
            ...
            Expr::BinaryOp(kind, l, r) => {
                let l = self.expr_to_ts(l);
                let r = self.expr_to_ts(r);
                match kind {
                    ...
                    BinaryOpKind::Concat => format!("({} + {})", l, r),
                }
            }
            Expr::UnaryOp(kind, e) => {
                let op = self.expr_to_ts(e);
                match kind {
            ...
            Expr::Call(name, args, _) => {
                ...
                if self.ffi_ts_impl.contains_key(name) {
                    ...
                    match name.as_str() {
                        ...
                        "Floor#" => format!("Math.floor({})", ts_args.join(", ")),
                        "Sin#" => format!("Math.sin({})", ts_args.join(", ")),
                        "Cos#" => format!("Math.cos({})", ts_args.join(", ")),
                        "Pow#" => format!("Math.pow({})", ts_args.join(", ")),
                        "Reverse#" => format!("[...{}].reverse()", ts_args.join(", ")),
                        "Sort#" => format!("[...{}].sort()", ts_args.join(", ")),
                        "Range#" => format!("[...Array({}).keys()]", ts_args.join(", ")),
                        "PageSize#" => "4096".to_string(),
                        "CpuCount#" => "navigator.hardwareConcurrency || 4".to_string(),
                        "Hostname#" => "window.location.hostname".to_string(),
                        "Errno#" => "0".to_string(),
                        "GetGlobalId#" | "GetLocalId#" | "GetGroupId#" => "0".to_string(),
                        _ => format!("this.{}({})", ts_name, ts_args.join(", ")),
                    }
                }
            }
            Expr::List(items) => {
    ...
    fn generate_rust_code(&mut self, _items: &[TopLevel], _bindings: &[Binding]) -> String {
        String::new()
    }

    /// Legacy stub — needed by archive.
    fn generate_transaction(&mut self, _output: &mut String, _txn: &Transaction) {}

    /// Emit TypeScript code for a statement.
    /// 2026-07-26: ARM backend removed; this function now serves the TS emitter path.
    /// Name kept for backward compat with callers in generate_ts_code.
    fn statement_to_rust(&mut self, out: &mut String, stmt: &Statement) {
        match stmt {
            ...
            // 2026-08-06 (endprogram plan): process boundary — exit the host,
    ...
    /// Emit TypeScript expression string.
    /// 2026-07-26: ARM backend removed; name kept for backward compat.
    fn expr_to_rust(&mut self, expr: &Expr) -> String {
        match expr {
            ...
            Expr::BinaryOp(kind, l, r) => {
                let l = self.expr_to_rust(l);
                let r = self.expr_to_rust(r);
                match kind {
                    BinaryOpKind::Add => format!("({} + {})", l, r),
                    BinaryOpKind::Sub => format!("({} - {})", l, r),
                    BinaryOpKind::Mul => format!("({} * {})", l, r),
            ...
            Expr::Call(name, args, _) => {
                ...
                match name.as_str() {
                    ...
                    "Floor#" => format!("({} as f64).floor() as i64", joined),
                    _ => format!("{}({})", name.replace('-', "_"), joined),
                }
            }
            Expr::List(items) => {
...
mod tests {
    ...
    fn test_ts_int_division_trunc() {
        let mut backend = WebstackGenerator::new();
        let out = backend.expr_to_ts(&Expr::BinaryOp(
            BinaryOpKind::Div,
            ...
            TopLevel::Transaction(Transaction {
                ...
                body: vec![
                    Statement::Assign(
                        ...
                        Expr::BinaryOp(BinaryOpKind::Add, Box::new(Expr::Identifier("count".into())), Box::new(Expr::Decimal(1))),
                    ),
                    Statement::Term(None),
                ],
    ...
    fn test_statement_to_ts_sync_block() {
        let mut backend = WebstackGenerator::new();
        let mut out = String::new();
        let stmt = Statement::SyncBlock(vec![
            Statement::Expression(Expr::BinaryOp(BinaryOpKind::Add, Box::new(Expr::Identifier("a".into())), Box::new(Expr::Decimal(1)))),
        ]);
        backend.statement_to_ts(&mut out, &stmt);
    ...
    fn test_statement_to_rust_assignment() {
        let mut backend = WebstackGenerator::new();
        let mut out = String::new();
        backend.statement_to_rust(&mut out, &Statement::Assign(
            Expr::Identifier("count".into()),
            Expr::BinaryOp(BinaryOpKind::Add, Box::new(Expr::Identifier("count".into())), Box::new(Expr::Decimal(1))),
        ));
    ...
    fn test_statement_to_rust_guarded() {
        ...
        backend.statement_to_rust(&mut out, &Statement::Guarded(
            Expr::BinaryOp(BinaryOpKind::Lt, Box::new(Expr::Identifier("count".into())), Box::new(Expr::Decimal(10))),
            vec![
                Statement::Assign(
                    Expr::Identifier("count".into()),
                    Expr::BinaryOp(BinaryOpKind::Add, Box::new(Expr::Identifier("count".into())), Box::new(Expr::Decimal(1))),
