...
impl WebstackGenerator {
    ...
    pub fn generate(
        ...
        match self.target {
            ...
            CodeTarget::Arm => {
                let rust_code = self.generate_arm_rust_code(items);
                WebstackOutput {
                    ts_code: String::new(),
                    rust_code,
                    js_glue: String::new(),
    ...
    fn generate_arm_rust_code(&mut self, items: &[TopLevel]) -> String {
        ...
        for name in &state_decls {
            output.push_str(&format!("    pub {}: i32,\n", name.replace('-', "_")));
        }
        output.push_str("}\n\n");
