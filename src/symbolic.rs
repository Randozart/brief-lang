...
pub fn eval_symbolic(expr: &Expr, state: &SymbolicState) -> SymbolicValue {
    match expr {
        ...
        Expr::Term | Expr::Slice { .. } | Expr::MultiSlice { .. } | Expr::ForAll { .. } | Expr::Exists { .. } | Expr::Block(_, _) | Expr::TupleDestructure(_, _) | Expr::Tuple(_) | Expr::Concat(_, _) | Expr::Cast(_, _) => SymbolicValue::Unknown,
    }
}
