...
from matching on its Briev type name string. The `ResolvedType` contains:
...
These take a Briev type and return an LLVM type string. They all duplicate
...
```rust
/// 2026-07-26: Returns the LLVM type for a Briev type, driven by the
...
correct — these are LLVM IR types, not Briev type names. The dispatch is on
...
`("i8", "i64")`, `("i32", "i64")`). These are LLVM type conversions, not Briev
