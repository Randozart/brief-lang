...
| Benchmark | Current Ratio | Best Ratio | Era | Briev Gap |
|-----------|-------------|------------|-----|-----------|
| sparse_dispatch | 0.82x | 0.09x | 5 | **0.73x → Briev loses** (previously won by 11×) |
| queue_drain | 0.97x | 0.01x | 5 | **0.96x → Briev loses** (previously won by 100×) |
...
Compile and run. Expected: Briev time drops from ~0.05s to ~0.001s.

**V2 (queue_drain):** Manually hoist the guard body from the hot path and fold the remaining
body. Expected: Briev time drops from ~0.06s to ~0.001s.

**V3 (interval_step):** Same as V2. Expected: Briev time drops from ~0.06s to ~0.001s.
...
| Benchmark | Current Ratio | Best Ratio | Era | Briev Gap |
|-----------|-------------|------------|-----|-----------|
| nbody_newton | 1.05x | 0.75x | 5 | **0.30x → Briev loses** (previously won by 33%) |
| nbody_sqrt | 0.97x | 0.85x | 10 | **0.12x → within parity** |
| nbody_sqrt_idio | 0.93x | 0.67x | 10 | **0.26x → Briev loses** (previously won by 49%) |
...
**V2:** Run nbody_sqrt_idio with forced off. If Briev time drops from ~3.6s toward ~2.6s,
...
| Benchmark | Current Ratio | Best Ratio | Era | Briev Gap |
|-----------|-------------|------------|-----|-----------|
| float_math | 0.97x | 0.81x | 5 | **0.16x → Briev loses** (previously won by 23%) |
...
Compile and run. Expected: Briev time drops from ~0.071s toward ~0.063s.
