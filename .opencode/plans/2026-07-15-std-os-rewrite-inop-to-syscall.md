...
Replace 98 `inop` declarations across 18 files in `lib/std/os/` with pure-Brief
...
| `lib/runtime/brief_rt.c` | 1 file | Keep `brief_syscall`, remove ~70 `brief_*` functions after migration |
...
### 5.1 The Brief string format

Brief strings are stored as: `[8-byte length prefix][UTF-8 data]`. The
...
// Input: ptr to Brief string (length prefix + UTF-8 data)
...
`brief_*` functions from `lib/runtime/brief_rt.c`. Keep:

- `brief_syscall` (the new core)
- `__rt_init`, `__rt_wait` (runtime initialization — needed by main)
- `brief_str_to_c` (until Phase 2 is complete)
- `brief_pagesize`, `brief_cpu_count` (until `Sysconf#` is added)

Delete all other `brief_*` functions (~50 functions).
...
- `src/backend/llvm/intrinsics.rs` — add `test_syscall_emits_call`:
  Verify `Syscall#(Open, 1,2,3,4,5,6)` emits `call i64 @brief_syscall`
...
After Phase 1, verify that `brief check` passes for a program using
...
| `brief check` on rewritten .bv files | Each `lib/std/os/*.bv` parses and passes type check |
