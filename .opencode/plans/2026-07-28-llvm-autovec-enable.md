...
Every reactive txn in Briev has a convergence contract: `[pre][post]`. The pre-condition
...
would be a miscompilation risk. But such programs violate the Briev language specification
(§3.2 of spec/SPEC.md: "Every txn must have a postcondition that provably terminates").

Adding `willreturn` to `#11` is therefore semantically correct for all valid Briev programs.
