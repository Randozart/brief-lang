...
# 3. Compare against our Briev-generated .ll
...
```bash
# Briev binary assembly
objdump -d /tmp/nbody_newton > /tmp/briev_nbody.s
# C reference assembly
objdump -d /tmp/nbody_newton_c > /tmp/clang_nbody.s

# Compare hot loop regions
diff /tmp/briev_nbody.s /tmp/clang_nbody.s | head -300
```
...
opt -O3 -S \
    ...
    briev_nbody.ll -o /dev/null 2>&1 | head -50
opt -O3 -S \
    -pass-remarks-missed=sroa,slp-vectorizer,loop-vectorize \
...
1. A table: "Clang vs Briev IR comparison" for all 19 benchmark C sources
2. A file: clang-generated `.ll` files for each C benchmark (saved to `docs/reference-ll/clang/`)
3. A file: LLVM pass-remarks for both Briev and clang IR
4. A written analysis: "The 3-5 things clang does differently from Briev"
