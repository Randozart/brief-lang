...
1. `cargo build`
2. `./target/debug/brief-compiler check lib/compiler/*.bv`
3. `./target/debug/brief-compiler check lib/std/*.bv`
4. `./target/debug/brief-compiler check lib/ffi/mappers/*.bv`
5. `./target/debug/brief-compiler check lib/ffi/mappers/**/*.bv`
6. `cargo test --lib`

## Files NOT to touch
- `src/dbrief/parser.rs` — `"result"` is the Dbrief Result<S,E> type keyword
